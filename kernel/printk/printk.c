...
static char * __initdata default_str = "console=tty0 console=/dev/null,115200n1";
static int __init default_console_setup(char *str)
{
	if (strstr(boot_command_line, "console=") == NULL) {
		printk(KERN_INFO "%s: %s\n", __func__, default_str);
		console_setup(default_str);
		printk_disable_uart = true;
	}

	return 1;
}
__setup("sec_dummy=", default_console_setup);
