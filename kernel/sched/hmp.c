...
extern atomic_t boost_refcount[];

#define fboost (atomic_read(&boost_refcount[1]))
#define sboost (atomic_read(&boost_refcount[2]))

#define hmp_boost (fboost || sboost)
#define sboost_only (!fboost && sboost)

/*
 * Background task affinity = 0-1. binary = 0000 0011. hexa = 0x03
 * System Background task affinity = 0-2. binary = 0000 0111. hexa = 0x07
 * Foreground task affinity = 0-2, 4-7. binary = 1111 0111. hexa = 0xF7
 */
static unsigned long background_mask = 0x03;
static unsigned long sys_background_mask = 0x07;
static unsigned long foreground_mask = 0xF7;

static int task_is_fg(struct task_struct *p)
{
	/*If this task is background*/
	if (cpumask_equal(tsk_cpus_allowed(p), to_cpumask(&background_mask)) || 
		cpumask_equal(tsk_cpus_allowed(p), to_cpumask(&sys_background_mask))) {
		return 0;
	}
	/*If this task is foreground*/
	if (cpumask_equal(tsk_cpus_allowed(p), to_cpumask(&foreground_mask))) {
		return 1;
	}
...
#ifdef CONFIG_SCHED_HMP_BOOST
	/*Least loaded (preferably idle) cores from big and little are eligible,
	  but we prefer big core for foreground and top-task while boosted*/
	if (hmp_boost && task_is_fg(p)) {
		goto select_fast;
	}
#endif
...
#ifdef CONFIG_SCHED_HMP_BOOST
	/*
	 * Our objective: Prioritize FG task for up-migration, then let other recent task with high load
	 * to migrate as well. To migrate other task (after FG has been checked), we half the dynamic
	 * threshold.
	 * E.g. if threshold is 1023(highest), then make it 511. If threshold is 100, make it 50.
	 * This ensures that high threshold are reduced by a good amount, and little impact on low 
	 * threshold (since it is already low).
	 * Second thing it ensures is threshold value remain in 0 to 500
	 * If 'only' semi-booster is on, then check for normal threshold reduced by 30%
	 */
	if (fboost)
    		boost_flag = task_is_fg(task_of(se)) || (se_load(se) > (B->threshold/2));
	else if (sboost_only)
	{
    		boost_flag = task_is_fg(task_of(se)) || (se_load(se) > (B->threshold - (B->threshold*3)/10));
	}

	if (boost_flag || (se_load(se) > B->threshold)) {
		check->status |= HMP_MIGRATION_APPROVED;
		check->result = 1;
	}
#else
