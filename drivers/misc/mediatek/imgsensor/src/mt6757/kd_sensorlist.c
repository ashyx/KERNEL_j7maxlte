...
#define SYSFS_CAMERA
#ifdef SYSFS_CAMERA
#if defined(titan6757_c10_n)
#include <sm5705/sm5705_flash.h>
#else
extern ssize_t rear_flash_show(struct device *dev, struct device_attribute *attr, char *buf);
extern ssize_t front_flash_show(struct device *dev, struct device_attribute *attr, char *buf);
extern ssize_t rear_flash_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size);
extern ssize_t front_flash_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size);
#endif
#endif
	...
	if (current && current->fs) {
		...
		if (ret != 0) {
			...
			if (IS_ERR_OR_NULL(fp)) {
				pr_err("%s open failed", CAM_HW_ERR_CNT_FILE_PATH);
				sys_umask(old_mask);
				set_fs(old_fs);
				return;
			}
...
void is_sec_copy_err_cnt_from_file(void)
{
	...
	pr_info("[BigData] %s REAR i2c_af(%d), i2c_sensor(%d), mipi_sensor(%d)\n", __func__,
		cam_hwparam_collector.rear_hwparam.i2c_af_err_cnt,
		cam_hwparam_collector.rear_hwparam.i2c_sensor_err_cnt,
		cam_hwparam_collector.rear_hwparam.mipi_sensor_err_cnt);
	pr_info("[BigData] %s FRONT i2c_sensor(%d), mipi_sensor(%d)\n", __func__,
		cam_hwparam_collector.front_hwparam.i2c_sensor_err_cnt,
		cam_hwparam_collector.front_hwparam.mipi_sensor_err_cnt);
...
// sysfs functions
#if !defined(titan6757_phone_n) // J7Max is after PVR
#define SYSFS_BEFORE_PV
#endif

#define REAR_CAMFW_PREFIX "W13KSIA"
#define FRONT_CAMFW_PREFIX "X13KLIA"

extern void get_rear_mfr_info(u8 *data);
extern void get_front_mfr_info(u8 *data);

extern unsigned int get_rear_af_macro_cal(void);
extern unsigned int get_rear_af_pan_cal(void);
extern void get_rear_moduleid(u8 *data);
extern void get_front_moduleid(u8 *data);

static ssize_t rear_camfw_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char fw_version[12]={0};
	get_rear_mfr_info(fw_version);
	pr_err("%s : fw ver: %s\n", __func__, fw_version);
...
static ssize_t rear_checkfw_factory_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char *retStr = "OK";
	char fw_version[12]={0};
	char *ref_version = REAR_CAMFW_PREFIX;
	get_rear_mfr_info(fw_version);
...
#if defined(titan6757_jade_n) || defined(titan6757_c10_n)
static ssize_t rear2_caminfo_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	// TODO: check its value.
	char *camInfoStr = "VALID=Y;";
	pr_err("%s : %s\n", __func__, camInfoStr);
	return snprintf(buf, 100, camInfoStr);
}
#endif
...
{
	unsigned int macro_cal = get_rear_af_macro_cal();
	unsigned int pan_cal = get_rear_af_pan_cal();	
...
static ssize_t front_checkfw_factory_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char *retStr = "OK";
	char fw_version[12]={0};
	char *ref_version = FRONT_CAMFW_PREFIX;
	get_front_mfr_info(fw_version);
	if(!strncmp(fw_version, ref_version, strlen(ref_version))) {
#ifdef SYSFS_BEFORE_PV
		retStr = "OK";
...
static ssize_t front_checkfw_user_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char *retStr = "OK";
	char fw_version[12]={0};
	char *ref_version = FRONT_CAMFW_PREFIX;
...
#define FROM_MODULE_ID_SIZE	10
	...
	get_rear_moduleid(rear_module_id);
...
static ssize_t front_cam_moduleid_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
#define FROM_MODULE_ID_SIZE	10
	char front_module_id[FROM_MODULE_ID_SIZE + 1] = "\0";
	get_front_moduleid(front_module_id);
...
#define FROM_MODULE_ID_SIZE	10
	...
	char fw_version[12]={0};
	pr_info("[Bigdata] %s\n", __func__);
	is_sec_get_rear_hw_param(&ec_param);

	get_rear_moduleid(rear_module_id);
	get_rear_mfr_info(fw_version);

	return sprintf(buf, "\"CAMIR_ID\":\"%c%c%c%c%cXX%02X%02X%02X\","
		"\"CAMFWR\":\"%s %s\","
		"\"I2CR_AF\":\"%d\",\"I2CR_SEN\":\"%d\",\"MIPIR_SEN\":\"%d\"\n",
				    ...
				    struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "c", 1)) {
		...
		if (ec_param)
			is_sec_init_err_cnt_file(ec_param);
	}
...
#define FROM_MODULE_ID_SIZE	10
	...
	char fw_version[12]={0};
	pr_info("[Bigdata] %s\n", __func__);
	is_sec_get_front_hw_param(&ec_param);

	get_front_moduleid(front_module_id);
	get_front_mfr_info(fw_version);

	return sprintf(buf, "\"CAMIF_ID\":\"%c%c%c%c%cXX%02X%02X%02X\","
		"\"CAMFWF\":\"%s %s\","
...
static DEVICE_ATTR(rear_hwparam, S_IRUGO|S_IWUSR|S_IWGRP, rear_camera_hw_param_show, rear_camera_hw_param_store);
static DEVICE_ATTR(front_hwparam, S_IRUGO|S_IWUSR|S_IWGRP, front_camera_hw_param_show, front_camera_hw_param_store);
#endif		/* CONFIG_CAMERA_HW_BIG_DATA */
...
#if defined(titan6757_jade_n) || defined(titan6757_c10_n)
// for main camera2 test in factory mode
static DEVICE_ATTR(rear2_caminfo, S_IRUGO, rear2_caminfo_show, NULL);
#endif
...
static DEVICE_ATTR(front_checkfw_factory, S_IRUGO, front_checkfw_factory_show, NULL);
static DEVICE_ATTR(front_checkfw_user, S_IRUGO, front_checkfw_user_show, NULL);
...
static DEVICE_ATTR(rear_flash_2, S_IRUGO|S_IWUSR|S_IWGRP, rear_flash_show, rear_flash_store);
#else
static DEVICE_ATTR(rear_flash, S_IRUGO|S_IWUSR|S_IWGRP, rear_flash_show, rear_flash_store);
...
static DEVICE_ATTR(front_moduleid, S_IRUGO, front_cam_moduleid_show, NULL);

static DEVICE_ATTR(SVC_rear_module, S_IRUGO, rear_cam_moduleid_show, NULL);
static DEVICE_ATTR(SVC_front_module, S_IRUGO, front_cam_moduleid_show, NULL);
...
int svc_cheating_prevent_device_file_create(void)
{
	...
	if (IS_ERR_OR_NULL(svc_sd)) {
		...
		pr_err("Success to find svc_sd : 0x%p svc : 0x%p\n", svc_sd, data);
	}

	Camera = kobject_create_and_add("Camera", data);
	if (IS_ERR_OR_NULL(Camera))
	        pr_err("Failed to create sys/devices/svc/Camera : 0x%p\n", Camera);
	else
		pr_err("Success to create sys/devices/svc/Camera : 0x%p\n", Camera);

	err = sysfs_create_file(Camera, &dev_attr_SVC_rear_module.attr);
	if (unlikely(err < 0)) {
		pr_err("rear_sysfs: failed to create device file, %s\n",
				dev_attr_SVC_rear_module.attr.name);
	}

	err = sysfs_create_file(Camera, &dev_attr_SVC_front_module.attr);
	if (unlikely(err < 0)) {
		pr_err("rear_sysfs: failed to create device file, %s\n",
				dev_attr_SVC_rear_module.attr.name);
	}
...
int create_rear_sysfs(void)
{
...
#if defined(titan6757_jade_n) || defined(titan6757_c10_n)
	err = device_create_file(rear_dev, &dev_attr_rear2_caminfo);
	if (unlikely(err < 0)) {
		pr_err("rear_sysfs: failed to create device file, %s\n",
	...
	err = device_create_file(rear_dev, &dev_attr_rear_hwparam);
	if (unlikely(err < 0)) {
		pr_err("rear_sysfs: failed to create device file, %s\n",
...
int create_front_sysfs(void)
{
	...
	err = device_create_file(front_dev, &dev_attr_front_checkfw_factory);
	if (unlikely(err < 0)) {
		pr_err("front_sysfs: failed to create device file, %s\n",
				dev_attr_front_checkfw_factory.attr.name);
	}

	err = device_create_file(front_dev, &dev_attr_front_checkfw_user);
	if (unlikely(err < 0)) {
		pr_err("front_sysfs: failed to create device file, %s\n",
	...
	err = device_create_file(front_dev, &dev_attr_front_moduleid);
	if (unlikely(err < 0)) {
		pr_err("front_sysfs: failed to create device file, %s\n",
				dev_attr_front_moduleid.attr.name);
	}

	err = device_create_file(front_dev, &dev_attr_front_hwparam);
	if (unlikely(err < 0)) {
		pr_err("rear_sysfs: failed to create device file, %s\n",
...
#if defined(titan6757_c10_n)
	err = device_create_file(flash_dev, &dev_attr_rear_flash_2);
	if (unlikely(err < 0)) {
		pr_err("flash_sysfs: failed to create device file, %s\n",
				dev_attr_rear_flash_2.attr.name);
	}
#else
...
static inline int adopt_CAMERA_HW_CheckIsAlive(void)
{
	...
	KD_IMGSENSOR_PROFILE_INIT();
	/* power on sensor */
	err_PwrOn = kdModulePowerOn((CAMERA_DUAL_CAMERA_SENSOR_ENUM *) g_invokeSocketIdx, g_invokeSensorNameStr,
		...
		for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
			if (g_invokeSocketIdx[i] != DUAL_CAMERA_NONE_SENSOR) {
				...
				if (sensorID == 0) {	/* not implement this feature ID */
					...
					printk("[DHL] No Sensor Found\n");
					err = ERROR_SENSOR_CONNECT_FAIL;
				} else {

					printk("[DHL] Sensor found ID = 0x%x\n", sensorID);
...
bool _hwPowerOn(PowerType type, int powerVolt)
{
	...
	if (!IS_ERR(reg)) {
        ...
        if (type == SUB_DVDD && powerVolt == Vol_1050) {
            if (regulator_set_voltage(reg, 1000000, 1000000) !=0 ) {
                PK_DBG("[_hwPowerOn]fail to regulator_set_voltage, powertype:%d powerId:%d\n",
                   type, powerVolt);
                return ret;
            }
            if (pmic_config_interface(0x1A74, 5, 0xf, 0)) {
                PK_DBG("[_hwPowerOn]fail to regulator_set_voltage, powertype:%d powerId:%d\n",
                   type, powerVolt);
                return ret;
            }
...
static int CAMERA_HW_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (camera_class == NULL) {
		...
		if (IS_ERR(camera_class))
			PK_ERR("failed to create device cam_dev_rear!\n");
	}
	create_rear_sysfs();
	create_front_sysfs();
	create_flash_sysfs();
	PK_DBG("[%s] create_camera_sysfs finished\n", __func__);
#endif
