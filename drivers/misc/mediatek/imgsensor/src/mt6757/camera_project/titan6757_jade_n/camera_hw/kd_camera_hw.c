...
PowerCust PowerCustList = {
	{
		...
		{GPIO_UNSUPPORTED, GPIO_MODE_GPIO, Vol_High},	/* for AFVDD; */
		...
		{GPIO_UNSUPPORTED, GPIO_MODE_GPIO, Vol_Low},	/* for SUB_AFVDD; */		
		...
		{GPIO_UNSUPPORTED, GPIO_MODE_GPIO, Vol_Low}       /* for MAIN2_AFVDD; */
	}
};
...
#endif
#if defined(IMX241_MIPI_MONO)
		{SENSOR_DRVNAME_IMX241_MIPI_MONO,
			{
				{SensorMCLK, Vol_High, 0},
				{RST, Vol_Low, 0},
				{SUB_DOVDD, Vol_1800, 0},
...
struct pinctrl_state *cam_ldo_main2_vcama_h;
...
//--------------------------------------------------------------------------
	cam_ldo_main2_vcama_h = pinctrl_lookup_state(camctrl, "cam_ldo_monocam_vcama_1");
	if (IS_ERR(cam_ldo_main2_vcamd_h)) {
		ret = PTR_ERR(cam_ldo_main2_vcamd_h);
		PK_DBG("%s : pinctrl err, cam_ldo_main2_vcamd_h\n", __func__);
	}

	cam_ldo_main2_vcama_l = pinctrl_lookup_state(camctrl, "cam_ldo_monocam_vcama_0");
	if (IS_ERR(cam_ldo_main2_vcamd_l)) {
		ret = PTR_ERR(cam_ldo_main2_vcamd_l);
		PK_DBG("%s : pinctrl err, cam_ldo_main2_vcamd_l\n", __func__);
	}

	cam_ldo_main2_vcamd_h = pinctrl_lookup_state(camctrl, "cam_ldo_monocam_vcamd_1");
	if (IS_ERR(cam_ldo_main2_vcamd_h)) {
		ret = PTR_ERR(cam_ldo_main2_vcamd_h);
		PK_DBG("%s : pinctrl err, cam_ldo_main2_vcamd_h\n", __func__);
	}

	cam_ldo_main2_vcamd_l = pinctrl_lookup_state(camctrl, "cam_ldo_monocam_vcamd_0");
	if (IS_ERR(cam_ldo_main2_vcamd_l)) {
		ret = PTR_ERR(cam_ldo_main2_vcamd_l);
		PK_DBG("%s : pinctrl err, cam_ldo_main2_vcamd_l\n", __func__);
	}
...
if (!IS_ERR(cam_mipi_switch_en_h)){
	printk("[DHL]cam_mipi_switch_en_h..!!\n");
	pinctrl_select_state(camctrl, cam_mipi_switch_en_h);
} 


if (Val == 1 && !IS_ERR(cam_mipi_switch_sel_h)){
	printk("[DHL]cam_mipi_switch_sel_h..!!\n");
	pinctrl_select_state(camctrl, cam_mipi_switch_sel_h);
}else{
	printk("[DHL]cam_mipi_switch_sel_l..!!\n");
...
break;
		...
		if (Val == 0 && !IS_ERR(cam_mipi_switch_en_l)){
			printk("[DHL]cam_mipi_switch_en_l..!!\n");
			pinctrl_select_state(camctrl, cam_mipi_switch_en_l);
		}else if (Val == 1 && !IS_ERR(cam_mipi_switch_en_h)){
			printk("[DHL]cam_mipi_switch_en_h..!!\n");
			pinctrl_select_state(camctrl, cam_mipi_switch_en_h);
		}else
			PK_ERR("%s : pinctrl err, PinIdx %d, Val %d, RST\n", __func__,
	...
	case DOVDD:
			printk("[DHL] DOVDD PinIdx = %d Val = %d\n",PinIdx,Val);
		if (PinIdx == 0) {
...
BOOL hwpoweron(PowerInformation pwInfo, char *mode_name)
{
	...
	printk("[DHL]hwpoweron hwpoweron =%d, pinSetIdx=%d \n", pwInfo.PowerType,pinSetIdx);
	...
	else if (pwInfo.PowerType == SUB_DOVDD) {
		if (PowerCustList.PowerCustInfo[SUB_DOVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
	...
	else if (pwInfo.PowerType == MIPI_SW) {
		/*mtkcam_gpio_set(pinSetIdx, MIPI_SW, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]); */
		printk("[DHL]pwInfo.PowerType == MIPI_SW [%d]..!!\n",pwInfo.Voltage);
		if (pwInfo.Voltage == Vol_High) {
			if (mtkcam_gpio_set(pinSetIdx, MIPI_SW, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_ON]))
				PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
		} else {
			if (mtkcam_gpio_set(pinSetIdx, MIPI_SW, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]))
			PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
		}
	...
	if (pwInfo.PowerType == AVDD)
	{
		if (pinSetIdx == 2)
		{
			if (PowerCustList.PowerCustInfo[CUST_MAIN2_AVDD].Gpio_Pin == GPIO_UNSUPPORTED)
			{
				...
				printk("[DHL] hwpowerdown MAIN2_AVDD \n");
			}
			else
			{
		...
		else if (pinSetIdx == 1)
		{
			if (PowerCustList.PowerCustInfo[CUST_SUB_AVDD].Gpio_Pin == GPIO_UNSUPPORTED)
			{
				...
				printk("[DHL] hwpowerdown SUB_AVDD \n");
			}
			else
			{
	...
	else if (pwInfo.PowerType == DVDD)
	{
		if (pinSetIdx == 2)
		{
			if (PowerCustList.PowerCustInfo[CUST_MAIN2_DVDD].Gpio_Pin == GPIO_UNSUPPORTED)
			{
				...
				printk("[DHL] hwpowerdown MAIN2_DVDD \n");
			}
			else
			{
		...
		else
		{
			if (PowerCustList.PowerCustInfo[CUST_DVDD].Gpio_Pin == GPIO_UNSUPPORTED)
			{
				...
				printk("[DHL] hwpowerdown SUB_SUB_DVDD \n");
			}
			else
			{
