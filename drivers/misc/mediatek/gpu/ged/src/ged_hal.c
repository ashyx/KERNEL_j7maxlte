...
static struct dentry *gpsDvfsListFreqEntry;
...
/*-----------------------------------------------------------------------------*/
static void *gpu_freq_list_start(struct seq_file *psSeqFile, loff_t *puiPosition)
{
	if (*puiPosition == 0)
		return SEQ_START_TOKEN;
	return NULL;
...
/*-----------------------------------------------------------------------------*/
static void gpu_freq_list_stop(struct seq_file *psSeqFile, void *pvData)
{

}
/*-----------------------------------------------------------------------------*/
static void *gpu_freq_list_next(struct seq_file *psSeqFile, void *pvData, loff_t *puiPosition)
{
	return NULL;
}
/*-----------------------------------------------------------------------------*/
static int gpu_freq_list_show(struct seq_file *psSeqFile, void *pvData)
{
	int i;
	int maxLevel;

	if (pvData != NULL) {
		maxLevel = mt_gpufreq_get_dvfs_table_num();
...
/*-----------------------------------------------------------------------------*/
static struct seq_operations gsGpuListFreqReadOps = {
	.start = gpu_freq_list_start,
	.stop = gpu_freq_list_stop,
	.next = gpu_freq_list_next,
	.show = gpu_freq_list_show,
};
/*-----------------------------------------------------------------------------*/
...
GED_ERROR ged_hal_init(void)
{
	...
	/* List available gpu freq */
	err = ged_debugFS_create_entry(
			"gpufreq_available_freq",
			gpsHALDir,
			&gsGpuListFreqReadOps,
			NULL,
			NULL,
			&gpsDvfsListFreqEntry);
	...
	/* Control the gpu lowbound freq (freq type)*/
	err = ged_debugFS_create_entry(
			"gpu_lowbound_freq",
			gpsHALDir,
	...
	/* Control the gpu upbound freq (freq type)*/
	err = ged_debugFS_create_entry(
			"gpu_upbound_freq",
			gpsHALDir,
...
void ged_hal_exit(void)
{
	...
	ged_debugFS_remove_entry(gpsDvfsListFreqEntry);
