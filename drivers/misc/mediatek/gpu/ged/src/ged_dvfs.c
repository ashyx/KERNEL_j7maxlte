...
static void ged_dvfs_set_gpu_lowbound_freq(unsigned long ulFreq)
{
	int level;

	level = ged_dvfs_freq_to_freqID(ulFreq);
	ged_dvfs_custom_boost_gpu_freq(level);
}
static void ged_dvfs_set_gpu_upbound_freq(unsigned long ulFreq)
{
	int level;

	level = ged_dvfs_freq_to_freqID(ulFreq);
	ged_dvfs_custom_ceiling_gpu_freq(level);
}
#endif
...
GED_ERROR ged_dvfs_system_init()
{
...
#endif
	/* Titan workaround: forced enable emu, it will user timer base dvfs algo */
	g_gpu_timer_based_emu = 1;
	...
	mtk_set_gpu_lowbound_freq_fp = ged_dvfs_set_gpu_lowbound_freq;
	mtk_set_gpu_upbound_freq_fp = ged_dvfs_set_gpu_upbound_freq;
