...
typedef struct {
	...
	unsigned int (*dsi_dcs_read_cmdq_lcm_reg_v2_1)(void *cmdq, unsigned char data_id,
		unsigned char offset, unsigned char cmd, unsigned char *buffer, unsigned char buffer_size);
...
typedef struct {
	...
	void (*dsi_set_withcmdq)(void *cmdq, unsigned cmd, unsigned char count,
			unsigned char *para_list, unsigned char force_update);
	int (*read_by_cmdq)(void *cmdq, unsigned data_id, unsigned offset,
			unsigned cmd, unsigned char *buffer, unsigned char size);

	void (*init)(void);
#if defined(CONFIG_LCD_GENERIC)
	void (*reduced_init) (int is_init); /*Add panel start check at frist booting*/
#endif
	void (*suspend)(void);
	void (*resume)(void);
...
extern int primary_display_dsi_set_withcmdq(unsigned cmd, unsigned char count,
		unsigned char *para_list, unsigned char force_update);
extern int ddp_check_power_mode(void);
#endif /* __LCM_DRV_H__ */
