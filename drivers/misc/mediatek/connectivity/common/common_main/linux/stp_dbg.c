...
INT32 stp_dbg_set_host_assert_info(UINT32 drv_type, UINT32 reason, UINT32 en)
{
	...
	if (drv_type <= WMTDRV_TYPE_ANT)
		STP_DBG_WARN_FUNC("it is %s (type:%d) who triggers firmware assert with reason:%d\n",
				  taskStr[drv_type],
				  drv_type,
				  reason);
	else
		STP_DBG_WARN_FUNC("%s (type:%d) module triggers firmware assert with reason:%d\n",
				  "UNKNOWN",
				  drv_type,
				  reason);
