...
INT32 wmt_lib_init(VOID)
{
	...
	if (chip_type == WMT_CHIP_TYPE_COMBO) {
		...
		#if defined(CONFIG_MTK_PMIC_CHIP_MT6355)
		if (gDevWmt.rWmtGenConf.co_clock_flag) {
			/*Need clk buf driver to turn on/off 26M*/
			clk_buf_init_connsys_clkbuf(1);
		} else {
			/*Need connsys driver to turn on/off 26M for TCXO*/
			clk_buf_init_connsys_clkbuf(0);
		}
		#endif
	}
	if (iRet) {
...
UINT32 wmt_lib_bug_switch_flag_set(UINT32 en)
{
	return wmt_btm_bug_switch_ctrl(en);
}

INT32 wmt_lib_conf_bug_report_flag_get(VOID)
{
	P_WMT_GEN_CONF pWmtGenConf = NULL;

	wmt_conf_read_file();
	pWmtGenConf = wmt_conf_get_cfg();
	WMT_INFO_FUNC("bug_report_switch_flag: %d\n", pWmtGenConf->bug_report_switch_flag);
	return pWmtGenConf->bug_report_switch_flag;
}
