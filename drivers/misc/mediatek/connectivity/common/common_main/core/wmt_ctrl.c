...
OSAL_THREAD thread_btif_rxd;
void *pThread_btif_rxd;
OSAL_THREAD *thread_stp_btm;
OSAL_THREAD *thread_stp_psm;
...
INT32 wmt_ctrl_rx(P_WMT_CTRL_DATA pWmtCtrlData /*UINT8 *pBuff, UINT32 buffLen, UINT32 *readSize */)
{
	...
	while (readLen == 0) {	/* got nothing, wait for STP's signal */
		...
		if (waitRet == 0) {
			...
			if (pThread_btif_rxd) {
				thread_btif_rxd.pThread = (struct task_struct*)pThread_btif_rxd;
				osal_thread_show_stack(&thread_btif_rxd);
			}
			if (thread_stp_btm)
				osal_thread_show_stack(thread_stp_btm);
			if (thread_stp_psm)
				osal_thread_show_stack(thread_stp_psm);
