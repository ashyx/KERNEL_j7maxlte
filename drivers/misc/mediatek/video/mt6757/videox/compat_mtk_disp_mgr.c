...
static int compat_get_inset_session_buffer(struct compat_disp_session_buf_info __user *data32,
...
static int compat_get_disp_scenario_input_config(struct compat_disp_scenario_config_t __user *data32,
...
static int compat_get_disp_layer_info(struct _compat_disp_layer_info __user *data32,
					struct disp_layer_info __user *data)
{
	...
	for (iCnt = 0; iCnt < 2; iCnt++) {
		...
		if (p != 0) {
			DISPDBG("compat_get_disp_layer_info => Has Right Parameter\n");
			/*data->input_config[iCnt] = compat_ptr(p);*/
			err |= put_user(compat_ptr(p), &(data->input_config[iCnt]));
		}
...
static int compat_put_disp_layer_info(struct _compat_disp_layer_info __user *data32,
					struct disp_layer_info __user *data)
{
	compat_uint_t u;
	int err = 0;
...
int _compat_ioctl_set_scenario(struct file *file, unsigned long arg)
{
	...
	ret = file->f_op->unlocked_ioctl(file, DISP_IOCTL_SET_SCENARIO, (unsigned long)data);
	return ret;
}
