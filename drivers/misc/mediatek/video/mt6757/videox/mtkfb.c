...
#ifdef CONFIG_LCD_GENERIC
char *mtkfb_gen_panel_name = "mtk_gen_panel";
#endif
...
static ssize_t mtkfb_read(struct fb_info *info, char __user *buf,
				size_t count, loff_t *ppos)
{
	return 0;
}

static ssize_t mtkfb_write(struct fb_info *info, const char __user *buf,
				size_t count, loff_t *ppos)
{
	return 0;
}
...
static int mtkfb_blank(int blank_mode, struct fb_info *info)
{
	...
	case FB_BLANK_NORMAL:
		printk("mtkfb_blank mtkfb_late_resume\n");
		if (bypass_blank) {
			DISPERR("FB_BLANK_UNBLANK bypass_blank %d\n", bypass_blank);
			break;
		}

		pm_stay_awake(fbdev->dev);
		dev_warn(fbdev->dev, "pm_stay_awake");
	...
	case FB_BLANK_POWERDOWN:
		printk("mtkfb_blank mtkfb_early_suspend\n");
		if (bypass_blank) {
...
#endif
int mtkfb_set_lcm_func_call(void (*func4)(void *, void *, void *, void *),
		void *data1, void *data2, void *data3, void *data4)
{
	...
	DISPDBG("%s, primary_display_setlcm_func_call() - start\n", __func__);
	primary_display_setlcm_func_call(func4, data1, data2, data3, data4);
	DISPDBG("%s, primary_display_setlcm_func_call() - end\n", __func__);
	return 0;
}
EXPORT_SYMBOL(mtkfb_set_lcm_func_call);
...
#ifdef CONFIG_LCD_GENERIC
	DISPMSG("%s, %s\n", __func__, mtkfb_gen_panel_name);
	return mtkfb_gen_panel_name;
#endif
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	if (dal_en == TRUE) {
		if (disp_helper_get_stage() == DISP_HELPER_STAGE_NORMAL) {
			/* dal_init should after mtkfb_fbinfo_init,
			* otherwise layer 3 will show dal background color
			*/
			enum DAL_STATUS ret;
	...
	r = device_init_wakeup(fbdev->dev, true);
	if (r) {
		dev_err(fbdev->dev, "failed to init wakeup device\n");
		goto cleanup;
	}
