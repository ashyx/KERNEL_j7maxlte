...
/* This is added for primary power status of lcd driver and any other custom driver.*/
int ddp_check_power_mode(void)
{
	int ret;

	if (pgc->plcm->drv->aod)
	ret = primary_display_check_power_mode();
	else
		ret = primary_display_get_power_mode();
...
int primary_display_suspend(void)
{
	...
	if (primary_display_get_power_mode_nolock() == DOZE_SUSPEND) {
		if (primary_display_get_lcm_power_state_nolock() != LCM_ON_LOW_POWER) {
			/* SEC :There is no need to aod set to 1 when doze suspend.
			*if (pgc->plcm->drv->aod)
...
void clear_rdam_underflow(void)
{
	rdma_underflow_flag = FALSE;
	rdma_underflow_irq_cnt[0] = 0;
	rdma_underflow_irq_cnt[1] = 0;
}
...
int primary_display_resume(void)
{
	...
	/* We called AOD function here, before entering _primary_path_lock to avoid deadlock. */
	if (pgc->state == DISP_ALIVE) {
		DISPCHECK("primary display path is already resume, skip\n");
		skip_update = primary_display_lcm_power_on_state(1);
		/* W/A  : path lock, before goto done. */
		_primary_path_lock(__func__);
		goto done;
	}
	...
	if (!primary_display_is_video_mode()) {
		...
		if (((primary_display_get_power_mode_nolock() == FB_RESUME) ||
			...
			/* We wait frame done irq, So remove mdelay(16) */
		...
		unsigned int queue_size, unsigned char force_update)
{
	...
	if (primary_display_is_video_mode()) {
		...
		disp_dsi_set_withrawcmdq(plcm, cmdq_handle_backlight, pdata, queue_size, force_update);
...
int _dsi_set_withcmdq(struct disp_lcm_handle *plcm, unsigned cmd, unsigned char count,
		unsigned char *para_list, unsigned char force_update)
{
	...
	if (primary_display_is_video_mode()) {
		...
		disp_dsi_set_withcmdq(plcm, cmdq_handle_backlight, cmd, count, para_list, force_update);
		DISPMSG("video mode<===disp_dsi_set_withcmdq\n");
		_cmdq_flush_config_handle_mira(cmdq_handle_backlight, 0);
	} else {
		cmdqRecReset(cmdq_handle_backlight);
...
/*#endif*/
	if (pgc->state == DISP_SLEPT)
		...
		primary_display_idlemgr_kick(__func__, 0);
		if (primary_display_cmdq_enabled())
			_dsi_set_withrawcmdq(pgc->plcm, pdata, queue_size, force_update);
		atomic_set(&delayed_trigger_kick, 1);
	}
...
int primary_display_dsi_set_withcmdq(unsigned cmd, unsigned char count,
	unsigned char *para_list, unsigned char force_update)
{
	int ret = 0;
...
/*#ifndef CONFIG_MTK_AAL_SUPPORT*/
	...
	_primary_path_lock(__func__);
/*#endif*/
	DISPMSG("===primary_display_dsi_set_withcmdq===\n");
	if (pgc->state == DISP_SLEPT) {
		DISPERR("Sleep State set backlight invald\n");
	} else {
		primary_display_idlemgr_kick(__func__, 0);
		if (primary_display_cmdq_enabled()) {
		DISPMSG("===>_dsi_set_withcmdq cmd %x count %x force_update %x\n", cmd, count, force_update);
		_dsi_set_withcmdq(pgc->plcm, cmd, count, para_list, force_update);
...
int _dsi_read_cmdq_cmd(struct disp_lcm_handle *plcm, unsigned data_id, unsigned offset,
			unsigned cmd, unsigned char *buffer, unsigned char size)
{
	...
	if (!primary_display_is_video_mode()) {
		...
		DISPMSG("data_id %x cmd %x buffer %p size %d\n", data_id, cmd, buffer, size);
		ret = disp_lcm_util_set_read_cmdq_cmd_v2(plcm, cmdq_handle, data_id, offset, cmd, buffer, size);
		DISPMSG("CMD<=disp_lcm_util_set_prepare_cmdq_v2\n");
	}
	cmdqRecDestroy(cmdq_handle);
	cmdq_handle = NULL;
...
int primary_display_dsi_read_cmdq_cmd(unsigned data_id, unsigned offset, unsigned cmd,
			unsigned char *buffer, unsigned char size)
{
	...
	if (pgc->state == DISP_SLEPT) {
		...
		if (primary_display_cmdq_enabled()) {
			...
			ret = _dsi_read_cmdq_cmd(pgc->plcm, data_id, offset, cmd, buffer, size);
			DISPMSG("_dsi_read_cmdq_cmd-\n");
		}
		atomic_set(&delayed_trigger_kick, 1);
	}
