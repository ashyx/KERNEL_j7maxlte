...
int disp_dsi_set_withcmdq(struct disp_lcm_handle *plcm, void *cmdq,
		unsigned cmd, unsigned char count, unsigned char *para_list,
						unsigned char force_update)
{
	LCM_DRIVER *lcm_drv = NULL;

	DISPFUNC();
	DISPMSG("===disp_dsi_set_withcmdq===\n");
	if (_is_lcm_inited(plcm)) {
		lcm_drv = plcm->drv;
	...
	unsigned int *pdata, unsigned int queue_size, unsigned char force_update)
{
	...
	DISPFUNC();
	DISPMSG("===disp_dsi_set_withrawcmdq===\n");
	if (_is_lcm_inited(plcm)) {
		lcm_drv = plcm->drv;
					...
					unsigned offset, unsigned cmd, unsigned char *buffer, unsigned char size)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->read_by_cmdq) {
			ret = lcm_drv->read_by_cmdq(handle, data_id, offset, cmd, buffer, size);
		} else {
