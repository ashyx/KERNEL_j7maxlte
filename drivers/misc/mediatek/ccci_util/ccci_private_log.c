...
static void *cccimdee_reserve_memory_alloc(unsigned int zone, unsigned int id, unsigned int size);
static void ccci_dump_buffer_init(void)
{
	...
	for (i = 0; i < 2; i++) {
		...
		while (node_ptr->ctlb_ptr != NULL) {
			...
			if (buff_en_bit_map & (1<<i)) {
				/* using DT for alloc memory */
				ptr->buffer = cccimdee_reserve_memory_alloc(i, j, node_ptr->init_size);
				if (ptr->buffer == NULL) {
					/* allocate buffer with vmalloc*/
					ptr->buffer = vmalloc(node_ptr->init_size);
					pr_err("[ccci]alloc ee dump memory:zone %d,id:%d, size:0x%x\n",
...
static int cccimdee_reserve_memory_init(struct reserved_mem *rmem)
{
	...
	pr_alert("[memblock]%s: 0x%llx - 0x%llx (0x%llx), virt:0x%llx\n", "ccci-md-ee-dump",
...
static void *cccimdee_reserve_memory_alloc(unsigned int zone, unsigned int id, unsigned int size)
{
	...
	pr_err("[ccci]alloc ee dump DT reserve memory:zone %d,id:%d, size:0x%x, virt:0x%llx, phy:0x%llx\n",
		zone, id, size, (unsigned long long)virt, cccimdee_reserved_phy_addr);
