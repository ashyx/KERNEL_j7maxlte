...
static int ppm_dump_c0_min_freq_proc_show(struct seq_file *m, void *v)
{
	unsigned int idx;

	ppm_lock(&ppm_main_info.lock);
	if (ppm_main_info.last_req.cpu_limit[0].max_cpu_core) {
		idx = ppm_main_info.last_req.cpu_limit[0].min_cpufreq_idx;
		seq_printf(m, "%d\n",  ppm_main_info.cluster_info[0].dvfs_tbl[idx].frequency);
	} else
		seq_puts(m, "0\n");
	ppm_unlock(&ppm_main_info.lock);
...
static int ppm_dump_c0_max_freq_proc_show(struct seq_file *m, void *v)
{
	unsigned int idx;

	ppm_lock(&ppm_main_info.lock);
	if (ppm_main_info.last_req.cpu_limit[0].max_cpu_core) {
		idx = ppm_main_info.last_req.cpu_limit[0].max_cpufreq_idx;
		seq_printf(m, "%d\n",  ppm_main_info.cluster_info[0].dvfs_tbl[idx].frequency);
	} else
		seq_puts(m, "0\n");
	ppm_unlock(&ppm_main_info.lock);
...
static int ppm_dump_c1_min_freq_proc_show(struct seq_file *m, void *v)
{
	unsigned int idx;

	ppm_lock(&ppm_main_info.lock);
	if (ppm_main_info.last_req.cpu_limit[1].max_cpu_core) {
		idx = ppm_main_info.last_req.cpu_limit[1].min_cpufreq_idx;
		seq_printf(m, "%d\n",  ppm_main_info.cluster_info[1].dvfs_tbl[idx].frequency);
	} else
		seq_puts(m, "0\n");
	ppm_unlock(&ppm_main_info.lock);
...
static int ppm_dump_c1_max_freq_proc_show(struct seq_file *m, void *v)
{
	unsigned int idx;

	ppm_lock(&ppm_main_info.lock);
	if (ppm_main_info.last_req.cpu_limit[1].max_cpu_core) {
		idx = ppm_main_info.last_req.cpu_limit[1].max_cpufreq_idx;
		seq_printf(m, "%d\n",  ppm_main_info.cluster_info[1].dvfs_tbl[idx].frequency);
	} else
		seq_puts(m, "0\n");
	ppm_unlock(&ppm_main_info.lock);
...
static int ppm_dump_c0_min_core_proc_show(struct seq_file *m, void *v)
{
	ppm_lock(&ppm_main_info.lock);
	seq_printf(m, "%d\n", ppm_main_info.last_req.cpu_limit[0].min_cpu_core);
	ppm_unlock(&ppm_main_info.lock);

	return 0;
}

static int ppm_dump_c0_max_core_proc_show(struct seq_file *m, void *v)
{
	ppm_lock(&ppm_main_info.lock);
	seq_printf(m, "%d\n", ppm_main_info.last_req.cpu_limit[0].max_cpu_core);
	ppm_unlock(&ppm_main_info.lock);

	return 0;
}

static int ppm_dump_c1_min_core_proc_show(struct seq_file *m, void *v)
{
	ppm_lock(&ppm_main_info.lock);
	seq_printf(m, "%d\n", ppm_main_info.last_req.cpu_limit[1].min_cpu_core);
	ppm_unlock(&ppm_main_info.lock);

	return 0;
}

static int ppm_dump_c1_max_core_proc_show(struct seq_file *m, void *v)
{
	ppm_lock(&ppm_main_info.lock);
	seq_printf(m, "%d\n", ppm_main_info.last_req.cpu_limit[1].max_cpu_core);
	ppm_unlock(&ppm_main_info.lock);

	return 0;
}

static int ppm_dump_freq_limit_proc_show(struct seq_file *m, void *v)
{
	unsigned int i, min, max;

	ppm_lock(&ppm_main_info.lock);
	for_each_ppm_clusters(i) {
		min = ppm_main_info.last_req.cpu_limit[i].min_cpufreq_idx;
		max = ppm_main_info.last_req.cpu_limit[i].max_cpufreq_idx;

		if (ppm_main_info.last_req.cpu_limit[i].max_cpu_core) {
			seq_printf(m, "Cluster %d min/max freq = %d/%d\n", i,
					ppm_main_info.cluster_info[i].dvfs_tbl[min].frequency,
					ppm_main_info.cluster_info[i].dvfs_tbl[max].frequency);
		} else
			seq_printf(m, "Cluster %d min/max freq = %d/%d\n", i, 0, 0);
	}
	seq_puts(m, "\n");
...
static int ppm_dump_core_limit_proc_show(struct seq_file *m, void *v)
{
	unsigned int i, min, max;

	ppm_lock(&ppm_main_info.lock);
	for_each_ppm_clusters(i) {
		min = ppm_main_info.last_req.cpu_limit[i].min_cpu_core;
		max = ppm_main_info.last_req.cpu_limit[i].max_cpu_core;

		seq_printf(m, "Cluster %d min/max core = %d/%d\n", i, min, max);
	}
	seq_puts(m, "\n");
	ppm_unlock(&ppm_main_info.lock);
...
PROC_FOPS_RO(dump_c0_min_freq);
PROC_FOPS_RO(dump_c0_max_freq);
PROC_FOPS_RO(dump_c1_min_freq);
PROC_FOPS_RO(dump_c1_max_freq);
PROC_FOPS_RO(dump_c0_min_core);
PROC_FOPS_RO(dump_c0_max_core);
PROC_FOPS_RO(dump_c1_min_core);
PROC_FOPS_RO(dump_c1_max_core);
PROC_FOPS_RO(dump_freq_limit);
PROC_FOPS_RO(dump_core_limit);
#ifdef PPM_VPROC_5A_LIMIT_CHECK
...
#ifdef CONFIG_SCHED_HMP_BOOST 
static int ppm_hmp_boost_proc_show(struct seq_file *m, void *v)
{
	int i;
	...
	seq_printf(m, "0 NONE\n");
	seq_printf(m, "1 FULL ENABLE\n-1 FULL DISABLE\n");
	seq_printf(m, "2 SEMI ENABLE\n-2 SEMI DISABLE\n");
	for (i=1; i<=MAX_HMP_BOOST_STATE; ++i)
		seq_printf(m, "%d(%s)\t", sched_hmp_boost_on(i), info[i]);
	seq_printf(m, "\n");

	return 0;	
}

static ssize_t ppm_hmp_boost_proc_write(struct file *file, const char __user *buffer,
				size_t count, loff_t *pos)
{
	int type, ret=-EINVAL;
	char *buf = ppm_copy_from_user_for_proc(buffer, count);
	...
	if (!kstrtoint(buf, 10, &type))
		ret = sched_hmp_boost(type);

	free_page((unsigned long)buf);
	return ((ret < 0)?ret:count);
}

PROC_FOPS_RW(hmp_boost);
#endif
...
int ppm_procfs_init(void)
{
	...
	const struct pentry entries[] = {
		...
		PROC_ENTRY(dump_c0_min_freq),
		PROC_ENTRY(dump_c0_max_freq),
		PROC_ENTRY(dump_c1_min_freq),
		PROC_ENTRY(dump_c1_max_freq),
		PROC_ENTRY(dump_c0_min_core),
		PROC_ENTRY(dump_c0_max_core),
		PROC_ENTRY(dump_c1_min_core),
		PROC_ENTRY(dump_c1_max_core),
		PROC_ENTRY(dump_freq_limit),
		PROC_ENTRY(dump_core_limit),
#ifdef PPM_VPROC_5A_LIMIT_CHECK
...
#ifdef CONFIG_SCHED_HMP_BOOST
	struct pentry boost = PROC_ENTRY(hmp_boost);
...
#ifdef CONFIG_SCHED_HMP_BOOST
	sched_dir = proc_mkdir("sched", dir);
	if (!sched_dir) {
		ppm_err("@%s: fail to create /proc/ppm/sched dir\n", __func__);
		return -ENOMEM;
	}

	if (!proc_create(boost.name, S_IRUGO | S_IWUSR | S_IWGRP, sched_dir, boost.fops))
		ppm_err("%s(), create /proc/ppm/sched/hmp_boost failed\n", __func__);
#endif
