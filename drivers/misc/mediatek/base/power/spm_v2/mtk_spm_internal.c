...
#ifdef CONFIG_SEC_PM
	if(last_resume_kernel_reason_suspend) {
		last_resume_kernel_reason_len +=
			snprintf(last_resume_kernel_reason + last_resume_kernel_reason_len,
			MAX_WAKEUP_REASON_LEN - last_resume_kernel_reason_len, "[SPM]%s|", buf);
	}
#endif
