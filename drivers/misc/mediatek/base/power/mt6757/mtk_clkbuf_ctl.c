...
static CLK_BUF_SWCTRL_STATUS_T  clk_buf_swctrl[RF_CLKBUF_NUM] = {
	CLK_BUF_SW_ENABLE,
	CLK_BUF_SW_ENABLE,
	CLK_BUF_SW_DISABLE,
	CLK_BUF_SW_DISABLE
...
static CLK_BUF_SWCTRL_STATUS_T  clk_buf_swctrl_modem_on[RF_CLKBUF_NUM] = {
	CLK_BUF_SW_ENABLE,
	CLK_BUF_SW_ENABLE,
	CLK_BUF_SW_DISABLE,
	CLK_BUF_SW_ENABLE
...
static CLK_BUF_SWCTRL_STATUS_T  clk_buf_onoff_status[RF_CLKBUF_NUM] = {
	CLK_BUF_SW_ENABLE,
	CLK_BUF_SW_DISABLE,
	CLK_BUF_SW_DISABLE,
	CLK_BUF_SW_DISABLE
	...
	if (is_pmic_clkbuf) {
		...
		if (id == CLK_BUF_NFC) {
			if (CLK_BUF3_STATUS != CLOCK_BUFFER_HW_CONTROL) {
				/* external NFC, use mt6355 XO_WCN */
				clk_buf_onoff_status[id] = onoff;
				pmic_clk_buf_swctrl[PMIC_CLK_BUF_CONN] = onoff;
			} else
				return false;
		}
		...
		if (id == CLK_BUF_AUDIO) {
			if (CLK_BUF4_STATUS != CLOCK_BUFFER_HW_CONTROL) {
				/* external audio, use mt6355 XO_CEL */
				clk_buf_onoff_status[id] = onoff;
				pmic_clk_buf_swctrl[PMIC_CLK_BUF_RF] = onoff;
			} else
				return false;
		}
#if 0
		...
		if ((id == CLK_BUF_NFC) || (id == CLK_BUF_AUDIO)) {
			clk_buf_swctrl[CLK_BUF_AUDIO] =
				clk_buf_onoff_status[CLK_BUF_NFC] | clk_buf_onoff_status[CLK_BUF_AUDIO];
		} else
...
#else /* not support TC10SP_XO_FOR_NFC_AUD_SUPPORT */
		...
		mutex_unlock(&clk_buf_ctrl_lock);
#endif /* TC10SP_XO_FOR_NFC_AUD_SUPPORT */
...
{
#ifndef TC10SP_XO_FOR_NFC_AUD_SUPPORT
...
#else /* TC10SP_XO_FOR_NFC_AUD_SUPPORT */
	u32 id, onoff;

	if (sscanf(buf, "%x %x", &id, &onoff) != 2)
		return -EPERM;
				 ...
				 char *buf)
{
	...
	len += snprintf(buf+len, PAGE_SIZE-len,
			"CLKBUF API status WCN/NFC/AUD=%d/%d/%d\n",
			clk_buf_onoff_status[1], clk_buf_onoff_status[2], clk_buf_onoff_status[3]);
	...
	len += snprintf(buf+len, PAGE_SIZE-len,
		"PMIC clkbuf WCN[NFC]/NFC/CEL[AUD] mode=%x/%x/%x, status=%d/%d/%d\n",
