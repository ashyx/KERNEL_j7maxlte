...
static int ccmni_open(struct net_device *dev)
{
	...
	/* P***Z***: sync 761a25e1477fe08ca5fcb1399160caacbe4e44fe */
	ret = ccmni_ctl->ccci_ops->get_ccmni_ch(ccmni->md_id, ccmni->index, &(ccmni->ch));
	if (ret) {
		CCMNI_ERR_MSG(ccmni->md_id, "get ccmni%d channel fail\n", ccmni->index);
		return ret;
	}
	/*******************************************************/
...
static int ccmni_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case SIOCCCMNICFG:
		...
		if (!ctlb_irat) {
			CCMNI_ERR_MSG(md_id, "SIOCSCCMNICFG: %s iRAT(MD%d->MD%d) fail\n",
				dev->name, md_id, md_id_irat);
			break;
		}
