...
static int m4u_tee_notifier(struct notifier_block *nb, unsigned long action, void *data)
{
	struct m4u_blowfish_context *bf_ctx = container_of(nb, struct m4u_blowfish_context, notifier);

	complete(&bf_ctx->completion);
	return 0;
}

#define M4U_DRV_UUID {0x00000000, 0x4D54, 0x4B5F, {0x42, 0x46, 0x4D, 0x34, 0x55, 0x44, 0x54, 0x41} }
...
#define M4U_TL_UUID {0x00000000, 0x4D54, 0x4B5F, {0x42, 0x46, 0x00, 0x00, 0x00, 0x4D, 0x34, 0x55} }
...
static int m4u_send_tdrv_msg(struct m4u_sec_context *ctx, enum tdriver_cmd cmd)
{
	...
	ret = wait_for_completion_interruptible_timeout(&bf_ctx->completion, 5 * HZ);
	if (ret <= 0) {
		M4UERR("%s wait_for_completion_interruptible_timeout failed - %d\n", ctx->name, ret);
		goto err_out;
	}
...
static int m4u_bf_ctx_init(struct m4u_sec_context *ctx)
{
	...
	ret = tz_iwnotify_chain_register(bf_ctx->notify_event, &bf_ctx->notifier);
	if (ret < 0) {
	...
	if (bf_ctx->ctx_type == CTX_TYPE_TDRV) {
		...
		ret = tzdev_kapi_mem_grant(bf_ctx->client_id, bf_ctx->shmem_id);
		if (ret) {
			func_ret = ret;
...
out_iwnotify:
	ret = tz_iwnotify_chain_unregister(bf_ctx->notify_event, &bf_ctx->notifier);
	if (ret < 0)
...
int m4u_exec_cmd(struct m4u_sec_context *ctx)
{
	...
	ret = tzdev_kapi_send(bf_ctx->client_id, ctx->m4u_msg, sizeof(*ctx->m4u_msg));
	if (ret < 0) {
	...
	ret = tzdev_kapi_recv(bf_ctx->client_id, ctx->m4u_msg, sizeof(*ctx->m4u_msg));
	if (ret < 0) {
	...
	if (ctx->m4u_msg->bf_msg.notify_flags == 0) {
		m4u_aee_print("%s notify error:cmd=0x%x,ret=%d, maybe others uses event %d??!!\n",
			ctx->name, ctx->m4u_msg->cmd, ret, bf_ctx->notify_event);
	}
