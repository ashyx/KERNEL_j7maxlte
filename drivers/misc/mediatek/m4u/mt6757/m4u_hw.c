...
void m4u_print_port_status(struct seq_file *seq, int only_print_active)
{
	...
	for (port = 0; port < gM4u_port_num; port++) {
		...
		if (m4u_index == 0) {
			...
			sec = m4uHw_get_field_by_mask(larb_base, SMI_LARB_SEC_CONx(larb_port),
							F_SMI_SEC_EN(1));
			if (sec) {
				mmu_en = m4uHw_get_field_by_mask(larb_base, SMI_LARB_SEC_CONx(larb_port),
							F_SMI_SEC_MMU_EN(1));
			} else {
				mmu_en = m4uHw_get_field_by_mask(larb_base, SMI_LARB_NON_SEC_CONx(larb_port),
							F_SMI_NON_SEC_MMU_EN(1));
			}
		} else {
			larb_port = m4u_port_2_larb_port(port);
		...
		M4U_PRINT_LOG_OR_SEQ(seq, "%s(%d,%d),", m4u_get_port_name(port), !!mmu_en, !!sec);
	}
