...
static int button_press_debounce = 0x650;/* about 50ms */
int cur_key;
...
static unsigned int check_pole_type(void)
{
	...
	if ((vol < (cust_vol_set.vol_max_4pole + 1)) && (vol > (cust_vol_set.vol_min_4pole - 1))) {
		ACCDET_INFO("[accdet] pole check:%d mv, AB=%d\n", vol, TYPE_AB_01);
		return TYPE_AB_01;
...
#if NO_USE_COMPARATOR
	mdelay(10);/* derease the time delay */
#else
	mdelay(30);
#endif
#ifdef CONFIG_ACCDET_EINT_IRQ
...
int accdet_get_dts_data(void)
{
	...
	case HW_V_07:
		...
		break;
	default:
		ACCDET_ERROR("[accdet]err: hw_rev = %d\n", tmp_hw_rev);
		break;
	}
...
if (hw_rev == HW_V_01) {
   ...
   /* ACCDET Analog Setting */
	...
	if (accdet_dts_data.accdet_mic_mode == 1)	{/* ACC mode */
		...
		/* set 2.4v, and PU 2M */
		pmic_pwrap_write(ACCDET_ADC_REG, pmic_pwrap_read(ACCDET_ADC_REG)|(0x01<<11));
#endif
