...
static const struct of_device_id apiocfg_of_ids[] = {
...
struct device_node *get_iocfg_np(void)
{
	struct device_node *np_iocfg;

	gpio_vbase.iocfg_regs = NULL;

	np_iocfg = of_find_compatible_node(NULL, NULL, apiocfg_of_ids[0].compatible);
	if (np_iocfg == NULL) {
		GPIOERR("IOCFG device node is NULL\n");
		return NULL;
	}
	return np_iocfg;
}
