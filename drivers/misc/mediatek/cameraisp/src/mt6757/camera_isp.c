...
static MINT32 ISP_WaitIrq(ISP_WAIT_IRQ_STRUCT *WaitIrq)
{
	...
	if (Timeout == 0) {
		...
		if (WaitIrq->bDumpReg)
			ISP_DumpReg();
		//++ Titan, To check MIPI error

		mipiCheckError = 0;

		if(WaitIrq->Type == ISP_IRQ_TYPE_INT_CAM_A_ST)
		{
		    mipiCheckError = ISP_RD32(ISP_SENINF0_BASE + 0x03b4);
		}
		else if(WaitIrq->Type == ISP_IRQ_TYPE_INT_CAM_B_ST)
		{
		    mipiCheckError = ISP_RD32(ISP_SENINF0_BASE + 0x07b4);
		}

		if(mipiCheckError != 0x9004)
		{// Error case
			LOG_ERR("Status check (%x) ISP_SENINF0_BASE + 0x03b4(%x) 0x07b4(%d), mipiCheck(%x)\n",WaitIrq->Type,
			ISP_RD32(ISP_SENINF0_BASE + 0x03b4), ISP_RD32(ISP_SENINF0_BASE + 0x07b4), mipiCheckError);
#ifdef CONFIG_CAMERA_HW_BIG_DATA
	...
	struct file *pFile)
{
	...
	if (IspInfo.UserCount > 0) {
		...
		spin_unlock(&(IspInfo.SpinLockIspRef));
		/*Titan++*/
		LOG_DBG("cpuhvfs_hint_mmc_event:1+");
		cpuhvfs_hint_mmc_event(1);
		LOG_DBG("cpuhvfs_hint_mmc_event:1-");
		/*Titan--*/
	...
	struct file *pFile)
{
	...
	if (IspInfo.UserCount > 0) {
		...
		spin_unlock(&(IspInfo.SpinLockIspRef));
		/*Titan++*/
		LOG_DBG("cpuhvfs_hint_mmc_event:0+");
		cpuhvfs_hint_mmc_event(0);
		LOG_DBG("cpuhvfs_hint_mmc_event:0-");
		/*Titan++*/
	}
