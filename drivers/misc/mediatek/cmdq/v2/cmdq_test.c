...
void testcase_allocate_path_resource(void)
{
#ifdef CMDQ_SECURE_PATH_SUPPORT
	CMDQ_MSG("%s\n", __func__);

	cmdq_sec_allocate_path_resource_unlocked(false);

	CMDQ_MSG("%s END\n", __func__);
#endif
}

static void testcase_loop_simple_secure(void)
{
#ifdef CMDQ_SECURE_PATH_SUPPORT
	CMDQ_MSG("%s\n", __func__);
	_testcase_simplest_command_loop_submit(1000, CMDQ_SCENARIO_DEBUG,
		(0x1 << CMDQ_ENG_MDP_RSZ0), gCmdqTestSecure);
#endif
	CMDQ_MSG("%s END\n", __func__);
}

static void testcase_loop_write_secure(void)
{
#ifdef CMDQ_SECURE_PATH_SUPPORT
	struct cmdqRecStruct *handle = NULL;
	const uint32_t PATTERN = (1 << 0) | (1 << 2) | (1 << 16);
	unsigned long dummy_va = CMDQ_TEST_MMSYS_DUMMY_VA, dummy_pa = CMDQ_TEST_MMSYS_DUMMY_PA;
	int32_t i, status;
	int32_t loop = gCmdqTestConfig[2];
	uint32_t value = 0;
	...
	for (i = 0; i < loop; i++) {
		/* set to 0xFFFFFFFF */
		CMDQ_REG_SET32((void *)dummy_va, ~0);
		CMDQ_LOG("pid: %d flush:%4d\n", current->pid, i);
		status = cmdq_task_flush(handle);
		if (status < 0) {
			/* flush fail */
			CMDQ_ERR("TEST FAIL: fail to flush task status:%d\n", status);
			break;
		}
		/* value check */
		value = CMDQ_REG_GET32((void *)dummy_va);
...
void testcase_loop_sec_hello(void)
{
#ifdef CMDQ_SECURE_PATH_SUPPORT
	int32_t status, i;
	int32_t loop = gCmdqTestConfig[2];
	...
	for (i = 0; i < loop; i++) {
		status = cmdq_sec_submit_to_secure_world_async_unlocked(CMD_CMDQ_TL_TEST_HELLO_TL, NULL,
			CMDQ_INVALID_THREAD, NULL, NULL);
		if (status < 0) {
			/* entry cmdqSecTL failed */
			CMDQ_ERR("entry cmdqSecTL failed, status:%d\n", status);
...
static void testcase_general_handling(int32_t testID)
{
	...
	switch (testID) {
	case 145:
		testcase_loop_sec_hello();
		break;
	case 144:
		testcase_loop_write_secure();
		break;
	case 143:
		testcase_loop_simple_secure();
		break;
	case 142:
		testcase_allocate_path_resource();
		break;
	case 141:
