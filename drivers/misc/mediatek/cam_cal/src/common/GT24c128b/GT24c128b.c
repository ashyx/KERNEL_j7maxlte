...
/*#define GT24c128b_DRIVER_ON 0*/
#ifdef GT24c128b_DRIVER_ON
...
static int iWriteEEPROM(u16 a_u2Addr, u32 a_u4Bytes, u8 *puDataInBytes)
{
	...
	if (a_u4Bytes + 2 > 8) {
		EEPROMDB("[GT24c128b] exceed I2c-mt65xx.c 8 bytes limitation (include address 2 Byte)\n");
		return -1;
	}
	...
	if (i4RetValue != (a_u4Bytes + 2)) {
		EEPROMDB("[GT24c128b] I2C write  failed!!\n");
		return -1;
	}
...
static int iReadEEPROM(u16 a_u2Addr, u32 ui4_length, u8 *a_puBuff)
{
	...
	if (ui4_length > 8) {
		EEPROMDB("[GT24c128b] exceed I2c-mt65xx.c 8 bytes limitation\n");
		return -1;
	}
	...
	if (i4RetValue != 2) {
		EEPROMDB("[GT24c128b] I2C read data failed!!\n");
		return -1;
	}
	#ifdef CONFIG_MTK_I2C_EXTENSION
	spin_lock(&g_EEPROMLock); /* for SMP */
	...
	spin_unlock(&g_EEPROMLock); /* for SMP */
#endif
	/* EEPROMDB("[GT24c128b] iReadEEPROM done!!\n"); */
	return 0;
}
...
static int iReadData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char *pinputdata)
{
	...
	if (ui4_offset + ui4_length >= 0x2000) {
		EEPROMDB("[GT24c128b] Read Error!! GT24c128b not supprt address >= 0x2000!!\n");
		return -1;
	}
	...
	do {
		if (i4ResidueDataLength >= 8) {
			...
			if (i4RetValue != 0) {
				EEPROMDB("[GT24c128b] I2C iReadData failed!!\n");
				return -1;
			}
			u4IncOffset += 8;
...
unsigned int gt24c128b_selective_read_region(struct i2c_client *client, unsigned int addr,
	unsigned char *data, unsigned int size)
{
	EEPROMDB("[GT24c128b] Start Read\n");
	g_pstI2Cclient = client;
	if (iReadData(addr, size, data) == 0) {
		EEPROMDB("[GT24c128b] Read success\n");
		return size;
	}
	EEPROMDB("[GT24c128b] Read failed\n");
	return 0;
}
...
#endif
{
	...
	if (_IOC_DIR(a_u4Command) != _IOC_NONE) {
		...
		if (pBuff == NULL) {
			EEPROMDB("[GT24c128b] ioctl allocate mem failed\n");
			return -ENOMEM;
		}
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user((u8 *) pBuff, (u8 *) a_u4Param, sizeof(stCAM_CAL_INFO_STRUCT))) {
				...
				kfree(pBuff);
				EEPROMDB("[GT24c128b] ioctl copy from user failed\n");
				return -EFAULT;
			}
		}
	}
	...
	EEPROMDB("[GT24c128b] init Working buffer address 0x%8x  command is 0x%8x\n", (u32)pWorkingBuff,
	(u32)a_u4Command);
	...
	case CAM_CALIOC_G_READ:
		EEPROMDB("[GT24c128b] Read CMD\n");
#ifdef EEPROMGETDLT_DEBUG
		do_gettimeofday(&ktv1);
#endif
		EEPROMDB("[GT24c128b] offset %x\n", ptempbuf->u4Offset);
		EEPROMDB("[GT24c128b] length %x\n", ptempbuf->u4Length);
		EEPROMDB("[GT24c128b] Before read Working buffer address 0x%8x\n", (u32)pWorkingBuff);
		...
		if (ptempbuf->u4Offset == 0x0024C32a) {
			*(u32 *)pWorkingBuff = 0x0124C32a;
			...
			while (readTryagain > 0) {
				i4RetValue =  iReadDataFromGT24c128b((u16)ptempbuf->u4Offset, ptempbuf->u4Length,
				pWorkingBuff);
				EEPROMDB("[GT24c128b] error (%d) Read retry (%d)\n", i4RetValue, readTryagain);
				if (i4RetValue != 0)
					readTryagain--;
				else
					readTryagain = 0;
		...
		EEPROMDB("[GT24c128b] After read Working buffer data  0x%4x\n", *pWorkingBuff);
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
		...
		EEPROMDB("[GT24c128b] to user length %d\n", ptempbuf->u4Length);
		EEPROMDB("[GT24c128b] to user  Working buffer address 0x%8x\n", (u32)pWorkingBuff);
		if (copy_to_user((u8 __user *) ptempbuf->pu1Params, (u8 *)pWorkingBuff,
		ptempbuf->u4Length)) {
			kfree(pBuff);
			kfree(pWorkingBuff);
			EEPROMDB("[GT24c128b] ioctl copy to user failed\n");
			return -EFAULT;
		}
	}
...
static int EEPROM_Open(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	EEPROMDB("[GT24c128b] EEPROM_Open\n");
...
static inline int RegisterEEPROMCharDrv(void)
{
	...
	if (alloc_chrdev_region(&g_EEPROMdevno, 0, 1, EEPROM_DRVNAME)) {
		EEPROMDB("[GT24c128b] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(g_EEPROMdevno, 1, EEPROM_DRVNAME)) {
		EEPROMDB("[GT24c128b] Register device no failed\n");
	...
	if (g_pEEPROM_CharDrv == NULL) {
		...
		EEPROMDB("[GT24c128b] Allocate mem for kobject failed\n");
	...
	if (cdev_add(g_pEEPROM_CharDrv, g_EEPROMdevno, 1)) {
		EEPROMDB("[GT24c128b] Attatch file operation failed\n");
...
static int EEPROM_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	EEPROMDB("[GT24c128b] g_pstI2Cclient->addr = 0x%8x\n", g_pstI2Cclient->addr);
	/* Register char driver */
	i4RetValue = RegisterEEPROMCharDrv();

	if (i4RetValue) {
		EEPROMDB("[GT24c128b] register char device failed!\n");
		return i4RetValue;
	}


	EEPROMDB("[GT24c128b] Attached!!\n");
	return 0;
}
...
static int __init EEPROM_i2C_init(void)
{
	...
	if (platform_driver_register(&g_stEEPROM_Driver)) {
		EEPROMDB("failed to register GT24c128b driver\n");
		return -ENODEV;
	}

	if (platform_device_register(&g_stEEPROM_Device)) {
		EEPROMDB("failed to register GT24c128b driver, 2nd time\n");
		return -ENODEV;
	}
