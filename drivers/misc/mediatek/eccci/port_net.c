...
/* P***Z*** sync: 761a25e1477fe08ca5fcb1399160caacbe4e44fe */
...
/* P***Z*** sync: 761a25e1477fe08ca5fcb1399160caacbe4e44fe */
static inline struct ccci_port *ccmni_get_port_by_id(int md_id, int ccmni_idx)
{
	int i = 0;
	struct ccci_port *port = NULL;
	struct port_proxy *ccci_port_proxy = NULL;

	ccci_port_proxy = ccci_md_get_port_proxy_by_id(md_id);

	while (i < ccci_port_proxy->port_number) {
		port = ccci_port_proxy->ports + i;
		if (port->minor == (ccmni_idx + CCCI_NET_MINOR_BASE))
			return port;
		i++;
	}
	return NULL;
...
/* P***Z*** sync: 761a25e1477fe08ca5fcb1399160caacbe4e44fe */
int ccci_get_default_channel(int md_id, int ccmni_idx, struct ccmni_ch *channel)
{
	int ret = 0;
	...
	case 2:
		...
		/* channel->tx_ack = 0xFF; */
		channel->dl_ack = CCCI_CCMNI3_DL_ACK;
		channel->multiq = md_id == MD_SYS1 ? 1 : 0;
		break;
	case 3:
		...
		/* channel->tx_ack = 0xFF; */
		channel->dl_ack = CCCI_CCMNI4_DLACK_RX;
		channel->multiq = md_id == MD_SYS1 ? 1 : 0;
		break;
	case 4:
		...
		/* channel->tx_ack = 0xFF; */
		channel->dl_ack = CCCI_CCMNI5_DLACK_RX;
		channel->multiq = md_id == MD_SYS1 ? 1 : 0;
		break;
	case 5:
		...
		/* channel->tx_ack = 0xFF; */
		channel->dl_ack = CCCI_CCMNI6_DLACK_RX;
		channel->multiq = md_id == MD_SYS1 ? 1 : 0;
		break;
	case 6:
		...
		/* channel->tx_ack = 0xFF; */
		channel->dl_ack = CCCI_CCMNI7_DLACK_RX;
		channel->multiq = md_id == MD_SYS1 ? 1 : 0;
		break;
	case 7:
	...
	default:
		...
		/* P***Z*** */
		channel->rx = CCCI_MAX_CH_NUM;
		channel->tx = CCCI_MAX_CH_NUM;
		channel->dl_ack = CCCI_MAX_CH_NUM;
		ret = -1;
		break;
	}
...
int ccci_get_ccmni_channel(int md_id, int ccmni_idx, struct ccmni_ch *channel)
{
	...
	if (md == NULL) {
		CCCI_ERR_MSG(md_id, NET, "fail to get modem by id %d, init ch as default value !!\n", md_id);
		ret = ccci_get_default_channel(md_id, ccmni_idx, channel);
		return ret;
	}

	ccci_port_proxy = ccci_md_get_port_proxy_by_id(md_id);

	if (ccmni_idx >= ccci_port_proxy->port_number) {
		CCCI_ERR_MSG(md_id, NET, "invalid ccmni_idx %d for md_id %d !!\n", ccmni_idx, md_id);
		return -1;
	}

	port = ccmni_get_port_by_id(md_id, ccmni_idx);
	if (port) {
		channel->rx = port->rx_ch;
		channel->rx_ack = 0xFF;
		channel->tx = port->tx_ch;
		channel->tx_ack = 0xFF;
		channel->dl_ack = port->dl_ack_ch;
		channel->multiq = md_id == MD_SYS1 ? 1 : 0;	/*it is not used for titan*/
		CCCI_ERR_MSG(md_id, NET, "ccmni_idx:%d, channel->tx:%d,port->tx_ch:%d, channel->dl_ack:%d, port->dl_ack_ch:%d\n",
			ccmni_idx, channel->tx, port->tx_ch, channel->dl_ack, port->dl_ack_ch);
	} else
		CCCI_ERR_MSG(md_id, NET, "ignore ccmni_idx %d !!\n", ccmni_idx);
...
int ccmni_send_pkt(int md_id, int ccmni_idx, void *data, int is_ack)
{
	...
	CCCI_DEBUG_LOG(md_id, NET, "port %s,txq_exp_index:%d, txq_index:%d send: %08X, %08X, %08X, %08X\n", port->name, port->txq_exp_index, port->txq_index,
...
/* P***Z***: sync: 761a25e1477fe08ca5fcb1399160caacbe4e44fe */
static inline void init_dl_ack_ch(struct ccci_port *port)
{
	int index = GET_CCMNI_IDX(port);

	switch (index) {
	case 0:
		port->dl_ack_ch = CCCI_CCMNI1_DL_ACK;
		break;
	case 1:
		port->dl_ack_ch = CCCI_CCMNI2_DL_ACK;
		break;
	case 2:
		port->dl_ack_ch = CCCI_CCMNI3_DL_ACK;
		break;
	case 3:
		port->dl_ack_ch = CCCI_CCMNI4_DLACK_RX;
		break;
	case 4:
		port->dl_ack_ch = CCCI_CCMNI5_DLACK_RX;
		break;
	case 5:
		port->dl_ack_ch = CCCI_CCMNI6_DLACK_RX;
		break;
	case 6:
		port->dl_ack_ch = CCCI_CCMNI7_DLACK_RX;
		break;
	case 7:
		port->dl_ack_ch = CCCI_CCMNI8_DLACK_RX;
		break;
#ifdef FEATURE_DIRECT_TETHERING_LOGGING
	case 8:
		port->dl_ack_ch = CCCI_CCMNILAN_DLACK_RX;
		break;
#endif
	case 9:
		port->dl_ack_ch = CCCI_CCMNI10_DLACK_RX;
		break;
	case 10:
		port->dl_ack_ch = CCCI_CCMNI11_DLACK_RX;
		break;
	case 11:
		port->dl_ack_ch = CCCI_CCMNI12_DLACK_RX;
		break;
	case 12:
		port->dl_ack_ch = CCCI_CCMNI13_DLACK_RX;
		break;
	case 13:
		port->dl_ack_ch = CCCI_CCMNI14_DLACK_RX;
		break;
	case 14:
		port->dl_ack_ch = CCCI_CCMNI15_DLACK_RX;
		break;
	case 15:
		port->dl_ack_ch = CCCI_CCMNI16_DLACK_RX;
		break;
	case 16:
		port->dl_ack_ch = CCCI_CCMNI17_DLACK_RX;
		break;
	case 17:
		port->dl_ack_ch = CCCI_CCMNI18_DLACK_RX;
		break;
	default:
		CCCI_ERROR_LOG(port->md_id, NET, "invalid ccmni index=0x%x\n", port->minor);
		break;
	}
}
