...
static struct c2k_port c2k_ports[] = {
	...
	{AT9_CH_C2K, AT9_CH_C2K, CCCI_C2K_AT9, CCCI_C2K_AT9,},	/*for meta data */
...
static int md_ccif_ring_buf_init(struct ccci_modem *md)
{
	...
	unsigned int ring_buf_limit_sz = md->mem_layout.smem_region_size -
		md->smem_layout.ccci_smart_logging_size - CCCI3_SMEM_OFFSET_CCIF_SMEM;
#else
	unsigned int ring_buf_limit_sz = md->mem_layout.smem_region_size - CCCI_SMEM_OFFSET_CCIF_SMEM;
#endif
	...
	for (i = 0; i < QUEUE_NUM; i++) {
		...
		if (md_ctrl->total_smem_size + bufsize > ring_buf_limit_sz) {
			CCCI_ERROR_LOG(md->index, TAG,
