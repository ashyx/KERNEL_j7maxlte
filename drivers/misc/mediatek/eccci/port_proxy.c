...
/* 0325 - start */
extern int mtk_wcn_wmt_func_off(int);
/* 0325 - end */
#endif
...
void drv_tri_panic_by_lvl(int md_id)
{
...
#ifdef CONFIG_MTK_SRIL_SUPPORT
	mdee = ctl->mdee_obj;
	crash_reason = mdee->crash_reason;
#endif
	...
	if (ccci_md_log_level == MD_LOG_LEVEL_LOW) {
		...
		/*
		* MD1 EE: wait until MD notify dump done info, timeout is 30s
		* MD3 EE: always wait 30s, because MD3 will must trigger EE when MD1 trigger
		* EE and MD3 dump time is small
		*/
		while (retry-- > 0) {
			if (ctl->ops->is_dump_done != NULL) {
...
#ifdef CONFIG_MTK_SRIL_SUPPORT
		if (ccci_h->channel == CCCI_RIL_IPC0_RX || ccci_h->channel == CCCI_RIL_IPC1_RX) {
			if ((unsigned int)(sched_clock() / 1000000) - ccci_h->reserved > 100)
				print_hex_dump(KERN_INFO, "(delayed) mif: RX: ",
...
int port_proxy_user_register(struct port_proxy *proxy_p, struct ccci_port *port)
{
	...
	if (rx_ch == CCCI_MD_LOG_RX)
		port_proxy_set_critical_user(proxy_p, CRIT_USR_MDLOG, 0); /* ttyC1 */
	if (rx_ch == CCCI_UART1_RX)
		port_proxy_set_critical_user(proxy_p, CRIT_USR_META, 0); /* MD logger control */
	return 0;
}
...
int port_proxy_send_msg_to_user(struct port_proxy *proxy_p, CCCI_CH ch, CCCI_MD_MSG msg, u32 resv)
{
	...
	if (skb == NULL) {
		CCCI_ERROR_LOG(proxy_p->md_id, TAG, "fail to allocate skb buff!!\n");
		return -CCCI_ERR_MEM_CHECK_FAIL;
	}
...
long port_proxy_user_ioctl(struct port_proxy *proxy_p, int ch, unsigned int cmd, unsigned long arg)
{
	...
	case CCCI_IOC_GET_OTHER_MD_STATE:
		...
		break;
	case CCCI_IOC_GET_MD_STATE_SYNC:
		CCCI_NOTICE_LOG(md_id, CHAR, "Get md state sync ioctl(%d) called by %s\n", ch, current->comm);
		other_md_state_for_user = exec_ccci_kern_func_by_md_id(MD_SYS3, ID_GET_MD_STATE, NULL, 0);
		state_for_user = exec_ccci_kern_func_by_md_id(MD_SYS1, ID_GET_MD_STATE, NULL, 0);
		if (get_modem_is_enabled(MD_SYS3)) {
			/* md1 invalid ---- */
			if ((state_for_user == MD_STATE_INVALID)
					&& (other_md_state_for_user == MD_STATE_INVALID))
				state_for_user = MD_STATE_INVALID;
			else if ((state_for_user == MD_STATE_INVALID)
					&& (other_md_state_for_user == MD_STATE_BOOTING))
				state_for_user = MD_STATE_INVALID;
			else if ((state_for_user == MD_STATE_INVALID)
					&& (other_md_state_for_user == MD_STATE_READY))
				state_for_user = MD_STATE_INVALID;
			else if ((state_for_user == MD_STATE_INVALID)
					&& (other_md_state_for_user == MD_STATE_EXCEPTION))
				state_for_user = MD_STATE_EXCEPTION;
			/* md1 booting ---- */
			else if ((state_for_user == MD_STATE_BOOTING)
					&& (other_md_state_for_user == MD_STATE_INVALID))
				state_for_user = MD_STATE_INVALID;
			else if ((state_for_user == MD_STATE_BOOTING)
					&& (other_md_state_for_user == MD_STATE_BOOTING))
				state_for_user = MD_STATE_BOOTING;
			else if ((state_for_user == MD_STATE_BOOTING)
					&& (other_md_state_for_user == MD_STATE_READY))
				state_for_user = MD_STATE_BOOTING;
			else if ((state_for_user == MD_STATE_BOOTING)
				...
				state_for_user = MD_STATE_EXCEPTION;
			/* md1 ready ---- */
			else if ((state_for_user == MD_STATE_READY)
					&& (other_md_state_for_user == MD_STATE_INVALID))
				state_for_user = MD_STATE_INVALID;
			else if ((state_for_user == MD_STATE_READY)
					&& (other_md_state_for_user == MD_STATE_BOOTING))
				state_for_user = MD_STATE_BOOTING;
					...
					&& (other_md_state_for_user == MD_STATE_READY))
				state_for_user = MD_STATE_READY;
			else if ((state_for_user == MD_STATE_READY)
					&& (other_md_state_for_user == MD_STATE_EXCEPTION))
				state_for_user = MD_STATE_EXCEPTION;
			/* md1 exception ---- */
			else if ((state_for_user == MD_STATE_EXCEPTION)
					&& (other_md_state_for_user == MD_STATE_INVALID))
				state_for_user = MD_STATE_EXCEPTION;
			else if ((state_for_user == MD_STATE_EXCEPTION)
					&& (other_md_state_for_user == MD_STATE_BOOTING))
		...
		if (state_for_user >= 0) {
			...
			CCCI_ERROR_LOG(md_id, CHAR, "Get MD state sync fail: %ld\n", state_for_user);
			ret = state_for_user;
		}
		break;
	...
	case CCCI_IOC_FORCE_MD_OFF:
		{
			int i;
		((struct ccci_modem *)(proxy_p->md_obj))->force_off_md = 1;
		ccif_disable_irq((struct ccci_modem *)proxy_p->md_obj);
		wdt_disable_irq((struct ccci_modem *)proxy_p->md_obj);

#ifdef CONFIG_SEC_FACTORY
		/* 0325 - start */
		mtk_wcn_wmt_func_off(0);
		mtk_wcn_wmt_func_off(1);
		mtk_wcn_wmt_func_off(2);
		mtk_wcn_wmt_func_off(3);
		mtk_wcn_wmt_func_off(9);
		/* 0325 - end */
#endif

		CCCI_INF_MSG(md_id, CHAR, "Force power off modem ioctl called by %s\n", current->comm);
			if (md_id == MD_SYS1) {
				exec_ccci_kern_func_by_md_id(MD_SYS1, FORCE_MD_OFF_MSG, NULL, 0);
				for (i = 0; i < 10; i++) {
					if (((struct ccci_modem *)(proxy_p->md_obj))->md_power_off_done)
						break;
					msleep(100);
				}
#ifdef CONFIG_MTK_ECCCI_C2K
