...
READ_START:
	...
	if (skb == NULL) {
		CCCI_ERROR_LOG(port->md_id, CHAR, "fail to peek skb buff!!\n");
		return -EFAULT;
	}
 ...
 exit:
	if (ret < 0 && (port->rx_ch == CCCI_RIL_IPC0_RX || port->rx_ch == CCCI_RIL_IPC1_RX))
		CCCI_ERROR_LOG(port->md_id, CHAR, "RILD failed to read ipc packet, ret = %d, rx_ch = %d\n", ret, port->rx_ch);
...
static int port_char_init(struct ccci_port *port)
{
	...
	if (dev == NULL) {
		CCCI_ERROR_LOG(md_id, CHAR, "fail to allocate cdev structure!!\n");
		return -EFAULT;
	}
...
#ifdef CONFIG_MTK_SRIL_SUPPORT
	if ((port->rx_ch == CCCI_RIL_IPC0_RX || port->rx_ch == CCCI_RIL_IPC1_RX) &&
		(state == GATED || state == BOOT_WAITING_FOR_HS2 || state == READY || state == RESET || state == EXCEPTION)) {
