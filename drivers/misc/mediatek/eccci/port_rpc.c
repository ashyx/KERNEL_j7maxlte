...
/* P***Z*** sync: 761a25e1477fe08ca5fcb1399160caacbe4e44fe */
static int ccci_rpc_remap_ch_queue(int md_id, struct ccci_modem *md, struct ccci_rpc_queue_mapping *map_tbl)
{
	unsigned int i, j = 0;
	...
	if (map_tbl->ch_cnt > MAX_CHANNEL_QUEUE_MAPPING) {
		CCCI_ERROR_LOG(md_id, RPC, "map table: invalid channel count %u\n", map_tbl->ch_cnt);
		return -1;
	}

	for (i = 0; i < map_tbl->ch_cnt; i++) {
		CCCI_INF_MSG(md_id, RPC, "rpc data: ch %u to q%u\n",
			map_tbl->ch_queue_mapping[i].channel, map_tbl->ch_queue_mapping[i].queue_type);
		if (map_tbl->ch_queue_mapping[i].channel >= CCCI_MAX_CH_NUM ||
			map_tbl->ch_queue_mapping[i].queue_type >= MAX_TXQ_NUM) {
			CCCI_ERROR_LOG(md_id, RPC, "map table: item %d is invalid: ch %u to q%u\n", i,
				map_tbl->ch_queue_mapping[i].channel, map_tbl->ch_queue_mapping[i].queue_type);
			return -1;
		}
		for (j = 0; j < ccci_port_proxy->port_number; j++) {
			port = ccci_port_proxy->ports + j;
			if (port->tx_ch == map_tbl->ch_queue_mapping[i].channel) {
				CCCI_INF_MSG(md_id, RPC, "remap ch %d from q%u to q%u\n", port->tx_ch,
					port->txq_index,  map_tbl->ch_queue_mapping[i].queue_type);
				port->txq_index = map_tbl->ch_queue_mapping[i].queue_type;
				break;
			}
			/*DL ACK queue is reused with exception queue index (q_num | 0xF0)*/
			if (port->dl_ack_ch == map_tbl->ch_queue_mapping[i].channel) {
				CCCI_INF_MSG(md_id, RPC, "remap ch %d from q 0x%x to 0x%x\n", port->dl_ack_ch,
					port->txq_exp_index,  (map_tbl->ch_queue_mapping[i].queue_type | 0xF0));
				port->txq_exp_index = map_tbl->ch_queue_mapping[i].queue_type | 0xF0;
				break;
			}
		}
	...
	for (j = 0; j < ccci_port_proxy->port_number; j++) {
		port = ccci_port_proxy->ports + j;
		CCCI_DEBUG_LOG(md_id, RPC, "tx[%u],ack[%u]:[%u],[%u]\n", port->tx_ch, port->dl_ack_ch,
			port->txq_index,  port->txq_exp_index & 0xF);
	}
	...
	case IPC_RPC_GET_RF_CLK_BUF_OP:
		{
			...
			if (count != CLKBUF_MAX_COUNT) {
				...
				CCCI_INF_MSG(md_id, RPC, "RF_CLK_BUF*Status %d, %d, %d, %d\n",
					clkbuf->CLKBuf_Status[0], clkbuf->CLKBuf_Status[1],
					clkbuf->CLKBuf_Status[2], clkbuf->CLKBuf_Status[3]);
...
#endif
    	/* P***Z*** sync: 761a25e1477fe08ca5fcb1399160caacbe4e44fe */
	case IPC_RPC_CCCI_QUEUE_MAPPING:
		{
			struct ccci_rpc_queue_mapping *map_tbl;
			...
			CCCI_INF_MSG(md_id, RPC, "op_id[0x%X]: pkt_num=%d, pkt[0] len %u!\n",
				     p_rpc_buf->op_id, pkt_num, pkt[0].len);

			map_tbl = (struct ccci_rpc_queue_mapping *)(pkt[0].buf);
			ccci_rpc_remap_ch_queue(md_id, port->modem, map_tbl);
			pkt_num = 0;
			tmp_data[0] = 0;
			pkt[pkt_num].len = sizeof(unsigned int);
			pkt[pkt_num++].buf = (void *)&tmp_data[0];
