/* tui/stui_hal_touch.c
 *
 * TUI HW Handler driver. Touchscreen functions.
 *
...
static int do_pm_notify(struct notifier_block *b, unsigned long ev, void *p)
{
	(void)b;
	(void)p;
	pr_err("[STUI] Device power event occurred. ev=%lu\n", ev);
...
static struct i2c_adapter *get_i2c_adapter(void *touch_int_data)
{
	...
	if (!i2c_adapt)
		pr_err("[STUI] I2C adapter is NULL.\n");
...
static int __maybe_unused request_touch(void)
{
	...
	if (!dev) {
		pr_err("[STUI] Touch device %s not found.\n", i2c_dev_name);
		return -ENXIO;
	}
	touch_int_data = dev_get_drvdata(dev);
	if (!touch_int_data) {
		pr_err("[STUI] Touchscreen driver internal data is empty.\n");
		return -EFAULT;
	}
	...
	if (ret < 0) {
		pr_err("[STUI] pm_runtime_get parent error: %d\n", ret);
	...
	if (retval < 0) {
		pr_err("[STUI] register_pm_notifier: retval=%d\n", retval);
		goto exit_err2;
	}
...
static int __maybe_unused release_touch(void)
{
	...
	if (local_irq_enabled) {
		...
		touch_int_data->irq_enabled = true;
		msleep(10);
		printk(KERN_DEBUG "[STUI] touch irq enabled\n");
	}
	...
	*/
	...
	if (pm_get_parent_ok) {
		...
		if (retval < 0)
			pr_err("[STUI] pm_runtime_put parent failed:%d\n", retval);
...
int stui_i2c_protect(bool is_protect)
{
	...
	if (is_protect) {
		ret = tpd_enter_tui();
		if (ret) {
			pr_err("[STUI] tpd_enter_tui failed:%d\n", ret);
			return ret;
		}
		ret = i2c_tui_enable_clock(1);
	} else {
		ret = tpd_exit_tui();
		if (ret) {
			pr_err("[STUI] tpd_exit_tui failed:%d\n", ret);
			return ret;
		}
		ret = i2c_tui_disable_clock(1);
	}
#else
	if (is_protect)
		ret = request_touch();
