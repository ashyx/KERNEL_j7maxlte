...
static struct cpufreq_interactive_tunables *user_tuned_parameters[NR_CPUS] = {NULL, };
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_POLICY_INIT:
		...
		if (!user_tuned_parameters[policy->cpu]) {
			...
			memcpy(tunables, user_tuned_parameters[policy->cpu], sizeof(*tunables));
			kfree(user_tuned_parameters[policy->cpu]);
		}
	...
	case CPUFREQ_GOV_POLICY_EXIT:
		if (!--tunables->usage_count) {
			...
			user_tuned_parameters[policy->cpu] = kzalloc(sizeof(*tunables), GFP_KERNEL);
			if (!user_tuned_parameters[policy->cpu]) {
				pr_err("%s: POLICY_EXIT: kzalloc failed\n", __func__);
				return -ENOMEM;
			}
			memcpy(user_tuned_parameters[policy->cpu], tunables, sizeof(*tunables));
