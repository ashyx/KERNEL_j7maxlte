...
static int sm5705_fg_dev_pinctrl_configure_irq(
	struct sec_fuelgauge_info *fuelgauge)
{
	...
	if (IS_ERR(set_state)) {
		pr_err("%s: cannot get fg_alrt_default\n", __func__);
		return -EINVAL;
	}

	retval = pinctrl_select_state(fuelgauge->pinctrl, set_state);
	if (retval) {
		pr_err("%s: cannot set pinctrl fg_alrt_default state\n", __func__);
		return -EINVAL;
	}
	return 0;
}
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		...
		case SEC_BATTERY_CAPACITY_CYCLE:
			sm5705_get_soc_cycle(fuelgauge->client);
			break;
...
//	struct battery_data_t *battery_data = NULL;
	...
	ret = sm5705_fg_dev_pinctrl_configure_irq(fuelgauge);
	if (ret)
	...
	fuelgauge->psy_fg = power_supply_register(
			&client->dev, &sm5705_fuelgauge_power_supply_desc, &fuelgauge_cfg);
	if (!fuelgauge->psy_fg) {
		dev_err(&client->dev,
...
err_parse_dt:
	if(fuelgauge->pdata)
		devm_kfree(&client->dev, fuelgauge->pdata);
//	if(battery_data)
