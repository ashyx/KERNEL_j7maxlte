	...
	case HV_CHARGER_STATUS:
		{
			...
			if (is_hv_wire_12v_type(battery->cable_type) ||  /* 20000mW */
				battery->max_charge_power >= (battery->pdata->pd_charging_charge_power + 5000)) {
				check_val = 2;
			} else if (is_hv_wire_type(battery->cable_type) || /* 14500mW */
				battery->max_charge_power >= (battery->pdata->pd_charging_charge_power - 500)) {
				check_val = 1;
			}
			i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n", check_val);
		}
...
static int sec_battery_probe(struct platform_device *pdev)
{
	...
	battery->skip_swelling = false;
#if 0
	sec_bat_set_current_event(battery, SEC_BAT_CURRENT_EVENT_USB_100MA, SEC_BAT_CURRENT_EVENT_USB_100MA); // HW dependent
#endif
