...
int timer_compensate_pre_handler(void)
{
	...
	if (err) {
		pr_info("unable to read the hardware clock\n");
		goto err_read;
	}
...
EXPORT_SYMBOL(timer_compensate_pre_handler);

int timer_compensate_post_handler(void)
{
	int err = -ENODEV;
	struct rtc_device *rtc = rtc_class_open(CONFIG_RTC_HCTOSYS_DEVICE);
	struct rtc_time		tm;
	...
	if (sleep_time.tv_sec >= 0)
		timekeeping_inject_offset(&sleep_time);
...
EXPORT_SYMBOL(timer_compensate_post_handler);
