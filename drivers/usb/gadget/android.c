...
static void android_work(struct work_struct *data)
{
	...
	printk(KERN_DEBUG "usb: %s config=%p,connected=%d,sw_connected=%d, sw_connected=%pK\n",
			__func__, cdev->config, dev->connected,
			dev->sw_connected, &dev->sw_connected);
...
static int a_dm_function_bind_config(struct android_usb_function *f,
					struct usb_configuration *c)
{
	return dm_bind_config(c, A_DM_PORT_NUM);
}

static struct android_usb_function a_dm_function = {
	.name           = "a_dm",
	.bind_config    = a_dm_function_bind_config,
};
