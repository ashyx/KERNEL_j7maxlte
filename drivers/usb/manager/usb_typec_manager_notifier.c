				...
				unsigned long action, void *data)
{
	...
	case STATUS_VBUS_LOW:
		...
		if(typec_manager.muic_attach_state_without_ccic
			...
			{
				msleep(500);
				pr_info("usb: [M] %s: without_ccic(%d), ccic_rid_state (%d)\n", __func__, typec_manager.muic_attach_state_without_ccic, typec_manager.ccic_rid_state);
				if( typec_manager.ccic_rid_state == RID_523K ||  typec_manager.ccic_rid_state == RID_619K)
					break;
			}	
