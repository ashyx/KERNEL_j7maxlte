		...
		int detect_threshold)
{
	...
	pr_info("%s int_ctrl = %d detect_period = %d detect_threshold = %d\n",
		__func__, int_ctrl, detect_period, detect_threshold);
	...
	if (etspi->drdy_irq_flag == DRDY_IRQ_DISABLE) {
		if (request_irq(gpio_irq, etspi_fingerprint_interrupt,
...
static void etspi_spi_pin_control(struct etspi_data *etspi,
	bool pin_set)
{
	int status = 0;
	...
	if (pin_set) {
		if (!IS_ERR(etspi->spi0_miso_set_cfg)) {
			...
			if (status)
				pr_err("%s: can't set spi0_miso_set_cfg\n",
					__func__);
		}
		if (!IS_ERR(etspi->spi0_cs_set_cfg)) {
			...
			if (status)
				pr_err("%s: can't set spi0_cs_set_cfg\n",
					__func__);
		}
		if (!IS_ERR(etspi->spi0_mosi_set_cfg)) {
			...
			if (status)
				pr_err("%s: can't set spi0_mosi_set_cfg\n",
					__func__);
		}
		if (!IS_ERR(etspi->spi0_clk_set_cfg)) {
			...
			if (status)
				pr_err("%s: can't set spi0_clk_set_cfg\n",
					__func__);
		}
	} else {
		if (!IS_ERR(etspi->spi0_miso_clr_cfg)) {
			...
			if (status)
				pr_err("%s: can't set spi0_miso_clr_cfg\n",
					__func__);
		}
		if (!IS_ERR(etspi->spi0_cs_clr_cfg)) {
			...
			if (status)
				pr_err("%s: can't set spi0_cs_clr_cfg\n",
					__func__);
		}
		if (!IS_ERR(etspi->spi0_mosi_clr_cfg)) {
			...
			if (status)
				pr_err("%s: can't set spi0_mosi_clr_cfg\n",
					__func__);
		}
		if (!IS_ERR(etspi->spi0_clk_clr_cfg)) {
			...
			if (status)
				pr_err("%s: can't set spi0_clk_clr_cfg\n",
					__func__);
		}
	}
}

static void etspi_irq_pin_control(struct etspi_data *etspi,
	bool pin_set)
{
	int status = 0;

	if (pin_set) {
		if (!IS_ERR(etspi->btp_irq)) {
			status = pinctrl_select_state(etspi->p,
				etspi->btp_irq);
			if (status)
				pr_err("%s: can't set btp_irq\n",
					__func__);
		}
	} else {
		if (!IS_ERR(etspi->btp_irq_sleep)) {
			status = pinctrl_select_state(etspi->p,
				etspi->btp_irq_sleep);
			if (status)
				pr_err("%s: can't set btp_irq_sleep\n",
					__func__);
		}
	}

}
static void etspi_chip_pin_control(struct etspi_data *etspi)
{
	int status = 0;

	if (!IS_ERR(etspi->btp_check)) {
		status = pinctrl_select_state(etspi->p,
			etspi->btp_check);
		if (status)
			pr_err("%s: can't set btp_check\n",
				__func__);
	}
	if (!IS_ERR(etspi->btp_irq_sleep)) {
		status = pinctrl_select_state(etspi->p,
	...
	if (!etspi->exp_sleep) {
		if (!IS_ERR(etspi->btp_reset)) {
			status = pinctrl_select_state(etspi->p,
				etspi->btp_reset);
			if (status)
				pr_err("%s: can't set btp_reset\n",
					__func__);
		}
	} else {
		pr_info("%s : Using expander btp_reset\n", __func__);
	}
	if (!etspi->exp_ldo) {
		if (!IS_ERR(etspi->btp_ldo)) {
			status = pinctrl_select_state(etspi->p,
				etspi->btp_ldo);
			if (status)
				pr_err("%s: can't set btp_ldo\n",
					__func__);
		}
	} else {
		pr_info("%s : Using expander btp_ldo\n", __func__);
	}
}
...
{
	if (etspi->ldocontrol) {
		etspi_irq_pin_control(etspi, pin_set);
#ifndef ENABLE_SENSORS_FPRINT_SECURE
		etspi_spi_pin_control(etspi, pin_set);
#endif
	}
}
		...
		if (etspi->min_cpufreq_limit) {
			if (ioc->len) {
				pr_info("%s FP_CPU_SPEEDUP ON:%d\n",
					__func__, ioc->len);
				mt_ppm_sysboost_core(BOOST_BY_FPSENSOR, 8);
				mt_ppm_sysboost_freq(BOOST_BY_FPSENSOR,
					etspi->min_cpufreq_limit);
			} else {
				pr_info("%s FP_CPU_SPEEDUP OFF\n", __func__);
				mt_ppm_sysboost_core(BOOST_BY_FPSENSOR, 0);
				mt_ppm_sysboost_freq(BOOST_BY_FPSENSOR, 0);
			}
		}
#endif
#endif
...
int etspi_init_platform(struct etspi_data *etspi)
{
	int status = 0;
	...
	if (etspi != NULL) {
		...
		if (etspi->ldo_pin) {
			...
			if (status < 0) {
				pr_err("%s gpio_request etspi_ldo_en failed\n",
					__func__);
				goto etspi_init_platform_ldo_failed;
			}
		...
		if (status < 0) {
			pr_err("%s gpio_requset etspi_sleep failed\n",
				__func__);
			goto etspi_init_platform_sleep_failed;
		}
		...
		if (status < 0) {
			pr_err("%s gpio_request etspi_drdy failed\n",
				__func__);
			goto etspi_init_platform_drdy_failed;
		}
		...
		if (status < 0) {
			pr_err("%s gpio_direction_input DRDY failed\n",
				__func__);
			goto etspi_init_platform_gpio_init_failed;
		}
	...
	return status;
etspi_init_platform_gpio_init_failed:
	gpio_free(etspi->drdyPin);
etspi_init_platform_drdy_failed:
	gpio_free(etspi->sleepPin);
etspi_init_platform_sleep_failed:
	gpio_free(etspi->ldo_pin);
etspi_init_platform_ldo_failed:
	pr_err("%s is failed\n", __func__);
	return status;
}
	...
	struct etspi_data *data)
{
	...
	if (of_property_read_u32(np, "etspi-exp_sleep",
			&data->exp_sleep))
		data->exp_sleep = 0;

	pr_info("%s: exp_sleep=%d\n",
		__func__, data->exp_sleep);

	if (of_property_read_u32(np, "etspi-exp_ldo",
			&data->exp_ldo))
		data->exp_ldo = 0;

	pr_info("%s: exp_ldo=%d\n",
		__func__, data->exp_ldo);
	...
	data->btp_check = pinctrl_lookup_state(data->p, "btp_check");
	if (IS_ERR(data->btp_check)) {
		pr_err("%s : could not get pins btp_check (%li)\n",
			__func__, PTR_ERR(data->btp_check));
		goto fail_pinctrl_get;
	}
	data->btp_irq = pinctrl_lookup_state(data->p, "btp_irq");
	if (IS_ERR(data->btp_irq)) {
		pr_err("%s : could not get pins btp_irq (%li)\n",
			__func__, PTR_ERR(data->btp_irq));
		goto fail_pinctrl_get;
	}
	data->btp_irq_sleep = pinctrl_lookup_state(data->p, "btp_irq_sleep");
	if (IS_ERR(data->btp_irq_sleep)) {
		pr_err("%s : could not get pins btp_irq_sleep (%li)\n",
			__func__, PTR_ERR(data->btp_irq_sleep));
		goto fail_pinctrl_get;
	}

	if (!data->exp_sleep) {
		data->btp_reset = pinctrl_lookup_state(data->p, "btp_reset");
		if (IS_ERR(data->btp_reset)) {
			pr_err("%s : could not get pins btp_reset (%li)\n",
				__func__, PTR_ERR(data->btp_reset));
				goto fail_pinctrl_get;
		}
	} else {
		pr_info("%s : Using expander btp_reset\n", __func__);
	...
	/* init pin status*/
	etspi_chip_pin_control(data);

	data->spi0_miso_set_cfg =
		pinctrl_lookup_state(data->p, "spi0_miso_set_cfg");
	if (IS_ERR(data->spi0_miso_set_cfg)) {
		pr_err("%s : could not get pins spi0_miso_set_cfg (%li)\n",
			__func__, PTR_ERR(data->spi0_miso_set_cfg));
		goto fail_pinctrl_get;
	}
	data->spi0_miso_clr_cfg =
		pinctrl_lookup_state(data->p, "spi0_miso_clr_cfg");
	if (IS_ERR(data->spi0_miso_clr_cfg)) {
		pr_err("%s : could not get pins spi0_miso_clr_cfg (%li)\n",
			__func__, PTR_ERR(data->spi0_miso_clr_cfg));
		goto fail_pinctrl_get;
	}
	data->spi0_cs_set_cfg =
		pinctrl_lookup_state(data->p, "spi0_cs_set_cfg");
	if (IS_ERR(data->spi0_cs_set_cfg)) {
		pr_err("%s : could not get pins spi0_cs_set_cfg (%li)\n",
			__func__, PTR_ERR(data->spi0_cs_set_cfg));
		goto fail_pinctrl_get;
	}
	data->spi0_cs_clr_cfg =
		pinctrl_lookup_state(data->p, "spi0_cs_clr_cfg");
	if (IS_ERR(data->spi0_cs_clr_cfg)) {
		pr_err("%s : could not get pins spi0_cs_clr_cfg (%li)\n",
			__func__, PTR_ERR(data->spi0_cs_clr_cfg));
		goto fail_pinctrl_get;
	}
	data->spi0_mosi_set_cfg =
		pinctrl_lookup_state(data->p, "spi0_mosi_set_cfg");
	if (IS_ERR(data->spi0_mosi_set_cfg)) {
		pr_err("%s : could not get pins spi0_mosi_set_cfg (%li)\n",
			__func__, PTR_ERR(data->spi0_mosi_set_cfg));
		goto fail_pinctrl_get;
	}
	data->spi0_mosi_clr_cfg =
		pinctrl_lookup_state(data->p, "spi0_mosi_clr_cfg");
	if (IS_ERR(data->spi0_mosi_clr_cfg)) {
		pr_err("%s : could not get pins spi0_mosi_clr_cfg (%li)\n",
			__func__, PTR_ERR(data->spi0_mosi_clr_cfg));
		goto fail_pinctrl_get;
	}
	data->spi0_clk_set_cfg =
		pinctrl_lookup_state(data->p, "spi0_clk_set_cfg");
	if (IS_ERR(data->spi0_clk_set_cfg)) {
		pr_err("%s : could not get pins spi0_clk_set_cfg (%li)\n",
			__func__, PTR_ERR(data->spi0_clk_set_cfg));
		goto fail_pinctrl_get;
	}
	data->spi0_clk_clr_cfg =
		pinctrl_lookup_state(data->p, "spi0_clk_clr_cfg");
	if (IS_ERR(data->spi0_clk_clr_cfg)) {
		pr_err("%s : could not get pins spi0_clk_clr_cfg (%li)\n",
			__func__, PTR_ERR(data->spi0_clk_clr_cfg));
		goto fail_pinctrl_get;
	}
#ifdef ENABLE_SENSORS_FPRINT_SECURE
	etspi_spi_pin_control(data, 0);
#endif
...
static int etspi_probe(struct spi_device *spi)
{
	...
	/* platform init */
	status = etspi_init_platform(etspi);
	if (status != 0) {
		pr_err("%s platforminit failed\n", __func__);
		goto etspi_probe_init_platform_failed;
	}
	...
	spi->chip_select = 0;
#ifdef CONFIG_MACH_MT6757
	etspi->spi_conf.holdtime = 3;
	etspi->spi_conf.high_time = 10;
	etspi->spi_conf.low_time = 10;
	etspi->spi_conf.cs_idletime = 2;
	etspi->spi_conf.ulthgh_thrsh = 0;
	etspi->spi_conf.cpol = 0; /* SPI_0 MODE */
	etspi->spi_conf.cpha = 0; /* SPI_0 MODE */
	etspi->spi_conf.com_mod = DMA_TRANSFER; /* DMA MODE */

	etspi->spi_conf.rx_mlsb = 1;
	etspi->spi_conf.tx_mlsb = 1;

	etspi->spi_conf.tx_endian = 0;
	etspi->spi_conf.rx_endian = 0;

	etspi->spi_conf.pause = 1;
	etspi->spi_conf.finish_intr = 1;
	etspi->spi_conf.deassert = 0;
	etspi->spi_conf.ulthigh = 0;
	etspi->spi_conf.tckdly = 0;

	spi->controller_data = (void *)&etspi->spi_conf;
#endif
...
#endif
etspi_probe_init_platform_failed:
