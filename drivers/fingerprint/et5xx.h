...
#include "../misc/mediatek/include/mt-plat/mt6757/include/mach/mtk_ppm_api.h"
#endif
#endif
...
struct etspi_data {
	...
	struct pinctrl *p;
	struct pinctrl_state *btp_check;
	struct pinctrl_state *btp_irq;
	struct pinctrl_state *btp_irq_sleep;
	struct pinctrl_state *btp_reset;
	...
	unsigned int ldocontrol; /* check for use the pin control */
	unsigned int exp_sleep;	/* check for use expander sleep pin */
	unsigned int exp_ldo;	/* check for use expander ldo pin */
