...
#if defined(CONFIG_MUIC_S2MU005)
	{ .name = "s2mu005-muic", },
...
#if defined(CONFIG_LEDS_S2MU005_RGB)
...
int of_s2mu005_pinctrl(struct i2c_client *i2c)
{
	struct pinctrl *muic_pinctrl;

	pr_info("%s\n", __func__);

	muic_pinctrl = devm_pinctrl_get_select_default(&i2c->dev);
				...
				const struct i2c_device_id *dev_id)
{
	...
	if (i2c->dev.of_node) {
		...
		/* TODO: remove after enable feature of board init GPIO control */
		ret = of_s2mu005_pinctrl(i2c);
		if (ret < 0) {
			pr_err("%s: Failed to set pinctrl\n", __func__);
			goto err;
		}
	} else
