...
{
	struct madera_pdata *pdata = &madera->pdata;
	int ldo_2p8_en_gpio, ldo_1p8_en_gpio, ldo_1p2_en_gpio, test_rst_gpio;
	int ret = 0;

	ldo_2p8_en_gpio = of_get_named_gpio(madera->dev->of_node,
					 "ldo-2p8-en-gpios", 0);
	if (gpio_is_valid(ldo_2p8_en_gpio)) {
		ret = gpio_request(ldo_2p8_en_gpio, "ldo_2p8_en_gpio");
		if (ret) {
			pr_err("%s: ldo_2p8_en_gpio error\n", __func__);
		}
		ret = gpio_direction_output(ldo_2p8_en_gpio, 1);
		if (ret)
			gpio_free(ldo_2p8_en_gpio);
	} else
		pr_info("%s: ldo_2p8_en_gpio is not supported\n", __func__);

	ldo_1p8_en_gpio = of_get_named_gpio(madera->dev->of_node,
					 "ldo-1p8-en-gpios", 0);
	if (gpio_is_valid(ldo_1p8_en_gpio)) {
		ret = gpio_request(ldo_1p8_en_gpio, "ldo_1p8_en_gpio");
		if (ret) {
			pr_err("%s: ldo_1p8_en_gpio error\n", __func__);
			return 0;
		}
	}
	ret = gpio_direction_output(ldo_1p8_en_gpio, 1);
	if (ret) {
		gpio_free(ldo_1p8_en_gpio);
		return 0;
	}

	ldo_1p2_en_gpio = of_get_named_gpio(madera->dev->of_node,
					 "ldo-1p2-en-gpios", 0);
	if (gpio_is_valid(ldo_1p2_en_gpio)) {
		ret = gpio_request(ldo_1p2_en_gpio, "ldo_1p2_en_gpio");
		if (ret) {
			pr_err("%s: ldo_1p2_en_gpio error\n", __func__);
			return 0;
		}
	}
	ret = gpio_direction_output(ldo_1p2_en_gpio, 1);
	if (ret) {
		gpio_free(ldo_1p2_en_gpio);
		return 0;
	}

	test_rst_gpio = of_get_named_gpio(madera->dev->of_node,
					 "test-rst-gpios", 0);
	if (gpio_is_valid(test_rst_gpio)) {
		ret = gpio_request(test_rst_gpio, "test_rst_gpio");
		if (ret) {
			pr_err("%s: test_rst_gpio error\n", __func__);
			return 0;
		}
	}
	ret = gpio_direction_output(test_rst_gpio, 1);
	if (ret) {
		gpio_free(test_rst_gpio);
		return 0;
	}
...
int madera_dev_init(struct madera *madera)
{
	...
	dev_info(madera->dev, "%s: init done\n", __func__);
