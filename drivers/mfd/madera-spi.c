...
#ifdef CONFIG_MTK_SPI
	/* MTK set SPI config */
	ret = spi_set_para(spi);
	if (ret) {
		dev_err(&spi->dev, "%s: spi_set_para failed\n", __func__);
	}
#endif
...
static int __init madera_spi_dev_init(void)
{
	pr_info("Entry : %s\n", __func__);

	return spi_register_driver(&madera_spi_driver);
}

late_initcall(madera_spi_dev_init);
