...
struct et6312b_data {
	...
	/* et6312b settings & smartglow tuning data*/
...
**/
static int s_led_en_ctrl(struct et6312b_data *data, bool on)
{
	struct pinctrl_state *p_state;

	if (on && !data->s_led_en) {
		p_state = pinctrl_lookup_state(data->pinctrl, "s_led_en");
		if (IS_ERR(p_state)) {
			dev_err(&data->client->dev, "%s: Failed to lookup s_led_en pinctrl.\n", __func__);
		} else {
			if (pinctrl_select_state(data->pinctrl, p_state))
				dev_err(&data->client->dev, "%s: Failed to configure s_led_en pinctrl.\n", __func__);
			else {
				data->s_led_en = true;
				return 0;
			}
		}
	} else if (!on && data->s_led_en) {
		p_state = pinctrl_lookup_state(data->pinctrl, "default");
		if (IS_ERR(p_state)) {
			dev_err(&data->client->dev, "%s: Failed to lookup default pinctrl.\n", __func__);
		} else {
			if (pinctrl_select_state(data->pinctrl, p_state))
				dev_err(&data->client->dev, "%s: Failed to configure default pinctrl.\n", __func__);
			else {
				data->s_led_en = false;
				return 0;
			}
...
{	
	...
	if (unlikely(s_led_en_ctrl(g_et6312b_data, false) < 0)) {
...
static int __inline et6312b_i2c_write_all(void)
{
	...
	if (unlikely(s_led_en_ctrl(g_et6312b_data, true) < 0))
					...
					u8 color, u8 led)
{
	 ...
	 */
	 ...
	 if(led==LED_R){
		return color ? (((color * (LED_MAX_CURRENT_R - LED_MIN_CURRENT_R)) + (LED_HIGHER_SUPPORTED_COLOR * LED_MIN_CURRENT_R)\
		- (LED_LOWER_SUPPORTED_COLOR * LED_MAX_CURRENT_R)) / (LED_HIGHER_SUPPORTED_COLOR - LED_LOWER_SUPPORTED_COLOR)) : 0;
	 }
	 else if(led==LED_G){
		return color ? (((color * (LED_MAX_CURRENT_G - LED_MIN_CURRENT_G)) + (LED_HIGHER_SUPPORTED_COLOR * LED_MIN_CURRENT_G)\
		- (LED_LOWER_SUPPORTED_COLOR * LED_MAX_CURRENT_G)) / (LED_HIGHER_SUPPORTED_COLOR - LED_LOWER_SUPPORTED_COLOR)) : 0;
	 }
     else if(led==LED_B){
		return color ? (((color * (LED_MAX_CURRENT_B - LED_MIN_CURRENT_B)) + (LED_HIGHER_SUPPORTED_COLOR * LED_MIN_CURRENT_B)\
		- (LED_LOWER_SUPPORTED_COLOR * LED_MAX_CURRENT_B)) / (LED_HIGHER_SUPPORTED_COLOR - LED_LOWER_SUPPORTED_COLOR)) : 0;
	 }
	 else{
		return color ? (((color * (LED_MAX_CURRENT_W - LED_MIN_CURRENT_W)) + (LED_HIGHER_SUPPORTED_COLOR * LED_MIN_CURRENT_W)\
		- (LED_LOWER_SUPPORTED_COLOR * LED_MAX_CURRENT_W)) / (LED_HIGHER_SUPPORTED_COLOR - LED_LOWER_SUPPORTED_COLOR)) : 0;
	 }
}
		...
		enum led_brightness color, u8 intensity)
{
	...
	if(r==g && g==b)
	{
		return ET6312B_FROM_COLOR(et6312b_color_scale(r, LED_W), \
			et6312b_color_scale(g, LED_W), et6312b_color_scale(b, LED_W));  
...
int et6312b_config_led(u8 rgb_num)
{
	...
	//if(!g_et6312b_data->led[rgb_num].color || !g_et6312b_data->led[rgb_num].ontime)	
	...
	#if 1 	
	// TSN: Check
	// Option a: Compose register values in shadow register, then update 
	// Option b: update RGB registers instantaneously 						
			if(unlikely((i2c_smbus_write_byte_data(g_et6312b_data->client, \
					ET6312B_REG_LEDXMD_RGBx_BASE + rgb_num, ET6312B_RESET)) < 0)) {
				dev_err(&g_et6312b_data->client->dev, \
					"%s: failure on i2c reset\n", __func__);
			}
	#endif	
...
static int et6312b_parse_dt(struct device *dev, struct et6312b_data *data) {
	...
	//To Do: check tuning data needs to be enumerated
