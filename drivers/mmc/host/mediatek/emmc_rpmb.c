...
static struct tz_uuid rpmb_uuid_raw = {
	.time_low = 0x00000000,
	.time_mid = 0x4D54,
	.time_hi_and_version = 0x4B5F,
	.clock_seq_and_node = {
		0x42, 0x46, 0x72, 0x70, 0x6D, 0x62, 0x64, 0x72
	}
};
static struct tz_uuid *rpmb_uuid = &rpmb_uuid_raw;
...
#if defined(CONFIG_TRUSTONIC_TEE_SUPPORT) || defined(CONFIG_BLOWFISH_TEE_SUPPORT)
...
#if defined(CONFIG_TRUSTONIC_TEE_SUPPORT)
#define emmc_rpmb_listenDci     emmc_rpmb_listenDci_trustonic
int emmc_rpmb_listenDci_trustonic(void *data)
{
...
#define emmc_rpmb_open_session()        emmc_rpmb_open_session_trustonic()
static int emmc_rpmb_open_session_trustonic(void)
{
	int cnt = 0;
	...
	while (cnt < 30) {
		...
		/* open session */
		mc_ret = mc_open_session(&rpmb_session, &rpmb_uuid, (uint8_t *) rpmb_dci, sizeof(dciMessage_t));
...
static int bf_client_id, bf_shmem_id;
static char *bf_shmem;
static int __kcmd_notifier(struct notifier_block *nb, unsigned long action, void *data)
{
	/* do nothing but wake up emmc_rpmb_listenDci_blowfish */
...
static unsigned int iwnotify_event = TZ_IWNOTIFY_OEM_NOTIFICATION_FLAG_4;
/* #define WAIT_FOR_NOTIFICATION_WITH_TMO */
#define emmc_rpmb_listenDci     emmc_rpmb_listenDci_blowfish
int emmc_rpmb_listenDci_blowfish(void *data)
{
	struct tdriver_msg msg;
	u32 cmdId;
#ifdef WAIT_FOR_NOTIFICATION_WITH_TMO
	unsigned long tmo;
#endif

	tzdev_print(0, "%s: DCI listener.\n", __func__);

	for (;;) {

		tzdev_print(0, "%s: Waiting for notification\n", __func__);

#ifdef WAIT_FOR_NOTIFICATION_WITH_TMO
		tmo = jiffies + 2*HZ;
		ret = wait_for_completion_interruptible_timeout(&kcmd_event, HZ*2+1);
		if (time_after(jiffies, tmo)) {
			tzdev_print(0, "%s: no DCI recevied, wait again\n", __func__);
			continue;
		}
#else
		wait_for_completion_interruptible(&kcmd_event);
#endif

		cmdId = rpmb_dci->command.header.commandId;

		tzdev_print(0, "%s: wait notification done!! cmdId = %x\n", __func__, cmdId);

		if (cmdId == 0)
			continue;

		msg.result = emmc_rpmb_execute(cmdId);

		msg.cmd = TEE_COMMAND;
		tzdev_kapi_send(bf_client_id, &msg, sizeof(msg));
		wait_for_completion_interruptible_timeout(&kcmd_event, 500);
		tzdev_kapi_recv(bf_client_id, &msg, sizeof(msg));
		if (msg.result == -1) {
			MSG(ERR, "%s: tzdev_kapi_recv returned: %d\n", __func__, msg.result);
			/* goto out_error; */
		}
		tzdev_print(0, "%s: wait notification done2\n", __func__);
...
#define emmc_rpmb_open_session()        emmc_rpmb_open_session_blowfish()
#define BLOWFISH_OPEN_SESSION_TMO       300
static int emmc_rpmb_open_session_blowfish(void)
{
	int cnt = 0, ret = 0;
	struct tdriver_msg msg;
	unsigned long open_tmo = jiffies + (BLOWFISH_OPEN_SESSION_TMO * HZ);

	tzdev_print(0, "%s start\n", __func__);

	while (!tzdev_is_up()) {
		tzdev_print(0, "blowfish NOT ready!, sleep 1s\n");
		msleep(1000);
		cnt++;
		if (cnt == BLOWFISH_OPEN_SESSION_TMO) {
			tzdev_print(0, "blowfish NOT ready for %ds!\n", cnt);
			return -ETIMEDOUT;
		}
	}
	...
	bf_shmem = kmalloc(ALLOCATED_DCI_MSG_SIZE, GFP_KERNEL);
	if (!bf_shmem) {
		tzdev_print(0, "%s, alloc memory failed\n", __func__);
		/* return -ENOMEM; */
		return 0;
	}
	memset(bf_shmem, 0, ALLOCATED_DCI_MSG_SIZE);

retry:
	tzdev_print(0, "tzdev_kapi_open()\n");
	bf_client_id = tzdev_kapi_open(rpmb_uuid);
	if (bf_client_id < 0) {
		tzdev_print(0, "tzdev_kapi_open failed - %d\n", bf_client_id);
		goto out_tmo;
	}

	tzdev_print(0, "tzdev_mem_register()\n");
	bf_shmem_id = tzdev_mem_register(bf_shmem, ALLOCATED_DCI_MSG_SIZE, 1);
	if (bf_shmem_id < 0) {
	...
	ret = tz_iwnotify_chain_register(iwnotify_event, &kcmd_notifier);
	if (ret < 0) {
		tzdev_print(0, "tz_iwnotify_chain_register failed - %d\n", ret);
		goto out_unregister;
	}

	tzdev_print(0, "tzdev_kapi_mem_grant()\n");
	ret = tzdev_kapi_mem_grant(bf_client_id, bf_shmem_id);
	if (ret < 0) {
	...
	msg.cmd = MAP_IWSHMEM;
	msg.arg = ALLOCATED_DCI_MSG_SIZE;
	msg.iwshmem_id = bf_shmem_id;

	tzdev_kapi_send(bf_client_id, &msg, sizeof(msg));
	wait_for_completion_interruptible_timeout(&kcmd_event, 2000);
	tzdev_kapi_recv(bf_client_id, &msg, sizeof(msg));
	if (msg.result == -1) {
		tzdev_print(0, "tzdev_kapi_recv for MAP_IWSHMEM failed\n");
		goto out_ungrant;
	}

	/* create a thread for listening DCI signals */
	rpmb_dci = (dciMessage_t *) bf_shmem;
	rpmbDci_th = kthread_run(emmc_rpmb_listenDci, NULL, "rpmb_Dci");
	if (IS_ERR(rpmbDci_th)) {
		MSG(ERR, "%s, init kthread_run failed!\n", __func__);
		goto out_ungrant;
...
out_ungrant:
	...
	tzdev_kapi_mem_revoke(bf_client_id, bf_shmem_id);

out_iwnotify:
	tzdev_print(0, "tz_iwnotify_chain_unregister()\n");
	ret = tz_iwnotify_chain_unregister(iwnotify_event, &kcmd_notifier);
	if (ret < 0)
		tzdev_print(0, "tz_iwnotify_chain_unregister failed - %d\n", ret);

out_unregister:
	tzdev_print(0, "tzdev_mem_release memory\n");
	tzdev_mem_release(bf_shmem_id);
	bf_shmem_id = 0;
	rpmb_dci = NULL;

out_close:
	tzdev_print(0, "tzdev_kapi_close()\n");
	ret = tzdev_kapi_close(bf_client_id);
	if (ret < 0)
		tzdev_print(0, "tzdev_kapi_close failed - %d\n", ret);

out_tmo:
	if (!time_after(jiffies, open_tmo)) {
		msleep(1000);
		goto retry;
	} else {
		tzdev_print(0, "tzdev_kapi_open aborted after %d seconds\n", BLOWFISH_OPEN_SESSION_TMO);
	}

	tzdev_print(0, "free kernel memory\n");
	kfree(bf_shmem);
