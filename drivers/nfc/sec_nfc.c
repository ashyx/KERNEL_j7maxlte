...
#include "../misc/mediatek/base/power/mt6757/mtk_clkbuf_ctl.h"
#include "../misc/mediatek/base/power/include/spm_v2/mtk_spm_resource_req.h"

//#define MTK_DMA_I2C
#ifdef MTK_DMA_I2C
#include <linux/dma-mapping.h>
static u8 *nfcDMABuf_va;
static dma_addr_t nfcDMABuf_pa;
...
#ifdef CONFIG_MTK_I2C_EXTENSION
	info->i2c_info.i2c_dev->addr = info->i2c_info.i2c_dev->addr & I2C_MASK_FLAG;
	info->i2c_info.i2c_dev->ext_flag |= I2C_DMA_FLAG;
...
//	mt6605_dev->client->timing = NFC_CLIENT_TIMING;
	...
	ret =
	  //  i2c_master_recv(mt6605_dev->client,
	    i2c_master_recv(info->i2c_info.i2c_dev,
	...
	if ( count > 8 ) {
		...
		info->i2c_info.i2c_dev->addr = info->i2c_info.i2c_dev->addr & I2C_MASK_FLAG;
		info->i2c_info.i2c_dev->ext_flag = (info->i2c_info.i2c_dev->ext_flag |
				I2C_ENEXT_FLAG | I2C_DMA_FLAG);
		ret = i2c_master_recv(info->i2c_info.i2c_dev, (char *)(uintptr_t)nfcDMABuf_pa, count);
		info->i2c_info.i2c_dev->ext_flag = 0;
		if (ret > 0)
			memcpy(buf, nfcDMABuf_va, count);
		else
			pr_err("%s i2c_dma fail! ret:%d\n", __func__, ret);
	} else
#endif
#if 1
	{
...
//		mt6605_dev->client->timing = NFC_CLIENT_TIMING;
		...
		ret_tmp =
		  //  i2c_master_send(mt6605_dev->client,
				i2c_master_send(info->i2c_info.i2c_dev, 
	...
	if ( count > 8 ) {
		...
		ret = i2c_master_send(info->i2c_info.i2c_dev, (char *)(uintptr_t)nfcDMABuf_pa, count);
	} else
#endif
#if 1 
	{
...
#ifdef MTK_DMA_I2C
	info->dev->coherent_dma_mask = DMA_BIT_MASK(32);
	nfcDMABuf_va = dma_alloc_coherent(info->dev, IIC_DMA_MAX_TRANSFER_SIZE,
			&nfcDMABuf_pa, GFP_KERNEL);
	if (!nfcDMABuf_va) {
		dev_err(dev, "Allocate DMA I2C Buffer failed!\n");
		return -1;
	}
	memset(nfcDMABuf_va, 0, IIC_DMA_MAX_TRANSFER_SIZE);
#endif
#ifdef CONFIG_MTK_I2C_EXTENSION
	I2CDMAWriteBuf =
...
static void sec_nfc_clk_work(struct work_struct *work)
{
	struct sec_nfc_platform_data *pdata = container_of(work,
						struct sec_nfc_platform_data, nfc_clk_wq);

	pr_debug("[sec-nfc]Clock work! %d\n", pdata->clk_enable);
...
static irqreturn_t sec_nfc_clk_irq(int irq, void *dev_id)
{
	...
	if (value == info->clk_state)
		return IRQ_HANDLED;
	pdata->clk_enable = value;

	if (value) {
		wake_lock(&info->nfc_wake_lock);
		spm_resource_req(SPM_RESOURCE_USER_UFS,	SPM_RESOURCE_CK_26M);
		queue_work(pdata->nfc_wq, &pdata->nfc_clk_wq);
...
static int nfc_state_print(struct sec_nfc_info *info)
{
	...
	if (pdata->nfc_pvdd != NULL) {
		...
		pvdd = gpio_get_value(pdata->pvdd);
...
static int __sec_nfc_probe(struct device *dev)
{
    ...
    if(pdata->pvdd) {
        ...
        if (ret) {
            dev_err(dev, "[NFC] failed to get gpio pvdd-gpio\n");
            gpio_free(pdata->pvdd);
            goto err_dev_reg;
        }
		if (!lpcharge) {
			dev_info(dev, "[NFC]sec-nfc pvdd on\n");
			gpio_direction_output(pdata->pvdd, 1);
		} else {
			dev_info(dev, "[NFC]sec-nfc pvdd off\n");
			gpio_direction_output(pdata->pvdd, 0);
		}
	} 

	if (of_get_property(dev->of_node, "sec-nfc,nfc_ap_clk", NULL)) {
		pdata->nfc_wq = create_singlethread_workqueue(SEC_NFC_DRIVER_NAME);
			if(!pdata->nfc_wq) {
				dev_err(info->dev, "failed to creat workqueue");
			}
		INIT_WORK(&pdata->nfc_clk_wq, &sec_nfc_clk_work);

		pdata->clk_irq = gpio_to_irq(pdata->clk_req);

		ret = request_threaded_irq(pdata->clk_irq, sec_nfc_clk_irq, NULL,
