...
static int ion_dma_reserved_heap_debug_show(struct ion_heap *heap, struct seq_file *s, void *unused);

ion_phys_addr_t ion_dma_reserved_allocate(struct ion_heap *heap, unsigned long size,
					  unsigned long align)
{
	struct ion_dma_reserved_heap
	*dma_reserved_heap = container_of(heap, struct ion_dma_reserved_heap, heap);
...
{
	struct ion_dma_reserved_buffer_info *buffer_info = (struct ion_dma_reserved_buffer_info *)buffer->priv_virt;
	...
	if (buffer_info->mva == 0) {
		...
		if (ret < 0) {
			...
			IONMSG("[ion_dma_reserved_heap_phys]: Error. Allocate MVA failed.\n");
			return -EFAULT;
		}
	...
	*len = buffer->size;
	...
	/* pr_debug("%s: MVA = 0x%x, len = 0x%lx.\n", __func__, buffer_info->mva, buffer->size); */

	return 0;
}

static int ion_dma_reserved_heap_phys(struct ion_heap *heap, struct ion_buffer *buffer,
				      ion_phys_addr_t *addr, size_t *len)
{
	struct ion_dma_reserved_buffer_info *buffer_info = (struct ion_dma_reserved_buffer_info *)buffer->priv_virt;
	...
	*len = buffer->size;
	...
	/* pr_err("[%s]: pa = 0x%lx, len = 0x%lx.\n", __func__, *addr, buffer->size); */
...
void ion_dma_reserved_heap_unmap_dma(struct ion_heap *heap, struct ion_buffer *buffer)
{
...
static int ion_dma_reserved_heap_debug_show(struct ion_heap *heap, struct seq_file *s,
					    void *unused)
{
	struct ion_dma_reserved_heap
	*dma_reserved_heap = container_of(heap, struct ion_dma_reserved_heap, heap);
	...
	gen_pool_for_each_chunk(dma_reserved_heap->pool, ion_dma_reserved_chunk_show, s);
	return 0;
}

struct ion_heap *ion_dma_reserved_heap_create(struct ion_platform_heap *heap_data)
{
	...
	gen_pool_add(dma_reserved_heap->pool, (unsigned long)dma_reserved_heap->phys_addr, dma_reserved_heap->size, -1);
	return &dma_reserved_heap->heap;
}

void ion_dma_reserved_heap_destroy(struct ion_heap *heap)
{
	struct ion_dma_reserved_heap *dma_reserved_heap = container_of(heap, struct ion_dma_reserved_heap, heap);
