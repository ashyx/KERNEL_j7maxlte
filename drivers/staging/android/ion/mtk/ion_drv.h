...
#ifdef MTK_SEC_SERVICE
struct ion_heap *ion_sec_heap_create(struct ion_platform_heap *unused);
void ion_sec_heap_destroy(struct ion_heap *heap);
#else
static inline struct ion_heap *ion_sec_heap_create(struct ion_platform_heap *unused)
{
	return NULL;
}

static inline void ion_sec_heap_destroy(struct ion_heap *heap) {}
#endif
