...
static void madera_micd_manual_timeout(struct madera_extcon_info *info)
{
...
static int madera_micd_manual_reading(struct madera_extcon_info *info, int val)
{
...
const struct madera_jd_state madera_micd_manual = {
...
int madera_extcon_manual_mic_reading(struct madera_extcon_info *info)
{
				   ...
				   bool clamp)
{
	...
	dev_info(madera->dev, "%s clamp mask=0x%x val=0x%x\n",
...
static int madera_micd_adc_read(struct madera_extcon_info *info)
{
	...
	dev_info(madera->dev, "MICDET_ADCVAL: 0x%x\n", val);
...
static int madera_micd_read(struct madera_extcon_info *info)
{
	...
	for (i = 0; i < 10 && !(val & MADERA_MICD_LVL_0_TO_8); i++) {
		...
		dev_info(madera->dev, "MICDET: 0x%x\n", val);
...
static int madera_hpdet_read(struct madera_extcon_info *info)
{
	...
	dev_info(madera->dev, "HPDET read: 0x%x\n", val);
...
static void madera_hs_mic_control(struct madera_extcon_info *info, int state)
{
	struct madera *madera = info->madera;
	...
	case MADERA_MIC_MUTE:
		dev_dbg(madera->dev, "Mute headset mic: 0x%04x\n",
			addr);
		snd_soc_dapm_mutex_lock(madera->dapm);
		regmap_update_bits(madera->regmap,
				   addr,
	...
	case MADERA_MIC_UNMUTE:
		dev_dbg(madera->dev, "Unmute headset mic: 0x%04x\n",
			addr);
...
int madera_micd_start(struct madera_extcon_info *info)
{
	...
	dev_info(madera->dev, "Disabling MICD_OVD\n");
	regmap_update_bits(madera->regmap,
...
void madera_micd_stop(struct madera_extcon_info *info)
{
	...
	dev_info(madera->dev, "Enabling MICD_OVD\n");
...
int madera_micd_button_start(struct madera_extcon_info *info)
{
	int ret;
...
static int madera_micd_button_process(struct madera_extcon_info *info, int val)
{
	...
	if (val < MADERA_MICROPHONE_MIN_OHM) {
		dev_info(madera->dev, "Mic button detected\n");
...
static irqreturn_t madera_hpdet_handler(int irq, void *data)
{
	...
	dev_info(madera->dev, "HPDET handler\n");
	...
	if (ret == -EAGAIN) {
		dev_info(madera->dev, "Repeat HPDET\n");
		goto out;
	}
...
static void madera_micd_handler(struct work_struct *work)
{
	...
	dev_info(madera->dev, "MICDET IRQ\n");
	...
	if (ret == -EAGAIN) {
		dev_info(madera->dev, "Repeat MICDET\n");
		goto out;
	}
...
static irqreturn_t madera_jackdet(int irq, void *data)
{
	...
	if (present < 0) {
		dev_err(madera->dev, "Failed to check jack status: %d\n",
			present);
		mutex_unlock(&info->lock);
		pm_runtime_put_autosuspend(info->dev);
		return IRQ_NONE;
				     ...
				     struct device_node *node)
{
	...
	of_property_read_u32(node, "cirrus,hs-mic", &pdata->hs_mic);
...
static int madera_extcon_probe(struct platform_device *pdev)
{
	...
	dev_info(madera->dev, " %s: probe done\n", __func__);
