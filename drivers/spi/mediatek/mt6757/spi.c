...
#define SPI_NO_LENGTH_LIMITATION
...
#define SPI_LOCAL_BUF_LENGTH	(4*1024)	/*4K*/
...
struct mt_spi_t {
	...
	void	*local_tx_buf;
	void		*local_rx_buf;
	dma_addr_t	local_tx_dma;
	dma_addr_t	local_rx_dma;
};
static int local_dma_mapping(struct mt_spi_t *ms, enum spi_dma_dir dir);
static void local_dma_unmapping(struct mt_spi_t *ms, enum spi_dma_dir dir);
...
#ifdef SPI_NO_LENGTH_LIMITATION
#define  EXTENSION_XFER_COUNT 32
static struct spi_transfer *extension_xfer[EXTENSION_XFER_COUNT];

static int insert_into_ext_xfer(struct spi_transfer *xfer)
{
	unsigned int i;
	int ret = 0;

	for (i = 0; i < EXTENSION_XFER_COUNT; i++)
		if (extension_xfer[i] == NULL) {
			extension_xfer[i] = xfer;
			break;
		}

	if (i == EXTENSION_XFER_COUNT) {
		SPI_ERR("the extension_xfer is full!\n");
		ret = -EFAULT;
	}

	return ret;
}
static int delete_from_ext_xfer(struct spi_transfer *xfer)
{
	int ret = 0;
	unsigned int i;

	for (i = 0; i < EXTENSION_XFER_COUNT; i++)
		if (extension_xfer[i] == xfer) {
			extension_xfer[i] = NULL;
			break;
		}

	if (i == EXTENSION_XFER_COUNT) {
		SPI_DBG("the xfer %p do not in extension_xfer!\n", xfer);
		ret = -EFAULT;
	}
	return ret;
}
static struct spi_transfer *extension_transfer_alloc(struct spi_transfer *xfer)
{
	struct spi_transfer *ext_xfer = NULL;
	...
	if (xfer == NULL)
		return NULL;
	/*modify this line*/
	if ((xfer->len <= PACKET_SIZE) || ((xfer->len % PACKET_SIZE) == 0))
		return NULL;

	first_len = xfer->len - (xfer->len % PACKET_SIZE);
	ext_xfer = kzalloc(sizeof(struct spi_transfer), GFP_ATOMIC);

	if (ext_xfer == NULL) {
		SPI_ERR("kzalloc spi_transfer failed!\n");
		return NULL;
	}
	ret = insert_into_ext_xfer(ext_xfer);
	if (ret < 0) {
		SPI_ERR("insert_into_ext_xfer failed!\n");
		kfree(ext_xfer);
		return NULL;
	}
	memcpy(ext_xfer, xfer, sizeof(struct spi_transfer));
	ext_xfer->len = xfer->len % PACKET_SIZE;
	if (xfer->tx_buf != NULL)
		ext_xfer->tx_buf = xfer->tx_buf + first_len;
	if (xfer->rx_buf != NULL)
		ext_xfer->rx_buf = xfer->rx_buf + first_len;
	if (xfer->tx_dma != 0)
		ext_xfer->tx_dma = xfer->tx_dma + first_len;
	if (xfer->rx_dma != 0)
		ext_xfer->rx_dma = xfer->rx_dma + first_len;

	xfer->len = first_len;

	return ext_xfer;
}
static void extension_transfer_free(struct spi_transfer *ext_xfer)
{
	int ret = 0;
	struct spi_transfer *pre_xfer = NULL;

	if (ext_xfer != NULL) {
		ret = delete_from_ext_xfer(ext_xfer);
		if (ret == 0) {
			pre_xfer = list_prev_entry(ext_xfer, transfer_list);
			if (pre_xfer)
				pre_xfer->len += ext_xfer->len;
			spi_transfer_del(ext_xfer);
			kfree(ext_xfer);
			ext_xfer = NULL;
		}
	}
}
static int mt_spi_msg_add_extension_xfer(struct spi_message *msg)
{
	int ret = 0;
	struct spi_transfer *xfer, *next_xfer;
	struct spi_transfer *ext_xfer = NULL;
	unsigned int ext_xfer_count = 0;

	if (msg == NULL)
		return -EFAULT;

	list_for_each_entry_safe(xfer, next_xfer, &msg->transfers, transfer_list) {
		ext_xfer = extension_transfer_alloc(xfer);
		if (ext_xfer) {
			list_add(&ext_xfer->transfer_list, &xfer->transfer_list);
			ext_xfer_count++;
		}
	}
	if (ext_xfer_count == 0)
		ret = -EFAULT;	/*no extension xfer*/

	return ret;
}
static void mt_spi_msg_del_extension_xfer(struct spi_message *msg)
{
		...
		if ((ms->cur_transfer->tx_buf != NULL)
			...
			if (ms->cur_transfer->tx_dma & (SPI_4B_ALIGN - 1)) {
				...
				dev_warn(&ms->pdev->dev,
					...
					ms->cur_transfer->tx_buf, ms->cur_transfer->tx_dma);
#endif
				if (ms->local_tx_buf == NULL)
					dev_warn(&ms->pdev->dev,"local_tx_buf is null\n");
				else {
					/* unmapping non 4byte align tx dma */
					transfer_dma_unmapping(ms, ms->cur_transfer);
					/* unmapping local dma */
					local_dma_unmapping(ms, DMA_TX);
					/* put data to local buf */
					memcpy(ms->local_tx_buf, ms->cur_transfer->tx_buf, ms->cur_transfer->len);
					/* mapping local dma */
					local_dma_mapping(ms, DMA_TX);
					/* not 4byte align use local buf */
					spi_writel(ms, SPI_TX_SRC_REG, cpu_to_le32(ms->local_tx_dma));
					/* re mapping to check if rx dma 4byte align */
					transfer_dma_mapping(ms, mode, ms->cur_transfer);
				}
			} else
		...
		if ((ms->cur_transfer->rx_buf != NULL)
			...
			if (ms->cur_transfer->rx_dma & (SPI_4B_ALIGN - 1)) {
				...
				dev_warn(&ms->pdev->dev,
					...
					ms->cur_transfer->rx_buf, ms->cur_transfer->rx_dma);
#endif
				if (ms->local_rx_buf == NULL)
					dev_warn(&ms->pdev->dev,"local_rx_buf is null\n");
				else {
					/* mapping local dma */
					local_dma_mapping(ms, DMA_RX);
					/* not 4byte align use local buf */
					spi_writel(ms, SPI_RX_DST_REG, cpu_to_le32(ms->local_rx_dma));
				}
			} else
...
static inline int spi_setup_packet(struct mt_spi_t *ms)
{
	...
	SPI_DBG("%s ms->cur_transfer->len = %d\n", __func__, ms->cur_transfer->len);
...
static int local_dma_mapping(struct mt_spi_t *ms, enum spi_dma_dir dir)
{
	...
	if (dir == DMA_TX) {
		ms->local_tx_dma = INVALID_DMA_ADDRESS;
		if (ms->local_tx_buf) {
			ms->local_tx_dma = dma_map_single(dev, (void *)ms->local_tx_buf,
						      SPI_LOCAL_BUF_LENGTH, DMA_TO_DEVICE);

			if (dma_mapping_error(dev, ms->local_tx_dma)) {
				dev_err(&ms->pdev->dev, "dma mapping local_tx_buf error.\n");
				return -ENOMEM;
			}
		}
	...
	if (dir == DMA_RX) {
		ms->local_rx_dma = INVALID_DMA_ADDRESS;
		if (ms->local_rx_buf) {
			ms->local_rx_dma = dma_map_single(dev, ms->local_rx_buf,
						      SPI_LOCAL_BUF_LENGTH, DMA_FROM_DEVICE);

			if (dma_mapping_error(dev, ms->local_rx_dma)) {
				if (ms->local_tx_buf)
					dma_unmap_single(dev, ms->local_tx_dma,
							 SPI_LOCAL_BUF_LENGTH, DMA_TO_DEVICE);

				dev_err(&ms->pdev->dev, "dma mapping local_rx_buf error.\n");
				return -ENOMEM;
			}
		}
	}
	SPI_DBG("local_dma_mapping success.\n");
	return 0;
}
...
static void local_dma_unmapping(struct mt_spi_t *ms, enum spi_dma_dir dir)
{
	...
	if (dir == DMA_TX) {
		if ((ms->local_tx_dma != INVALID_DMA_ADDRESS) && (ms->local_tx_dma != 0)) {
			dma_unmap_single(dev, ms->local_tx_dma, SPI_LOCAL_BUF_LENGTH, DMA_TO_DEVICE);
			SPI_INFO(dev, "local_tx_dma:0x%x,unmapping\n", (unsigned int)ms->local_tx_dma);
			ms->local_tx_dma = INVALID_DMA_ADDRESS;
		}
	}
	if (dir == DMA_RX) {
		if ((ms->local_rx_dma != INVALID_DMA_ADDRESS) && (ms->local_rx_dma != 0)) {
			dma_unmap_single(dev, ms->local_rx_dma, SPI_LOCAL_BUF_LENGTH, DMA_FROM_DEVICE);
			SPI_INFO(dev, "local_rx_dma:0x%x,unmapping\n", (unsigned int)ms->local_rx_dma);
			ms->local_rx_dma = INVALID_DMA_ADDRESS;
		}
	}
}
...
{
#ifdef SPI_NO_LENGTH_LIMITATION
	mt_spi_msg_del_extension_xfer(msg);
#endif
	list_del(&msg->queue);
	msg->status = status;
...
/*remove for spi transmit in suspend*/
...
#ifdef SPI_NO_LENGTH_LIMITATION
	ret = mt_spi_msg_add_extension_xfer(msg);
	if ((ret == 0) && (chip_config != NULL))
		chip_config->pause = 1;	/*need use pause mode*/
#endif
...
#define SPI_4B_ALIGN 0x4
	...
	if (!msg->is_dma_mapped) {
		if (xfer->rx_dma & (SPI_4B_ALIGN - 1)) {
			/* unmapping non 4byte align rx dma */
			transfer_dma_unmapping(ms, ms->cur_transfer);
			/* unmapping local dma */
			local_dma_unmapping(ms, DMA_RX);
			/* put data to local buf */
			if (ms->cur_transfer->rx_buf && ms->local_rx_buf)
				memcpy(ms->cur_transfer->rx_buf, ms->local_rx_buf, ms->cur_transfer->len);
		}
...
static int __init mt_spi_probe(struct platform_device *pdev)
{
	...
	ms->local_tx_buf = kzalloc(SPI_LOCAL_BUF_LENGTH, GFP_ATOMIC | GFP_DMA);
	ms->local_rx_buf = kzalloc(SPI_LOCAL_BUF_LENGTH, GFP_ATOMIC | GFP_DMA);

	/*init buf value*/
	memset(ms->local_tx_buf, 0, SPI_LOCAL_BUF_LENGTH);
	memset(ms->local_rx_buf, 0, SPI_LOCAL_BUF_LENGTH);
...
out_free:
	free_irq(irq, ms);
	if (ms->local_tx_buf) {
		kfree(ms->local_tx_buf);
		ms->local_tx_buf = NULL;
	}
	if (ms->local_rx_buf) {
		kfree(ms->local_rx_buf);
		ms->local_rx_buf = NULL;
	}
/*out:*/
...
static int mt_spi_suspend(struct platform_device *pdev, pm_message_t message)
{
	 ...
	 */
	/*remove for spi transmit when interrupt happen*/
