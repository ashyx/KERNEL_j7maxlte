...
{
	int ret;
	static u8 tx_buf[255] __aligned(4) = {0x00, };
...
int select_irq_msg(struct ssp_data *data)
{
	...
	chTempBuf = kzalloc(4, GFP_KERNEL | GFP_DMA);

	ret = ssp_spi_read(data->spi, chTempBuf, 4);
	...
	if(chLength == 0)
		ssp_errf("lengh is 0 - buf:0x%x 0x%x msg_type:%d mcuint1 %d",chTempBuf[0],chTempBuf[1],msg_type, gpio_get_value(data->mcu_int1)); 
	...
	case AP2HUB_WRITE:
		...
		if (!list_empty(&data->pending_list)) {
			...
			if (msg->dead && !msg->free_buffer) {
				msg->buffer = kzalloc(msg->length, GFP_KERNEL | GFP_DMA);
				msg->free_buffer = 1;
...
exit:
	...
	case HUB2AP_WRITE:
		...
		buffer = kzalloc(chLength, GFP_KERNEL | GFP_DMA);
		if (buffer == NULL) {
	...
	u8 uSensorType, u8 *uSendBuf, u16 uLength)
{
	...
	msg->options = AP2HUB_WRITE;
	msg->buffer = kzalloc(uLength + 1, GFP_KERNEL | GFP_DMA);
	msg->free_buffer = 1;
	...
	u8 uSensorType, u8 *uSendBuf, u16 uLength)
{
	...
	buffer = kzalloc(uLength + 1, GFP_KERNEL | GFP_DMA);
	msg->cmd = command;
	msg->length = uLength + 1;
...
int flush(struct ssp_data *data, u8 uSensorType)
{
	...
	buffer = kzalloc(1, GFP_KERNEL | GFP_DMA);
...
int get_batch_count(struct ssp_data *data, u8 uSensorType)
{
	...
	buffer = kzalloc(4, GFP_KERNEL | GFP_DMA);
...
int set_sensor_position(struct ssp_data *data)
{
	...
	msg->length = 3;
	msg->options = AP2HUB_WRITE;
	msg->buffer = kzalloc(3, GFP_KERNEL | GFP_DMA);
	msg->free_buffer = 1;
...
void set_proximity_threshold(struct ssp_data *data)
{
	...
	if(data->prox_type == PROX_LIGHT_CM36657) {
		...
		msg->buffer = (char *)kzalloc(8, GFP_KERNEL | GFP_DMA);
		msg->length = 8;
		...
		msg->buffer = (char *)kzalloc(4, GFP_KERNEL | GFP_DMA);
		msg->length = 4;
...
void set_light_coef(struct ssp_data *data)
{
	if(data->prox_type == PROX_LIGHT_CM36657) {
		...
		msg->options = AP2HUB_WRITE;
		msg->buffer = (char *) kzalloc(sizeof(data->light_coef), GFP_KERNEL | GFP_DMA);
		msg->free_buffer = 1;
...
void set_proximity_barcode_enable(struct ssp_data *data, bool bEnable)
{
	...
	msg->length = 1;
	msg->options = AP2HUB_WRITE;
	msg->buffer = kzalloc(1, GFP_KERNEL | GFP_DMA);
	msg->free_buffer = 1;
...
uint64_t get_sensor_scanning_info(struct ssp_data *data)
{
	...
	temp_result = (uint64_t *)kzalloc(8, GFP_KERNEL | GFP_DMA);
	
	msg->cmd = MSG2SSP_AP_SENSOR_SCANNING;
	msg->length = 8;
	msg->options = AP2HUB_READ;
	msg->buffer = (char *)temp_result;
	msg->free_buffer = 0;
...
unsigned int get_firmware_rev(struct ssp_data *data)
{
	...
	temp_result = (u32 *)kzalloc(4, GFP_KERNEL | GFP_DMA);
	*temp_result = SSP_INVALID_REVISION;
	...
	memcpy(&result, temp_result, sizeof(result));
...
int set_big_data_start(struct ssp_data *data, u8 type, u32 length)
{
	...
	msg->length = 5;
	msg->options = AP2HUB_WRITE;
	msg->buffer = kzalloc(5, GFP_KERNEL | GFP_DMA);
	msg->free_buffer = 1;
...
int set_time(struct ssp_data *data)
{
	...
	msg->length = 12;
	msg->options = AP2HUB_WRITE;
	msg->buffer = kzalloc(12, GFP_KERNEL | GFP_DMA);
	msg->free_buffer = 1;
...
int get_time(struct ssp_data *data)
{
	...
	buffer = kzalloc(12, GFP_KERNEL | GFP_DMA);
...
void set_gyro_cal_lib_enable(struct ssp_data *data, bool bEnable)
{
	...
	msg->length = 1;
	msg->options = AP2HUB_WRITE;
	msg->buffer = (char*) kzalloc(1, GFP_KERNEL | GFP_DMA);
	msg->free_buffer = 1;
