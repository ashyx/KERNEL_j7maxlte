...
{
	struct sensor_info sensorinfo[SENSOR_TYPE_MAX-NUM_SENSOR_INFO_1] = {
...
static void init_sensorlist(struct ssp_data *data)
{
	...
	if(data->prox_type != PROX_LIGHT_CM36657) {
		...
		memcpy(&(data->info[SENSOR_TYPE_PROXIMITY]), &prox_info, sizeof(struct sensor_info));
		memcpy(&(data->info[SENSOR_TYPE_PROXIMITY_RAW]), &prox_raw_info, sizeof(struct sensor_info));
	}
}
...
static int ssp_iio_configure_ring(struct iio_dev *indio_dev)
{
	...
	indio_dev->modes |= (INDIO_BUFFER_SOFTWARE | INDIO_BUFFER_TRIGGERED);
