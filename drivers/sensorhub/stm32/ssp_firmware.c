...
unsigned int get_module_rev(struct ssp_data *data)
{
	if(data->prox_type == PROX_LIGHT_CM36657)
		...
		return SSP_FIRMWARE_REVISION_STM_OLD;
}
				...
				struct stm32fwu_spi_cmd *cmd, u8 dummy_bytes)
{
	...
	// buffer aligned
    static char tx_buf __aligned(4);
    static char rx_buf __aligned(4);
...
{
	static u8 tx_buf[3] = {0,};
	static u8 rx_buf[3] = {0,};
	static u8 dummy_byte = 0;
	struct spi_message m;
	int ret;
#if BYTETOBYTE_USED
	int i;
...
{
	int ret;
	static u8 rx_buf[STM_MAX_BUFFER_SIZE] = {0, };
...
static int change_to_bootmode(struct ssp_data *data)
{
	...
	spi_set(data, SPI_MODE_0, NORM_SPI_HZ);
...
static int update_mcu_bin(struct ssp_data *data, int iBinType)
{
	...
	/* HW request: I2C line is reversed */
		if(data->prox_type == PROX_LIGHT_CM36657)
			...
			ret = load_kernel_fw_bootmode(data->spi, BL_FW_NAME_OLD);
		break;
...
int check_fwbl(struct ssp_data *data)
{
	...
	if ((data->curr_fw_rev == SSP_INVALID_REVISION)
		...
		if(data->prox_type == PROX_LIGHT_CM36657)		
			...
			fw_revision = SSP_FIRMWARE_REVISION_STM_OLD;
