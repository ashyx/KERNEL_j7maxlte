...
/************************************************************************/
/* SPI Communication setting for MTK SPI		*/
/************************************************************************/
void spi_set(struct ssp_data *data, char spi_mode, unsigned int speed)
{
	switch(spi_mode)
	{
		case SPI_MODE_0:
		...
		case SPI_MODE_1:
			...
			data->spi_conf.cpol = 0; /* SPI_1 MODE */
			data->spi_conf.cpha = 1; /* SPI_1 MODE */
			break;
	}
	
	data->spi->bits_per_word = 8;
	data->spi->max_speed_hz = speed;
	data->spi->chip_select = 0;

	// MTK need this setting
	...
	data->spi->controller_data = (void *)&data->spi_conf;
}
	...
	struct ssp_data *data)
{
	...
	if (IS_ERR(data->p)) {
		errorno = -EINVAL;
		pr_err("[SSP] %s: failed pinctrl_get\n", __func__);
		goto dt_exit;
	}

	data->spi2_miso_set_cfg =
		pinctrl_lookup_state(data->p, "spi2_miso_set_cfg");
	if (IS_ERR(data->spi2_miso_set_cfg)) {
		pr_err("[SSP] %s : could not get pins spi2_miso_set_cfg (%li)\n",
			__func__, PTR_ERR(data->spi2_miso_set_cfg));
		goto fail_pinctrl_get;
	}
	data->spi2_miso_clr_cfg =
		pinctrl_lookup_state(data->p, "spi2_miso_clr_cfg");
	if (IS_ERR(data->spi2_miso_clr_cfg)) {
		pr_err("[SSP] %s : could not get pins spi2_miso_clr_cfg (%li)\n",
			__func__, PTR_ERR(data->spi2_miso_clr_cfg));
		goto fail_pinctrl_get;
	}
	data->spi2_cs_set_cfg =
		pinctrl_lookup_state(data->p, "spi2_cs_set_cfg");
	if (IS_ERR(data->spi2_cs_set_cfg)) {
		pr_err("[SSP] %s : could not get pins spi2_cs_set_cfg (%li)\n",
			__func__, PTR_ERR(data->spi2_cs_set_cfg));
		goto fail_pinctrl_get;
	}
	data->spi2_cs_clr_cfg =
		pinctrl_lookup_state(data->p, "spi2_cs_clr_cfg");
	if (IS_ERR(data->spi2_cs_clr_cfg)) {
		pr_err("[SSP] %s : could not get pins spi2_cs_clr_cfg (%li)\n",
			__func__, PTR_ERR(data->spi2_cs_clr_cfg));
		goto fail_pinctrl_get;
	}
	data->spi2_mosi_set_cfg =
		pinctrl_lookup_state(data->p, "spi2_mosi_set_cfg");
	if (IS_ERR(data->spi2_mosi_set_cfg)) {
		pr_err("[SSP] %s : could not get pins spi2_mosi_set_cfg (%li)\n",
			__func__, PTR_ERR(data->spi2_mosi_set_cfg));
		goto fail_pinctrl_get;
	}
	data->spi2_mosi_clr_cfg =
		pinctrl_lookup_state(data->p, "spi2_mosi_clr_cfg");
	if (IS_ERR(data->spi2_mosi_clr_cfg)) {
		pr_err("[SSP] %s : could not get pins spi2_mosi_clr_cfg (%li)\n",
			__func__, PTR_ERR(data->spi2_mosi_clr_cfg));
		goto fail_pinctrl_get;
	}
	data->spi2_clk_set_cfg =
		pinctrl_lookup_state(data->p, "spi2_clk_set_cfg");
	if (IS_ERR(data->spi2_clk_set_cfg)) {
		pr_err("[SSP] %s : could not get pins spi2_clk_set_cfg (%li)\n",
			__func__, PTR_ERR(data->spi2_clk_set_cfg));
		goto fail_pinctrl_get;
	}
	data->spi2_clk_clr_cfg =
		pinctrl_lookup_state(data->p, "spi2_clk_clr_cfg");
	if (IS_ERR(data->spi2_clk_clr_cfg)) {
		pr_err("[SSP] %s : could not get pins spi2_clk_clr_cfg (%li)\n",
			__func__, PTR_ERR(data->spi2_clk_clr_cfg));
		goto fail_pinctrl_get;
	}
	...
	if (!IS_ERR(data->spi2_miso_set_cfg)) {
		status = pinctrl_select_state(data->p,
			data->spi2_miso_set_cfg);
		if (status)
			pr_err("[SSP] %s: can't set spi2_miso_set_cfg\n",
				__func__);
	}
	if (!IS_ERR(data->spi2_cs_set_cfg)) {
		status = pinctrl_select_state(data->p,
			data->spi2_cs_set_cfg);
		if (status)
			pr_err("[SSP] %s: can't set spi2_cs_set_cfg\n",
				__func__);
	}
	if (!IS_ERR(data->spi2_mosi_set_cfg)) {
		status = pinctrl_select_state(data->p,
			data->spi2_mosi_set_cfg);
		if (status)
			pr_err("[SSP] %s: can't set spi2_mosi_set_cfg\n",
				__func__);
	}
	if (!IS_ERR(data->spi2_clk_set_cfg)) {
		status = pinctrl_select_state(data->p,
			data->spi2_clk_set_cfg);
		if (status)
			pr_err("[SSP] %s: can't set spi2_clk_set_cfg\n",
				__func__);
	}
...
#endif
	/* prox type*/
	if (of_property_read_u32(np, "ssp,prox-type", &data->prox_type))
		data->prox_type = 0;

	ssp_info("prox-type = %d", data->prox_type);
...
*/
	...
	data->spi = spi;
	// for CONFIG_MACH_MT6757
	spi_set(data, SPI_MODE_1, NORM_SPI_HZ);
...
static int __init ssp_init(void)
{
	...
	status = spi_register_driver(&ssp_driver);
	if (status < 0) {
		pr_err("[SSP] %s spi_register_driver error.\n", __func__);
		return status;
	}

	pr_info("[SSP] %s is successful\n", __func__);
