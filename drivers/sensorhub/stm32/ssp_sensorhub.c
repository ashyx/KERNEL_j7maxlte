				...
				const char *data, int length)
{
	...
	log_str = kzalloc(log_size, GFP_ATOMIC | GFP_DMA);
	if (unlikely(!log_str)) {
				...
				size_t count, loff_t *pos)
{
	...
	buffer = kzalloc(count * sizeof(char), GFP_KERNEL | GFP_DMA);
	if (unlikely(!buffer)) {
...
void ssp_read_big_library_task(struct work_struct *work)
{
	...
	hub_data->big_events.library_data = kzalloc(big->length, GFP_ATOMIC | GFP_DMA);

	while (residue > 0) {
		buf_len = residue > DATA_PACKET_SIZE
			? DATA_PACKET_SIZE : residue;

		msg = kzalloc(sizeof(*msg), GFP_ATOMIC | GFP_DMA);
