 ...
 * muic_afc.c
 *
 * Copyright (C) 2014-2017 Samsung Electronics
...
int muic_restart_afc(void)
{
	...
	cancel_delayed_work(&gpmuic->afc_prepare_delay_enafc_work);
	schedule_delayed_work(&gpmuic->afc_prepare_delay_enafc_work, msecs_to_jiffies(200)); // 200ms
	pr_info("%s:afc_prepare_delay_enafc_work START\n", __func__);
...
static void muic_afc_prepare_delay_enafc_work(struct work_struct *work)
{
	struct afc_ops *afcops = gpmuic->regmapdesc->afcops;
...
#if GPIO_EXPANDER
	pr_info("%s: GPIO Expander can't read\n", __func__);
	sm5705_switch_val = !(gpio);
#else
...
int muic_get_switch_gpio_value(void)
{
	int sm5705_switch_gpio = gpmuic->sm5705_switch_gpio;
	int sm5705_switch_val;
	int ret;
	...
	ret = gpio_request(sm5705_switch_gpio, "SM5705_SWITCH_GPIO");
	if (ret) {
		pr_err("%s: failed to gpio_request SM5705_SWITCH_GPIO\n", __func__);
		return 0;
	}
	...
	return sm5705_switch_val;
}
EXPORT_SYMBOL(muic_get_switch_gpio_value);
#endif
		...
		const char *buf, size_t size)
{
	...
	pr_info("%s: param_val:%c\n", __func__, param_val);
#ifdef CONFIG_MUIC_HV
	ret = sec_set_param(CM_OFFSET + 1, param_val);

	if (ret < 0) {
		pr_err("%s: set_param failed - %c:(%d)\n",
...
void muic_init_afc_state(muic_data_t *pmuic)
{
	...
	INIT_DELAYED_WORK(&gpmuic->afc_prepare_delay_enafc_work, muic_afc_prepare_delay_enafc_work);
