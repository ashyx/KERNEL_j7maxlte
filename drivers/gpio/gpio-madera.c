...
static int madera_gpio_direction_in(struct gpio_chip *chip, unsigned int offset)
{
	...
	if (madera_gpio->output[offset]) {
		madera_gpio->output[offset] = false;
				     ...
				     unsigned int offset, int value)
{
	...
	if (!madera_gpio->output[offset]) {
		madera_gpio->output[offset] = true;

		ret = pm_runtime_get_sync(chip->dev);
		if (ret < 0) {
			dev_err(chip->dev, "Failed to resume device: %d\n",
				ret);
			dump_stack();
			return ret;
		}
