...
static int pcal6416a_gpio_direction_input(struct gpio_chip *gc, unsigned off)
{
	...
	pr_info("%s(gpio=[%d])\n", __func__, chip->pdata->gpio_start + off);
	mutex_lock(&chip->lock);
	/* input set bit 1  */
		...
		struct pcal6416a_platform_data *pdata)
{
	...
	ret = of_property_read_u32(np, "pcal6416a,addr", &pdata->addr);
	if (ret < 0) {
		pr_err("[%s]: Unable to read pcal6416a,ngpio\n", __func__);
		pdata->addr = 0;
		return ret;
		...
		const char *buf, size_t count)
{
	...
	gpio_pcal6416a = data->pdata->gpio_start + off;
		...
		struct device_attribute *devattr, char *buf)
{
	...
	pcal6416a_read_reg(data, PCAL6416A_INPUT, &read_input);
	pcal6416a_read_reg(data, PCAL6416A_DAT_OUT, &chip_state.reg_output);
	pcal6416a_read_reg(data, PCAL6416A_CONFIG, &chip_state.reg_config);
	pcal6416a_read_reg(data, PCAL6416A_DRIVE0, &chip_state.reg_drive0);
	pcal6416a_read_reg(data, PCAL6416A_DRIVE1, &chip_state.reg_drive1);
	pcal6416a_read_reg(data, PCAL6416A_EN_PULLUPDOWN,
					&chip_state.reg_enpullupdown);
	pcal6416a_read_reg(data, PCAL6416A_SEL_PULLUPDOWN,
...
{
	struct pcal6416a_chip *data = s->private;
	struct pcal6416a_chip chip_state;
	int i, drv_str;
	uint16_t read_input;
					...
					const struct i2c_device_id *id)
{
	...
	pcal6416a_probe_cnt += (pcal6416a_probe_cnt < NUM_PCAL6416A_CHIP - 1)
				 ? 1 : 0;
