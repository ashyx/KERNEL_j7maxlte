...
static int pcal6524_probe_cnt;

static struct dentry *expander_debugfs_root;
		...
		struct pcal6524_platform_data *pdata)
{
	...
	ret = of_property_read_u32(np, "pcal6524,addr", &pdata->addr);
	if (ret < 0) {
	...
	if (!gpio_is_valid(pdata->reset_gpio)) {
		pr_err("[%s]: No reset_gpio specified\n", __func__);
		return -EIO;
	}
#if 0
...
static int pcal6524_reset_chip(struct pcal6524_chip *dev)
{
	int retval;
		...
		const char *buf, size_t count)
{
	...
	gpio_pcal6524 = data->pdata->gpio_start + off;
		...
		struct device_attribute *devattr, char *buf)
{
	...
	for (i = 0; i < PCAL6524_PORT_CNT; i++) {
		pcal6524_read_reg(data, PCAL6524_INPUT + i, &read_input[i]);
		pcal6524_read_reg(data, PCAL6524_DAT_OUT + i, &chip_state.reg_output[i]);
		pcal6524_read_reg(data, PCAL6524_CONFIG + i, &chip_state.reg_config[i]);
		if (i < 2) {
			pcal6524_read_reg(data, PCAL6524_DRIVE0 + i, &chip_state.reg_drive0[i]);
			pcal6524_read_reg(data, PCAL6524_DRIVE1 + i, &chip_state.reg_drive1[i]);
			pcal6524_read_reg(data, PCAL6524_DRIVE2 + i, &chip_state.reg_drive2[i]);
		}
		pcal6524_read_reg(data, PCAL6524_EN_PULLUPDOWN + i, &chip_state.reg_enpullupdown[i]);
		pcal6524_read_reg(data, PCAL6524_SEL_PULLUPDOWN + i, &chip_state.reg_selpullupdown[i]);
	}

	for (i = 0; i <= PCAL6524_MAX_GPIO; i++) {
		bufp += sprintf(bufp, "Expander[%03d]", data->pdata->gpio_start + i);
		quot = i / 8;
		rest = i % 8;
...
static DEVICE_ATTR(expgpio0, 0644,
		show_pcal6524_gpio_state, store_pcal6524_gpio_inout);
#if (NUM_PCAL6524_CHIP > 1)
static DEVICE_ATTR(expgpio1, 0644,
		show_pcal6524_gpio_state, store_pcal6524_gpio_inout);
#endif

static struct device_attribute *expgpio_attrs[] = {
	&dev_attr_expgpio0,
#if (NUM_PCAL6524_CHIP > 1)
	&dev_attr_expgpio1,
#endif
...
{
	struct pcal6524_chip *data = s->private;
	struct pcal6524_chip chip_state;
	int i, drv_str;
	int quot = 0, rest = 0;
	uint8_t read_input[3];
	...
	for (i = 0; i <= PCAL6524_MAX_GPIO; i++) {
		seq_printf(s, "Expander[%03d]", data->pdata->gpio_start + i);
		quot = i / 8;
		rest = i % 8;
...
{
	return single_open(file, expander_show, inode->i_private);
}
					...
					const struct i2c_device_id *id)
{
	...
	while (1) {
		if (!pcal6524_probe_cnt)
			ret = pcal6524_reset_chip(dev);
		else
			ret = 0;

		if (ret)
			dev_err(dev->gpio_chip.dev,
				 "[%s]reset failed\n", __func__);
		else {
	...
	sprintf(msg, "expander%d", pdata->addr);
	dev->expander = sec_device_create(dev, msg);
	if (IS_ERR(dev->expander)) {
		dev_err(&client->dev,
	...
	ret = sysfs_create_file(&dev->expander->kobj,
			 &expgpio_attrs[pdata->addr]->attr);
	if (ret) {
		dev_err(&client->dev,
	...
	if (!expander_debugfs_root) {
		debugfs_root = debugfs_create_dir("expander", NULL);
		if (IS_ERR_OR_NULL(debugfs_root)) {
			dev_err(&client->dev,
					"Failed to create debugfs dir for expander\n");
			goto err_debug_dir;
		}
		expander_debugfs_root = debugfs_root;
	}

	sprintf(msg, "gpio%d", pdata->addr);
	dev->debugfs_reg = debugfs_create_file(msg, S_IFREG | S_IRUGO,
			 expander_debugfs_root, dev, &expander_operations);
	if (IS_ERR_OR_NULL(dev->debugfs_reg)) {
		dev_err(&client->dev, "Failed to create debugfs file for gpio\n");
		goto err_debug_file;
	}

	i2c_set_clientdata(client, dev);

	pcal6524_probe_cnt += (pcal6524_probe_cnt < NUM_PCAL6524_CHIP - 1)
				 ? 1 : 0;

	return 0;

err_debug_file:
	debugfs_remove_recursive(expander_debugfs_root);
err_debug_dir:
	sysfs_remove_file(&dev->expander->kobj,
			 &expgpio_attrs[pdata->addr]->attr);
err_destroy:
	sec_device_destroy(dev->expander->devt);
err:
...
static int pcal6524_gpio_remove(struct i2c_client *client)
{
	...
	if (!IS_ERR_OR_NULL(expander_debugfs_root))
		debugfs_remove_recursive(expander_debugfs_root);
	sysfs_remove_file(&dev->expander->kobj,
			 &expgpio_attrs[dev->pdata->addr]->attr);
	mutex_destroy(&dev->lock);
	kfree(dev);
	return 0;
}
