...
static int proximity_open_cancelation(struct cm36686_data *data)
{
	...
	err = vfs_read(cancel_filp, (char *)&buf,
	...
	struct device_attribute *attr, char *buf)
{
	...
	als_it = (cs_conf1 & 0xc0) >> 6;
	ps_duty = (ps_conf1 & 0xc0) >> 6;
	ps_pers = (ps_conf1 & 0x30) >> 4;
	ps_it = (ps_conf1 & 0x0e) >> 1;
	ps_smart_pers = (ps_conf3 & 0x10) >> 4;
...
irqreturn_t cm36686_irq_thread_fn(int irq, void *data)
{
	...
	if (cm36686->power_state & PROXIMITY_ENABLED) {
		...
		SENSOR_INFO("FACTORY: near/far=%d, ps code = %d\n",
				val, ps_data);
#else
		SENSOR_INFO("near/far=%d, ps code = %d (close:0, far:1)\n",
				val, ps_data);
	...
	struct cm36686_platform_data *pdata)
{
	...
	if ((ret < 0) || (!pdata->vled_same_vdd)) {
		...
		pdata->vled_same_vdd = 0;

		pdata->vled_ldo = of_get_named_gpio_flags(np, "cm36686,vled_ldo",
			0, &flags);
		if (pdata->vled_ldo < 0) {
	...
	ret = of_property_read_u32(np, "cm36686,ps_conf3_reg", &temp);
	if (ret < 0) {
