...
static DEVICE_ATTR(dhr_sensor_info, S_IRUGO, st_lsm6ds3_accel_dhr_sensor_info_show, NULL);
...
static int st_lsm6ds3_vdd_onoff(struct lsm6ds3_data *cdata, int onoff)
{
	/* ldo control */
	if (cdata->lsm6ds3_ldo_pin) {
		gpio_set_value(cdata->lsm6ds3_ldo_pin, onoff);
		if (onoff)
			msleep(20);
		return 0;
	}
...
static int st_lsm6ds3_parse_dt(struct lsm6ds3_data *cdata)
{
	...
	cdata->lsm6ds3_ldo_pin = of_get_named_gpio_flags(np,
				"st,vdd_ldo_pin", 0, &flags);
	if (cdata->lsm6ds3_ldo_pin < 0) {
		SENSOR_INFO("Cannot set vdd_ldo_pin through DTSI\n\n");
		cdata->lsm6ds3_ldo_pin = 0;
	} else {
		ret = gpio_request(cdata->lsm6ds3_ldo_pin, "st,vdd_ldo_pin");
		if (ret < 0)
			SENSOR_ERR("gpio %d request failed %d\n",
				cdata->lsm6ds3_ldo_pin, ret);
		else
			gpio_direction_output(cdata->lsm6ds3_ldo_pin, 0);
	}
...
exit_err_chip_id_or_i2c_error:
	if (cdata->lsm6ds3_ldo_pin)
		gpio_free(cdata->lsm6ds3_ldo_pin);
exit_of_node:
