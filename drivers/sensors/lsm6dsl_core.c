...
#if 0
static int lsm6dsl_set_odr(struct lsm6dsl_data *cdata, int sindex, u8 odr)
{
	int err = 0;
	...
	if (cdata->enabled & (1 << sindex)) {
		err = lsm6dsl_write_data_with_mask(cdata,
				lsm6dsl_odr_table.addr[sindex],
				lsm6dsl_odr_table.mask[sindex],
				odr, true);
		if (err < 0) {
			return err;
		}
...
static int lsm6dsl_get_step_c_data_fifo(struct lsm6dsl_data *cdata)
{
	...
	*/
	...
	if ((cdata->enabled & (1 <<LSM6DSL_STEP_COUNTER))) {
		...
		time_hi = (int)((timestamp_new & TIME_HI_MASK) >> TIME_HI_SHIFT);
		time_lo = (int)(timestamp_new & TIME_LO_MASK);
		input_report_rel(cdata->sc_input, REL_MISC, cdata->last_steps_c);
...
static void lsm6dsl_irq_management(struct work_struct *data_work)
{
	...
	if (src_value & LSM6DSL_SRC_STEP_COUNTER_DATA_VAL) {
	...
	if (src_value & LSM6DSL_SRC_STEP_DETECTOR_DATA_VAL) {
		input_report_rel(cdata->sd_input, REL_MISC, 1);
		input_sync(cdata->sd_input);
	}
				    ...
				    bool enable)
{
	if (enable) {
		...
		SENSOR_INFO("enable : %d, steps_c : %d, timestamp : %d \n",
				enable, (int)cdata->steps_c, (int)cdata->step_timestamp);
...
static int lsm6dsl_enable_sensors(struct lsm6dsl_data *cdata, int sindex)
{
	...
	case LSM6DSL_SIGN_MOTION:
		...
		cdata->sign_motion_event_ready = true;
				...
				int onoff, int sindex)
{
	...
	case LSM6DSL_ACCEL:
		...
		mode = LSM6DSL_SELF_TEST_POS_SIGN_VAL;
		break;
	case LSM6DSL_GYRO:
	...
	if(!onoff)
		mode = LSM6DSL_SELF_TEST_DISABLED_VAL;
		...
		s32 *NOST, s32 *ST, s32 *DIFF_ST)
{
	...
	err = lsm6dsl_set_selftest(cdata, ON, LSM6DSL_ACCEL);
	if (err < 0)
		goto XL_HW_SELF_EXIT;
	...
	err = lsm6dsl_set_selftest(cdata, OFF, LSM6DSL_ACCEL);
	if (err < 0)
		goto XL_HW_SELF_EXIT;
		...
		s32 *NOST, s32 *ST, s32 *DIFF_ST)
{
	...
	err = lsm6dsl_set_selftest(cdata, ON, LSM6DSL_GYRO);
	if (err < 0)
		goto G_HW_SELF_EXIT;
	...
	err = lsm6dsl_set_selftest(cdata, OFF, LSM6DSL_GYRO);
	if (err < 0)
		goto G_HW_SELF_EXIT;
			...
			char *out_str, u8 sindex)
{
	...
	case LSM6DSL_GYRO:
		...
		hw_st_ret = lsm6dsl_gyro_hw_selftest(cdata, NOST,
						ST, DIFF_ST);
		cal_pass = 1;
