...
#define DEFAULT_PRATE                   P_TIME_US(5000) /* 0x38 */
...
struct tmd3725_data {
	...
	int prox_vdd_ldo_pin;
	bool prox_cal_complete;
};

static int tmd3725_prox_vdd_onoff(struct tmd3725_data *taos, int onoff)
{
	/* ldo control */
	if (taos->prox_vdd_ldo_pin) {
		gpio_set_value(taos->prox_vdd_ldo_pin, onoff);
		if (onoff)
			msleep(20);
		return 0;
	}
...
static int tmd3725_parse_dt(struct tmd3725_data *taos, struct device *dev)
{
	...
	taos->prox_vdd_ldo_pin = of_get_named_gpio_flags(np,
				"taos,vdd_ldo_pin", 0, &flags);
	if (taos->prox_vdd_ldo_pin < 0) {
		SENSOR_INFO("Cannot set vdd_ldo_pin through DTSI\n");
		taos->prox_vdd_ldo_pin = 0;
	} else {
		ret = gpio_request(taos->prox_vdd_ldo_pin, "prox_vdd_en");
		if (ret < 0)
			SENSOR_ERR("gpio %d request failed %d\n",
				taos->prox_vdd_ldo_pin, ret);
		else
			gpio_direction_output(taos->prox_vdd_ldo_pin, 0);
	}

	taos->prox_vled_ldo_pin = of_get_named_gpio_flags(np,
				"taos,vled_ldo_pin", 0, &flags);
	if (taos->prox_vled_ldo_pin < 0) {
		SENSOR_INFO("Cannot set vled_ldo_pin through DTSI\n\n");
	...
	const struct i2c_device_id *id)
{
	...
	tmd3725_prox_vdd_onoff(taos, ON);
...
err_chip_id_or_i2c_error:
	...
	if (taos->prox_vdd_ldo_pin)
		gpio_free(taos->prox_vdd_ldo_pin);
