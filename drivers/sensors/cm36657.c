...
struct cm36657_data {
	...
	struct cm36657_platform_data *pdata;
...
static int cm36657_vdd_onoff(struct cm36657_platform_data *pdata, int onoff)
{
	/* ldo control */
	if (pdata->vdd_ldo_pin) {
		gpio_set_value(pdata->vdd_ldo_pin, onoff);
		if (onoff)
			msleep(20);
		return 0;
	}
...
static int cm36657_prox_vled_onoff(struct cm36657_data *cm36657, int onoff)
{
	...
	if (!cm36657->prox_vled) {
		SENSOR_INFO("VLED get regulator\n");
		cm36657->prox_vled =
			regulator_get(&cm36657->i2c_client->dev, "cm36657,vled");
...
static void cm36657_light_enable(struct cm36657_data *cm36657)
{
	...
	hrtimer_start(&cm36657->light_timer, ns_to_ktime(200 * NSEC_PER_MSEC),
		HRTIMER_MODE_REL);
}
...
static int cm36657_get_ps_adc_value(struct cm36657_data *cm36657, uint16_t *data)
{
	int ret = 0;
...
static int cm36657_get_canc_ps(struct cm36657_data *cm36657, int is_first)
{
	...
	if(ps_average < DEFAULT_ADC) {
		cm36657->pdata->offset = ps_average;
	} else if (is_first && (ps_average >= 3000)) {
		cm36657->pdata->offset = ps_average / 2;
	} else if (ps_average >= 4000) {
		cm36657->pdata->offset = 4000;
	} else {
		cm36657->pdata->offset = ps_average - DEFAULT_ADC;
	}

	SENSOR_INFO("ps_average : %d, offset : %d\n", ps_average, cm36657->pdata->offset);

	cm36657_i2c_write_word(cm36657, REG_PS_CANC, cm36657->pdata->offset);
...
static int cm36657_control_and_report(struct cm36657_data *cm36657, uint8_t mode)
{
  	int changeThd = 0;
  	int ps_status = 0;
  	int direction = 0;
	...
	if(mode == CONTROL_PS) {
		...
		cm36657_i2c_read_word(cm36657, INT_FLAG, &int_flag);
	...
	/* double check value and find the direction */
	if(ps_status == PS_CLOSE_AND_AWAY) {
		changeThd = 1;
		...
		if(ps_data <= cm36657_thd_tbl[cm36657->cur_lv][0]) {
            		direction = 1;
         	} else if(ps_data >= cm36657_thd_tbl[cm36657->cur_lv][1]) {
            		direction = 0;
         	}
	} else if(ps_status == PS_AWAY) {
		direction = 1;
		...
		if(ps_data == 0 || (ps_data <= LEVEL3_THDL && cm36657_thd_tbl[cm36657->cur_lv][0] == LEVEL3_THDL)) {
			cm36657_get_canc_ps(cm36657, 0);
		}
	} else if(ps_status == PS_CLOSE) {
		direction = 0;
	...
	SENSOR_INFO("ps_status = %d, ps_data = %d\n", ps_status, ps_data);
	SENSOR_INFO("proximity current_level = %d, current_direction = %d, (high_thd, low_thd) = (%d,%d) \n",
				cm36657->cur_lv, direction,
				cm36657_thd_tbl[cm36657->cur_lv][1],
				cm36657_thd_tbl[cm36657->cur_lv][0]);

	input_report_abs(cm36657->prox_input_dev, ABS_DISTANCE, direction);
	input_sync(cm36657->prox_input_dev);

	if((direction == 0) && (cm36657->cur_lv == 0 || cm36657->cur_lv == 1)) {
		cm36657->cur_lv++;
		changeThd = 1;
	} else if((direction == 1) &&(cm36657->cur_lv == 1 || cm36657->cur_lv == 2)) {
		cm36657->cur_lv--;
		changeThd = 1;
	}
	...
	if(changeThd == 1) {
		...
		/* Set THD */
		cm36657_i2c_write_word(cm36657, REG_PS_THDL, cm36657_thd_tbl[cm36657->cur_lv][0]);
		cm36657_i2c_write_word(cm36657, REG_PS_THDH, cm36657_thd_tbl[cm36657->cur_lv][1]);

		SENSOR_INFO("proximity next_level = %d, (high_thd, low_thd) = (%d,%d) \n",
				cm36657->cur_lv,
				cm36657_thd_tbl[cm36657->cur_lv][1],
				cm36657_thd_tbl[cm36657->cur_lv][0]);
...
static DEVICE_ATTR(poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
	cm36657_poll_delay_show, cm36657_poll_delay_store);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (thresh_value > 2) {
		...
		err = cm36657_i2c_write_word(cm36657, REG_PS_THDH, cm36657_thd_tbl[0][1]);
		if (err < 0) {
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (thresh_value > 2) {
		...
		err = cm36657_i2c_write_word(cm36657, REG_PS_THDH, cm36657_thd_tbl[1][0]);
		if (err < 0) {
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (thresh_value > 2) {
		...
		err = cm36657_i2c_write_word(cm36657, REG_PS_THDH, cm36657_thd_tbl[1][1]);
		if (err < 0) {
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (thresh_value > 2) {
		...
		err = cm36657_i2c_write_word(cm36657, REG_PS_THDH, cm36657_thd_tbl[2][0]);
		if (err < 0) {
...
irqreturn_t cm36657_irq_handler(int irq, void *data)
{
	...
	queue_work(cm36657->light_wq, &cm36657->work_prox);
...
static int cm36657_setup_reg(struct cm36657_data *cm36657)
{
	...
	err = cm36657_i2c_write_word(cm36657,
			REG_PS_CONF1,
			CM36657_PS_IT_4T | CM36657_PS_PERS_4 | CM36657_PS_START);
	if (err < 0) {
	...
	err = cm36657_i2c_write_word(cm36657,
			REG_PS_CONF3, CM36657_LED_I_110 | CM36657_PS_START2);
	if (err < 0) {
		SENSOR_ERR("cm36657_ps_reg is failed. %d\n", err);
		return err;
	}
	...
	/*disable PS func*/
	cm36657->ps_conf1_val = CM36657_PS_IT_4T  | CM36657_PS_PERS_4 | CM36657_PS_START;
...
{
	int rc = -EIO;
	struct cm36657_platform_data *pdata = cm36657->pdata;
...
static void cm36657_work_func_light(struct work_struct *work)
{
	...
	cm36657_i2c_read_word(cm36657, REG_ALS_GREEN_DATA, &cm36657->green_data);
...
static enum hrtimer_restart cm36657_prox_timer_func(struct hrtimer *timer)
{
	...
	queue_work(cm36657->prox_wq, &cm36657->work_prox_avg);
	hrtimer_forward_now(&cm36657->prox_timer, cm36657->prox_poll_delay);
	return HRTIMER_RESTART;
}

/* device tree parsing function */
static int cm36657_parse_dt(struct device *dev,
	struct cm36657_platform_data *pdata)
{
	...
	pdata->irq = of_get_named_gpio_flags(np, "cm36657,irq_gpio", 0, &flags);
	if (pdata->irq < 0) {
		SENSOR_ERR("get prox_int error\n");
		return -ENODEV;
	}

	pdata->vdd_ldo_pin = of_get_named_gpio_flags(np,
				"cm36657,vdd_ldo_pin", 0, &flags);
	if (pdata->vdd_ldo_pin < 0) {
		SENSOR_INFO("Cannot set vdd_ldo_pin through DTSI\n");
		pdata->vdd_ldo_pin = 0;
	} else {
		ret = gpio_request(pdata->vdd_ldo_pin, "cm36657_vdd_en");
		if (ret < 0)
			SENSOR_ERR("gpio %d request failed %d\n",
				pdata->vdd_ldo_pin, ret);
		else
			gpio_direction_output(pdata->vdd_ldo_pin, 0);
	}
	...
	const struct i2c_device_id *id)
{
	...
	struct cm36657_platform_data *pdata = NULL;
	...
	if (client->dev.of_node) {
		...
		sizeof(struct cm36657_platform_data), GFP_KERNEL);
		if (!pdata) {
			dev_err(&client->dev, "Failed to allocate memory\n");
			ret = -ENOMEM;
			goto done;
		}
		ret = cm36657_parse_dt(&client->dev, pdata);
		if (ret)
			goto err_devicetree;
	} else
...
static struct of_device_id cm36657_match_table[] = {
	{ .compatible = "cm36657",},
	{},
};
