...
int ist30xx_cmd_gesture(struct ist30xx_data *data, u16 value)
{
	...
	if (value == IST30XX_ENABLE) {
		...
		ret =
		    ist30xx_write_cmd(data, IST30XX_HIB_INTR_MSG,
				      IST30XX_LPM_VALUE);
		if (ret)
	...
	if (ret) {
		...
		if (value == IST30XX_ENABLE) {
			tsp_info("%s, spay : %d, aod : %d\n", __func__,
				 (data->g_reg.b.
				  ctrl & IST30XX_GETURE_CTRL_SPAY) ? 1 : 0,
				 (data->g_reg.b.
				  ctrl & IST30XX_GETURE_CTRL_AOD) ? 1 : 0);
		} else {
	...
	if (ret)
		...
		tsp_info("%s, spay : %d, aod : %d\n", __func__,
			 (value & IST30XX_SPAY) ? 1 : 0,
			 (value & IST30XX_AOD) ? 1 : 0);
#endif
...
{
	struct ist30xx_dt_data *dt_data = data->dt_data;
	struct regulator *regulator_avdd;
	int ret = 0;

	if (dt_data->vdd_en_flag) {
		gpio_direction_output(dt_data->vdd_en, en);
		tsp_info("%s gpio_direction_ouput:%d\n", __func__, en);
		return 0;
	} else {
		regulator_avdd = regulator_get(NULL, dt_data->regulator_avdd);
		if (IS_ERR(regulator_avdd)) {
			tsp_err("%s: Failed to get %s regulator.\n",
				__func__, dt_data->regulator_avdd);
