	...
	if (tsp->dir.swap_xy) {
		for (i = 0; i < tsp->ch_num.rx; i++) {
			...
			for (j = 0; j < tsp->ch_num.tx; j++) {
				...
				if (flag == NODE_FLAG_CDC) {
					val =
					    (int)node->cdc[(j * tsp->ch_num.rx)
							   + i];
					if (val < 0)
						val = 0;
				} else if (flag == NODE_FLAG_BASE) {
					val =
					    (int)node->base[(j * tsp->ch_num.rx)
							    + i];
					if (val < 0)
						val = 0;
				} else if (flag == NODE_FLAG_DIFF) {
		...
		for (i = 0; i < tsp->ch_num.tx; i++) {
			...
			for (j = 0; j < tsp->ch_num.rx; j++) {
				...
				if (flag == NODE_FLAG_CDC) {
					val =
					    (int)node->cdc[(i * tsp->ch_num.rx)
							   + j];
					if (val < 0)
						val = 0;
				} else if (flag == NODE_FLAG_BASE) {
					val =
					    (int)node->base[(i * tsp->ch_num.rx)
							    + j];
					if (val < 0)
						val = 0;
				} else if (flag == NODE_FLAG_DIFF) {
			    ...
			    struct TSP_NODE_BUF *node)
{
	...
	ret =
	    ist30xx_burst_read(data->client, addr, tmp_cdcbuf, node->len, true);
	if (unlikely(ret))
		goto err_read_node;
			...
			for (j = 0; j < tsp->ch_num.tx; j++) {
				if (j == 0) {
					if (ium)
						val =
						    node->
						    rom_self_cp[tsp->ch_num.tx +
								i];
					else
						val =
						    node->
						    memx_self_cp[tsp->ch_num.tx
								 + i];
					if (val < 0)
						val = 0;
					count +=
			 ...
			 bool ium, bool miscal)
{
	...
	if (ium) {
		...
		ret =
		    ist30xx_burst_read(data->client, addr, tmp_cpbuf, len,
				       true);
		if (unlikely(ret))
			goto err_read_cp;
		...
		ret =
		    ist30xx_burst_read(data->client, addr, tmp_self_cpbuf, len,
				       true);
		if (unlikely(ret))
			goto err_read_cp;
...
ssize_t ist30xx_frame_cdcbase(struct device * dev,
	...
	if (ret) {
		count =
		    snprintf(header, 128,
			     ...
			     tkey->ch_num[2].tx, tkey->ch_num[2].rx,
...
ssize_t ist30xx_refresh_show(struct device * dev,
			     struct device_attribute * attr, char *buf)
{
	int ret = 0;
	struct ist30xx_data *data = dev_get_drvdata(dev);
...
ssize_t ist30xx_timerms_store(struct device * dev,
			      struct device_attribute * attr, const char *buf,
			      size_t size)
{
	int ms;
			     ...
			     struct device_attribute * attr, char *buf)
{
	...
	size =
	    sprintf(buf,
		    ...
		    "%d %d %d %d %d %d %d %d", data->chip_id, tsp->dir.swap_xy,
		    ...
		    tkey->ch_num[3].tx, tkey->ch_num[3].rx, 255, 255,
		    data->cdc_addr);
			     ...
			     size_t size)
{
	...
	if (data->status.power) {
		...
		ist30xx_write_cmd(data, IST30XX_HIB_CMD,
				  (eHCOM_SET_DBG_MODE << 16) |
				  (data->debugging_mode & 0xFFFF));
	}
			       ...
			       size_t size)
{
	...
	if (data->status.power) {
		...
		ist30xx_write_cmd(data, IST30XX_HIB_CMD,
				  (eHCOM_SET_REC_MODE << 16) | (data->rec_mode &
								0xFFFF));
			       ...
			       size_t size)
{
	...
	sscanf(buf, "%c %d %x %d", &sec_info->cmd, &sec_info->addr,
	       &sec_info->val, &sec_info->size);
	...
	if (sec_info->cmd == DIRECT_CMD_WRITE) {
		ret =
		    ist30xx_write_sec_info(data, sec_info->addr, &sec_info->val,
					   1);
		if (ret)
...
ssize_t ist30xx_sec_info_show(struct device * dev,
			      struct device_attribute * attr, char *buf)
{
	int i, count = 0;
	int ret = 0;
