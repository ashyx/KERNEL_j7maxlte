...
int ist30xx_set_input_device(struct ist30xx_data *data)
{
	...
	input_set_abs_params(data->input_dev, ABS_MT_PRESSURE, 0, IST30XX_MAX_Z,
			     0, 0);
#endif
...
void ist30xx_special_cmd(struct ist30xx_data *data, int cmd)
{
	if (cmd == 0) {
		ist30xx_burst_read(data->client, IST30XX_HIB_GESTURE_REG,
				   data->g_reg.full,
				   sizeof(data->g_reg.full) / IST30XX_DATA_LEN,
				   true);
		...
		if (data->g_reg.b.evt & IST30XX_GETURE_EVT_SPAY) {
			...
			tsp_noti("AOD Trigger~ (%d, %d)\n", data->g_reg.b.evt_x,
				 data->g_reg.b.evt_y);
			data->g_reg.b.evt = 0;
			ist30xx_burst_write(data->client,
					    IST30XX_HIB_GESTURE_REG,
					    data->g_reg.full, 1);
	...
	if (press) {
		...
		if (data->tsp_touched[id] == false) {
			...
			ret =
			    ist30xx_read_reg(data->client,
					     IST30XX_HIB_TOUCH_STATUS, &status);
			if (ret == 0) {
		...
		if (data->tsp_touched[id] == true) {
			...
			tsp_noti("%s%d mc:%d (0x%02x) (%x,P%02XT%04X)\n",
				 TOUCH_UP_MESSAGE, id + 1, data->move_count[id],
...
void recording_data(struct ist30xx_data *data, bool idle)
{
	...
	ret =
	    ist30xx_burst_read(data->client, addr, buf32 + 1,
	...
	for (i = data->rec_start_ch.tx; i <= data->rec_stop_ch.tx; i++) {
		...
		ret = ist30xx_burst_read(data->client, addr,
					 ...
					 (i * tsp->ch_num.rx +
					  data->rec_start_ch.rx), len, true);
		if (ret)
			goto err_rec_fail;
	}

	ist30xx_recording_put_frame(data, buf32,
				    IST30XX_MAX_SELF_NODE_NUM +
				    IST30XX_MAX_NODE_NUM + 1);
...
static irqreturn_t ist30xx_irq_thread(int irq, void *ptr)
{
	...
	if (data->intr_debug1_size > 0) {
		...
		ist30xx_burst_read(data->client,
				   IST30XX_DA_ADDR(data->intr_debug1_addr),
	...
	if (data->intr_debug2_size > 0) {
		...
		ist30xx_burst_read(data->client,
				   IST30XX_DA_ADDR(data->intr_debug2_addr),
	...
	if (unlikely((*msg & IST30XX_EXCEPT_MASK) == IST30XX_EXCEPT_VALUE)) {
		...
		ret = ist30xx_burst_read(data->client, IST30XX_HIB_COORD,
					 &msg[offset], IST30XX_MAX_EXCEPT_SIZE,
					 true);
		if (unlikely(ret))
	...
	if (unlikely((*msg & CALIB_MSG_MASK) == CALIB_MSG_VALID)) {
		if (data->status.calib == 1) {
			ret =
			    ist30xx_burst_read(data->client,
					       IST30XX_HIB_INTR_MSG,
					       data->status.calib_msg,
					       IST30XX_MAX_CALIB_SIZE, true);
	...
	if (finger_cnt > 0) {
		ret =
		    ist30xx_burst_read(data->client, IST30XX_HIB_COORD,
	...
	if (data->intr_debug3_size > 0) {
		...
		ist30xx_burst_read(data->client,
				   IST30XX_DA_ADDR(data->intr_debug3_addr),
...
static void reset_work_func(struct work_struct *work)
{
	...
	struct ist30xx_data *data =
	    container_of(delayed_work, struct ist30xx_data,
			 work_reset_check);
...
static int ist30xx_parse_dt(struct device *dev, struct ist30xx_data *data)
{
	...
	data->dt_data->vdd_en = of_get_named_gpio(np, "imagis,vdd_en", 0);
	if (data->dt_data->vdd_en < 0) {
		data->dt_data->vdd_en_flag = 0;
		tsp_info("%s: vdd_en_flag %d\n", __func__,
			 data->dt_data->vdd_en_flag);
	} else {
		data->dt_data->vdd_en_flag = 1;
		tsp_info("%s: vdd_en_flag %d\n", __func__,
			 data->dt_data->vdd_en_flag);
	}
	...
	if (of_property_read_u32
	    (np, "imagis,pat_function", &data->dt_data->pat_function) >= 0)
		tsp_info("%s() pat_function: %d\n", __func__,
			 data->dt_data->pat_function);

	if (of_property_read_u32
	    (np, "imagis,afe_base", &data->dt_data->afe_base) >= 0)
		tsp_info("%s() afe_base: %x\n", __func__,
			 data->dt_data->afe_base);
#endif
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		data->dt_data =
		    kzalloc(sizeof(struct ist30xx_dt_data), GFP_KERNEL);
	...
	while (retry-- > 0) {
		ret =
		    ist30xx_read_reg(data->client, rSYS_CHIPID, &data->chip_id);
		data->chip_id &= 0xFFFF;
		if (unlikely(ret == 0)) {
