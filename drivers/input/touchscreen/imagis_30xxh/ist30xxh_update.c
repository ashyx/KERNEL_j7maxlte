...
/* get pat data */
	...
	tsp_info
	    ("%s : test_result=%x, status.update=%d update_keystring=%d initialized=%d\n",
	     __func__, data->test_result.data[0], data->status.update,
	     data->status.update_keystring, data->initialized);
	...
	ret =
	    ist30xx_read_sec_info(data, PAT_CAL_COUNT_FIX_VERSION, buf32, len);
	if (unlikely(ret)) {
		tsp_err("sec info read fail\n");
		kfree(buf32);
		return ret;
	...
	tsp_info
	    ("%s: pat_function=%d afe_base=%04X cal_count=%X tune_fix_ver=%04X\n",
...
/* pat_function(2)) */
		...
		} else if (data->dt_data->pat_function ==
			   PAT_CONTROL_FORCE_UPDATE) {
/* pat_function(5)) */
			data->cal_count = PAT_MAGIC_NUMBER;
		}
	} else {
...
/* pat_function(6)) */
	...
	temp =
	    ((data->cal_count & 0xff) << 16) | (data->fw.cur.fw_ver & 0xffff);
...
int ist30xx_auto_bin_update(struct ist30xx_data *data)
{
	...
	if (!data->dt_data->fw_path) {
		tsp_err("do not exist fw_path : %s \n", data->dt_data->fw_path);
		return 0;
	}
