...
int ist30xx_parse_cmcs_bin(const u8 * buf, const u32 size)
{
	...
	if (!strncmp
	    (ts_cmcs->magic1, IST30XX_CMCS_MAGIC, sizeof(ts_cmcs->magic1))
		...
		if (unlikely(!ts_cmcs->items.item)) {
	...
	for (i = 0; i < ts_cmcs->cmds.cnt; i++)
		tsp_verb(" (%d): 0x%08x, 0x%08x\n",
			 i, ts_cmcs->cmds.cmd[i].addr,
			 ts_cmcs->cmds.cmd[i].value);
	tsp_verb(" param\n");
	...
	tsp_verb("	chksum: 0x%08x\n, 0x%08x\n, 0x%08x, 0x%08x\n",
		 ...
		 ts_cmcs->param.cm_sensor_chksum,
	...
	tsp_verb
	    ("	x(%d,%d),y(%d,%d),gtx_x(%d,%d),gtx_y(%d,%d),key(%d,%d)\n",
	...
	for (i = 0; i < ts_cmcs->items.cnt; i++) {
		if (!strcmp(ts_cmcs->items.item[i].spec_type, "N")) {
			...
			tsp_verb
			    (" %s: screen(%4d, %4d), gtx(%4d, %4d), key(%4d, %4d)\n",
			     ts_cmcs->items.item[i].name,
...
int ist30xx_set_cmcs_fw(struct ist30xx_data *data, CMCS_PARAM param,
			u32 * buf32)
{
	int ret;
	int len;
			    ...
			    u32 * buf32, int mode)
{
	...
	if (mode == CMCS_FLAG_CM) {
		...
		if (len > 0) {
			ret =
			    ist30xx_burst_write(data->client, waddr, buf32,
						len);
			if (ret)
				return ret;
		...
		len = (param.cm_sensor2_size / 2) / IST30XX_DATA_LEN;
		buf32 += len;
		tsp_verb("%08x %08x %08x %08x\n",
			 buf32[0], buf32[1], buf32[2], buf32[3]);
		tsp_verb("%08x(%d)\n", waddr, len);
		...
		buf32 += len;
		waddr = IST30XX_DA_ADDR(param.sensor2_addr);
		len = (param.cs_sensor2_size / 2) / IST30XX_DATA_LEN;
		buf32 += len;
		tsp_verb("%08x %08x %08x %08x\n",
			 buf32[0], buf32[1], buf32[2], buf32[3]);
		tsp_verb("%08x(%d)\n", waddr, len);
		...
		buf32 += len;
		waddr = IST30XX_DA_ADDR(param.sensor2_addr);
		len = (param.jit_sensor2_size / 2) / IST30XX_DATA_LEN;
		buf32 += len;
		tsp_verb("%08x %08x %08x %08x\n",
			 buf32[0], buf32[1], buf32[2], buf32[3]);
		tsp_verb("%08x(%d)\n", waddr, len);
...
int ist30xx_cmcs_wait(struct ist30xx_data *data, int mode)
{
	...
	if (mode == CMCS_FLAG_CM)
		...
		val = CMCS_ENABLE_INT;
	else if (mode == CMCS_FLAG_HVDD)
		val = CMCS_ENABLE_HVDD;
	else
		return -EPERM;
...
hvdd_end:
	...
	tsp_warn("HVDD test fail\n");

	return -EPERM;
}

int ist30xx_item_test(struct ist30xx_data *data, CMCS_ITEM items,
		      CMCS_PARAM param, const char *mode, u32 flag, u32 * buf32,
		      s16 * buf)
{
	int i;
	int ret;
...
/* 6. CM Test */
		...
		ret =
		    ist30xx_item_test(data, ts_cmcs->items, ts_cmcs->param,
...
/* 7. CMJIT Test */
		...
		ret = ist30xx_item_test(data, ts_cmcs->items, ts_cmcs->param,
...
// 8. CS Test 
	...
	if (flag & CMCS_FLAG_HVDD) {
/* 9. HVDD Test */
		tsp_info("%s test!!\n", CMCS_HVDD);

		ist30xx_enable_irq(data);
		if (ist30xx_cmcs_wait(data, CMCS_FLAG_HVDD)) {
			tsp_info("%s test fail!\n", CMCS_HVDD);
			ret = -ENOEXEC;
		}
		ist30xx_disable_irq(data);
...
/* Channel */
	count =
	    sprintf(msg,
		    ...
		    tsp->screen.rx, tsp->gtx.num, tsp->gtx.ch_num[0],
		    tsp->gtx.ch_num[1], tsp->gtx.ch_num[2], tsp->gtx.ch_num[3],
		    tkey->key_num, tkey->ch_num[0].tx, tkey->ch_num[0].rx,
...
/* Slope */
	count += sprintf(msg, "%d %d %d %d %d %d %d %d %d %d ",
			 ts_cmcs->spec_slope.x_min, ts_cmcs->spec_slope.x_max,
			 ts_cmcs->spec_slope.y_min, ts_cmcs->spec_slope.y_max,
			 ts_cmcs->spec_slope.gtx_x_min,
			 ts_cmcs->spec_slope.gtx_x_max,
			 ts_cmcs->spec_slope.gtx_y_min,
			 ts_cmcs->spec_slope.gtx_y_max,
...
/* CR */
	count += sprintf(msg, "%d %d %d %d %d %d ",
			 ...
			 ts_cmcs->spec_cr.screen_max, ts_cmcs->spec_cr.gtx_min,
			 ts_cmcs->spec_cr.gtx_max, ts_cmcs->spec_cr.key_min,
...
/* sysfs: /sys/class/touch/cmcs/cmcs_hvdd */
ssize_t ist30xx_cmcs_hvdd_show(struct device * dev,
			       struct device_attribute * attr, char *buf)
{
	int ret;
	struct ist30xx_data *data = dev_get_drvdata(dev);
...
/* sysfs: /sys/class/touch/cmcs/cmcs_hvdd_custom */
ssize_t ist30xx_cmcs_hvdd_custom_show(struct device * dev,
				      struct device_attribute * attr, char *buf)
{
	int ret;
	int bin_size = 0;
	...
	mutex_lock(&data->lock);
	ret = ist30xx_cmcs_test(data, bin, bin_size, CMCS_FLAG_HVDD);
	mutex_unlock(&data->lock);
				 ...
				 struct device_attribute * attr, char *buf)
{
	...
	mutex_lock(&data->lock);
	ret = ist30xx_cmcs_test(data, bin, bin_size, CMCS_FLAG_ALL);
	mutex_unlock(&data->lock);
				 ...
				 struct device_attribute * attr, char *buf)
{
	...
	snprintf(fw_path, MAX_FILE_PATH, "/sdcard/%s", IST30XX_CMCS_NAME);
		       ...
		       const char *mode, bool fail_list)
{
		...
		if (!strcmp(ts_cmcs->items.item[i].name, mode)) {
			if (!strcmp(ts_cmcs->items.item[i].spec_type, "T")) {
				spec =
				    ...
				    &ts_cmcs->spec_item[i].spec_total;
				success = true;
				break;
			}
		}
	...
	if (fail_list) {
		count =
		    sprintf(msg,
			    "Spec: screen(%d~%d), gtx(%d~%d), key(%d~%d)\n",
...
static DEVICE_ATTR(cmcs_hvdd, S_IRUGO, ist30xx_cmcs_hvdd_show, NULL);
static DEVICE_ATTR(cmcs_hvdd_custom, S_IRUGO, ist30xx_cmcs_hvdd_custom_show,
		   NULL);
...
static struct attribute *cmcs_attributes[] = {
	&dev_attr_info.attr,
	&dev_attr_cmcs_hvdd.attr,
	&dev_attr_cmcs_hvdd_custom.attr,
...
/* /sys/class/touch/cmcs/... */
	...
	if (!data->dt_data->cmcs_path) {
		tsp_err("do not exist cmcs_path : %s \n", data->dt_data->cmcs_path);
		return 0;
	}
