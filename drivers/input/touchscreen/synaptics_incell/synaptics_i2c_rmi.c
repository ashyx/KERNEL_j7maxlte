...
static int synaptics_rmi4_pinctrl_configure(struct synaptics_rmi4_data *rmi4_data, int active);
...
static void synaptics_rmi4_release_all_finger(struct synaptics_rmi4_data *rmi4_data)
{
	...
	for (ii = 0; ii < rmi4_data->num_of_fingers; ii++) {
		...
		if (rmi4_data->finger[ii].state) {
			...
			time_diff = rmi4_data->time_released[ii].tv_sec - rmi4_data->time_pressed[ii].tv_sec;
			if (rmi4_data->time_longest < time_diff)
				rmi4_data->time_longest = time_diff;
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		if (finger_status != OBJECT_NOT_PRESENT) {
			...
			if (rmi4_data->finger[finger].state) {
				...
				if (rmi4_data->max_z_value < z)
					rmi4_data->max_z_value = z;
				if (rmi4_data->min_z_value > z)
					rmi4_data->min_z_value = z;
				rmi4_data->sum_z_value += z;

				rmi4_data->finger[finger].print_type = TOUCH_PRESS;

				if ((rmi4_data->fingers_already_present > 4) && (rmi4_data->check_multi == 0)) {
			...
			if (rmi4_data->finger[finger].state) {
				...
				time_diff = rmi4_data->time_released[finger].tv_sec - rmi4_data->time_pressed[finger].tv_sec;
				if (rmi4_data->time_longest < time_diff)
					rmi4_data->time_longest = time_diff;
...
static void	synaptics_init_product_info_v7(struct synaptics_rmi4_data *rmi4_data)
{
	...
	} else if (strncmp(rmi4_data->rmi4_mod_info.product_id_string, "TD4303", 6) == 0) {
		rmi4_data->product_id = SYNAPTICS_PRODUCT_ID_TD4303;
	} else {
...
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	/*if (gpio_is_valid(rmi4_data->board->tsp_rst)) {
			input_info(true, &rmi4_data->i2c_client->dev,
					"%s: reset start\n",
					__func__);
			gpio_direction_output(rmi4_data->board->tsp_rst, 0);
			msleep(rmi4_data->board->reset_delay_ms);
			gpio_direction_output(rmi4_data->board->tsp_rst, 1);
	}*/
...
static int synaptics_parse_dt(struct i2c_client *client)
{
	...
	pdata->tsp_rst= of_get_named_gpio(np, "synaptics,tsp_rst", 0);
	...
	if (of_property_read_string(np, "synaptics,pinctrl_active_name", &pdata->pinctrl_active_name)) {
		input_err(true, dev, "Failed to get pincrl_active_name property\n");
		pdata->pinctrl_active_name = "tsp_active";
	}
	
	if (of_property_read_string(np, "synaptics,pinctrl_suspend_name", &pdata->pinctrl_suspend_name)) {
		input_err(true, dev, "Failed to get pincrl_suspend_name property\n");
		pdata->pinctrl_suspend_name = "tsp_suspend";
	}

	input_info(true, dev, "irq :%d, irq_type: 0x%04x, project/model_name: %s/%s, device_num: %d, pinctrl active/suspend name : %s/%s \n",
			pdata->gpio, pdata->irq_type, pdata->project_name, pdata->model_name, pdata->device_num, pdata->pinctrl_active_name,
			pdata->pinctrl_suspend_name);
...
static int synaptics_rmi4_setup_drv_data(struct i2c_client *client)
{
	...
	rmi4_data = devm_kzalloc(&client->dev, sizeof(struct synaptics_rmi4_data), GFP_KERNEL);
	if (!rmi4_data) {
...
*/
	...
	if (!lcdtype) {
		input_err(true, &client->dev, "%s : Do not load driver due to : lcd detached \n",
			__func__);
		return -ENODEV;
	}
	...
	retval = synaptics_rmi4_pinctrl_configure(rmi4_data, 1);
	if (retval)
...
err_tsp_reboot:
	...
	rmi4_data->min_z_value = 0xFFFFFFFF;
...
static int synaptics_rmi4_pinctrl_configure(struct synaptics_rmi4_data *rmi4_data, int active)
{
	...
	input_dbg(true, &rmi4_data->i2c_client->dev, "%s: %d\n", __func__, active);

	if (active) {
		set_state_i2c =	pinctrl_lookup_state(rmi4_data->pinctrl, pdata->pinctrl_active_name);
		if (IS_ERR(set_state_i2c)) {
			input_err(true, &rmi4_data->i2c_client->dev,
				"%s: cannot get %s state\n", __func__, pdata->pinctrl_active_name);
			return PTR_ERR(set_state_i2c);
		}
	} else {
		set_state_i2c =	pinctrl_lookup_state(rmi4_data->pinctrl, pdata->pinctrl_suspend_name);
		if (IS_ERR(set_state_i2c)) {
			input_err(true, &rmi4_data->i2c_client->dev,
				"%s: cannot get %s state\n", __func__, pdata->pinctrl_suspend_name);
			return PTR_ERR(set_state_i2c);
		}
	}
	retval = pinctrl_select_state(rmi4_data->pinctrl, set_state_i2c);
	if (retval) {
		input_err(true, &rmi4_data->i2c_client->dev,
			"%s: cannot set pinctrl(int pin) %d state\n", __func__, active);
		return retval;
	}
...
static int synaptics_rmi4_stop_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	synaptics_rmi4_pinctrl_configure(rmi4_data, 0);
...
static int synaptics_rmi4_start_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	synaptics_rmi4_pinctrl_configure(rmi4_data, 1);
