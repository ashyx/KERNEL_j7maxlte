...
/* suport hybrid abs
 * abs : very high sensitibity, support Hover
 * hybrid abs : high sensitivty, high accuracy
 */
#define SUPPORT_HYBRID_INCELL
#undef SUPPORT_FULL_INCELL
...
#define AMP_OPEN_TEST_INT_DUR_ONE 40      // integration duration /* 145 for td43xx*/
#define AMP_OPEN_TEST_INT_DUR_TWO 10       // integration duration /* 10 for td43xx*/
...
#define HIC_FULL_RAW_CAP_TEST_LIMIT_LOWER 80
#define HIC_FULL_RAW_CAP_TEST_LIMIT_UPPER 3000

#define HIC_NOISE_TEST_LIMIT  28
#define HIC_NOISE_TEST_NUM_OF_FRAMES 50

#define HIC_TX_SHORT_TEST_LIMIT_NONDIAGONAL  30
#define HIC_TX_SHORT_TEST_LIMIT_DIAGONAL 200

#define HIC_RX_SHORT_TEST_LIMIT_NONDIAGONAL  30
#define HIC_RX_SHORT_TEST_LIMIT_DIAGONAL 200

#define HIC_HYBRID_SENSING_RW_TEST_LIMIT_RX_LOWER 700
#define HIC_HYBRID_SENSING_RW_TEST_LIMIT_RX_UPPER 40000
#define HIC_HYBRID_SENSING_RW_TEST_LIMIT_TX_LOWER 70000
#define HIC_HYBRID_SENSING_RW_TEST_LIMIT_TX_UPPER 110000
...
static int synaptics_rmi4_f54_reset(struct synaptics_rmi4_data *rmi4_data);
...
/*
 * param_count : integer, countable variable and cleared by hqm
 * param_id : string, ID information
 * param_check : string or integer, hardwawre defect check
 *
 * TCHK: TSP FLASH CHECKSUM
 * TITO: tsp pattern check
 * TMOD: ic panel modue id
 * TVEN: TSP IC vendor
 * TMUL: multi finger count
 * TCOM: i2c error count
 * THLD: TSP FINGER HOLDING TIME
 * TTCN: TSP FINGER PRESSED COUNT
 * TMXZ: TSP FINGER Z VALUE MAX
 * TMNZ: TSP FINGER Z VALUE MIN
 * TAVZ: TSP FINGER Z VALUE AVG
 */

static ssize_t param_id_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sec_cmd_data *sec = dev_get_drvdata(dev);
	struct synaptics_rmi4_data *rmi4_data = container_of(sec, struct synaptics_rmi4_data, sec);
	char vendorid[SEC_CMD_STR_LEN];

	memset(vendorid, 0x00, SEC_CMD_STR_LEN);

	snprintf(vendorid, 7, rmi4_data->firmware_name + 14);

	return snprintf(buf, PAGE_SIZE, "\"TMOD\":\"SY%02X%02X%02X%02X%02X%02X%02X\",\"TVEN\":\"%s\"", 
				rmi4_data->panel_revision, rmi4_data->ic_revision_of_ic, rmi4_data->fw_version_of_ic,
			0x00, 0x00, 0x00, 0x00, vendorid);
}

static ssize_t param_check_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sec_cmd_data *sec = dev_get_drvdata(dev);
	struct synaptics_rmi4_data *rmi4_data = container_of(sec, struct synaptics_rmi4_data, sec);
	...
	ret = snprintf(buf, PAGE_SIZE, "\"TITO\":\"%08d\",\"TCHK\":\"%d\"", 0x00, rmi4_data->checksum_result);
...
static ssize_t param_count_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sec_cmd_data *sec = dev_get_drvdata(dev);
	struct synaptics_rmi4_data *rmi4_data = container_of(sec, struct synaptics_rmi4_data, sec);
	unsigned int avg_z = 0;

	if (rmi4_data->all_finger_count > 0)
		avg_z = (rmi4_data->sum_z_value / rmi4_data->all_finger_count);

	return snprintf(buf, PAGE_SIZE,
		"\"TMUL\":\"%d\",\"TCOM\":\"%d\",\"THLD\":\"%ld\",\"TTCN\":\"%d\","
		"\"TMXZ\":\"%d\",\"TMNZ\":\"%d\",\"TAVZ\":\"%d\"",
		rmi4_data->multi_count, rmi4_data->comm_err_count,
		rmi4_data->time_longest, rmi4_data->all_finger_count,
		rmi4_data->max_z_value, rmi4_data->min_z_value, avg_z);
}

static ssize_t param_count_clear_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	rmi4_data->max_z_value = 0;
	rmi4_data->min_z_value = 0;
	rmi4_data->sum_z_value = 0;

	return count;
}

static DEVICE_ATTR(param_id, S_IRUGO, param_id_show, NULL);
static DEVICE_ATTR(param_check, S_IRUGO, param_check_show, NULL);
static DEVICE_ATTR(param_count, S_IRUGO | S_IWUSR | S_IWGRP, param_count_show, param_count_clear_store);

static struct attribute *param_attributes[] = {
	&dev_attr_param_id.attr,
	&dev_attr_param_check.attr,
	&dev_attr_param_count.attr,
	NULL,
};

static struct attribute_group param_attr_group = {
	.attrs = param_attributes,
};
...
#ifdef SUPPORT_HYBRID_INCELL
static void get_hic_rawcap(void *dev_data);
static void run_hic_rawcap_read(void *dev_data);
static void get_hic_delta(void *dev_data);
static void run_hic_delta_read(void *dev_data);
static void run_hic_rx_short_test(void *dev_data);
static void run_hic_tx_short_test(void *dev_data);
static void run_hic_abs_rawcap_read_all(void *dev_data);
static void run_hic_abs_rawcap_read(void *dev_data);
static void run_hic_interference_metric_read(void *dev_data);
#else
...
#ifdef SUPPORT_HYBRID_INCELL
	{SEC_CMD("get_rawcap", get_hic_rawcap),},
	{SEC_CMD("run_rawcap_read", run_hic_rawcap_read),},
	{SEC_CMD("get_delta",get_hic_delta),},
	{SEC_CMD("run_delta_read", run_hic_delta_read),},
	{SEC_CMD("run_rx_short_test", run_hic_rx_short_test),},
	{SEC_CMD("run_tx_short_test", run_hic_tx_short_test),},
	{SEC_CMD("run_abscap_read_all", run_hic_abs_rawcap_read_all),},
	{SEC_CMD("run_abscap_read", run_hic_abs_rawcap_read),},
	{SEC_CMD("run_hic_interference_metric_read", run_hic_interference_metric_read),},
#else
...
static bool is_report_type_valid(struct synaptics_rmi4_data *rmi4_data, enum f54_report_types report_type)
{
	...
	case F54_HIC_RX_SHORT:
	case F54_HIC_TX_SHORT:
		return true;
		break;
	default:
...
// it is not essential to change the dislpay ratio
/*
...
*/
	...
	if(!strcmp(pdata->project_name,"J3pop")) {
		if (rmi4_data->fw_version_of_ic == 0x08)
		{
			input_info(true, &rmi4_data->i2c_client->dev, "%s: In J327, firmware Ver 0x08 need to force update \n",
				__func__);
			return false;
		}
	}
...
static void fw_update(void *dev_data)
{
	...
	if (retval < 0) {
		snprintf(buf, sizeof(buf), "%s", tostring(NA));
	...
	snprintf(buf, sizeof(buf), "%s", tostring(OK));
...
static void get_fw_ver_bin(void *dev_data)
{
	...
	char buf[SEC_CMD_STR_LEN] = { 0 };
	sec_cmd_set_default_result(sec);
	snprintf(buf, sizeof(buf), "SY%02X%02X%02X",
...
static void get_config_ver(void *dev_data)
{
	...
	snprintf(buf, SEC_CMD_STR_LEN, "%s_SY_%02d%02d",
...
static void get_checksum_data(void *dev_data)
{
	...
	sec_cmd_set_default_result(sec);
	snprintf(buf, sizeof(buf), "%X", rmi4_data->rmi4_mod_info.pr_number);
	sec->cmd_state = SEC_CMD_STATUS_OK;
	sec_cmd_set_cmd_result(sec, buf, strlen(buf));
}
...
static void get_chip_vendor(void *dev_data)
{
	...
	snprintf(buf, sizeof(buf), "%s", tostring(SYNAPTICS));
	sec->cmd_state = SEC_CMD_STATUS_OK;
	sec_cmd_set_cmd_result(sec, buf, strlen(buf));
}
...
static void get_chip_name(void *dev_data)
{
	...
	case SYNAPTICS_PRODUCT_ID_S5807:
		snprintf(buf, sizeof(buf), "%s", tostring(S5807));
		break;
	case SYNAPTICS_PRODUCT_ID_S5806:
		snprintf(buf, sizeof(buf), "%s", tostring(S5806));
		break;
	case SYNAPTICS_PRODUCT_ID_TD4100:
		snprintf(buf, sizeof(buf), "%s", tostring(TD4100));
		break;
	case SYNAPTICS_PRODUCT_ID_TD4300:
		snprintf(buf, sizeof(buf), "%s", tostring(TD4300));
		break;
	case SYNAPTICS_PRODUCT_ID_TD4303:
		snprintf(buf, sizeof(buf), "%s", tostring(TD4303));
		break;
	default:
		snprintf(buf, sizeof(buf), "%s", tostring(NA));
	}
...
static void get_x_num(void *dev_data)
{
	...
	sec_cmd_set_default_result(sec);
	snprintf(buf, sizeof(buf), "%d", rmi4_data->f54->tx_assigned);
	sec->cmd_state = SEC_CMD_STATUS_OK;
	sec_cmd_set_cmd_result(sec, buf, strlen(buf));
}
...
static void get_y_num(void *dev_data)
{
	...
	sec_cmd_set_default_result(sec);
	snprintf(buf, sizeof(buf), "%d", rmi4_data->f54->rx_assigned);
	sec->cmd_state = SEC_CMD_STATUS_OK;
	sec_cmd_set_cmd_result(sec, buf, strlen(buf));
}
...
{
	struct synaptics_rmi4_f54_handle *f54 = rmi4_data->f54;
	struct sec_cmd_data *sec = &rmi4_data->sec;
	int node;
	...
	if (sec->cmd_param[0] < 0 ||
			sec->cmd_param[0] >= f54->tx_assigned ||
			sec->cmd_param[1] < 0 ||
			sec->cmd_param[1] >= f54->rx_assigned) {
		input_info(true, &rmi4_data->i2c_client->dev, "%s: parameter error: %u,%u\n",
			__func__, sec->cmd_param[0], sec->cmd_param[1]);
		node = -1;
	} else {
		node = sec->cmd_param[0] * f54->rx_assigned +
						sec->cmd_param[1];
...
static void get_hic_rawcap(void *dev_data)
{
	...
	report_data = rmi4_data->f54->factory_data->rawcap_data[node];

	snprintf(buf, sizeof(buf), "%d", report_data);
	sec->cmd_state = SEC_CMD_STATUS_OK;
...
static void run_hic_rawcap_read(void *dev_data)
{
	...
	int tx_num;
	int rx_num;
	signed short  *p_image = NULL;
	unsigned char cmd_state = SEC_CMD_STATUS_RUNNING;
	short max_value;
	short min_value;
	...
	p_image = kzalloc(tx_num * rx_num * sizeof(signed short), GFP_KERNEL);
	if (!p_image) {
		input_err(true, &rmi4_data->i2c_client->dev,
				"%s: Failed to alloc mem for p_image\n",
				__func__);
		retval = -ENOMEM;
		goto out;
	}
	...
	if (!synaptics_rmi4_f54_get_report_type(rmi4_data, F54_FULL_RAW_CAP_NO_RX_COUPLING)) {
		snprintf(buf, sizeof(buf), "%s", "run_hic_full_raw_test: Failed to get report type 20");
		cmd_state = SEC_CMD_STATUS_FAIL;
		goto sw_reset;
	}

	p_image[0] = (signed short)(f54->report_data[0] & 0xff ) | (signed short)(f54->report_data[1] << 8);
	max_value = min_value = p_image[0];
...
/*
			if ((p_image[i * rx_num + j] < HIC_FULL_RAW_CAP_TEST_LIMIT_LOWER) ||
				(p_image[i * rx_num + j] > HIC_FULL_RAW_CAP_TEST_LIMIT_UPPER)) {

				input_err(true, &rmi4_data->i2c_client->dev,
						"%s: fail at (tx%-2d, rx%-2d) = %-4d (limit = %d, %d)\n",
						__func__, i, j, p_image[i*rx_num + j],
						HIC_FULL_RAW_CAP_TEST_LIMIT_LOWER, HIC_FULL_RAW_CAP_TEST_LIMIT_UPPER);
			}
*/
			k += 2;

			max_value = max(max_value, p_image[i * rx_num + j]);
			min_value = min(min_value, p_image[i * rx_num + j]);

			input_info(true, &rmi4_data->i2c_client->dev, "%s: [%d] %d\n", __func__, i * rx_num + j, p_image[i * rx_num + j]);
		}
	}


	input_info(true, &rmi4_data->i2c_client->dev, "%s: min: %d, max: %d", __func__, min_value, max_value);
	snprintf(buf, sizeof(buf), "%d,%d", min_value, max_value);

	cmd_state = SEC_CMD_STATUS_OK;

	memcpy(rmi4_data->f54->factory_data->rawcap_data, p_image, tx_num * rx_num * sizeof(signed short));
...
static void get_hic_delta(void *dev_data)
{
	...
	report_data = rmi4_data->f54->factory_data->delta_data[node];
...
static void run_hic_delta_read(void *dev_data)
{
	...
	signed short *p_noise_max = NULL;
	signed short *p_noise_min = NULL;
	unsigned char *p_noise_data = NULL;
	signed short noise_delta;
	unsigned int buffer_size;

	short max_delta_value;
	short min_delta_value;
	unsigned char cmd_state = SEC_CMD_STATUS_RUNNING;

	tx_num = f54->tx_assigned;
	rx_num = f54->rx_assigned;
	buffer_size = tx_num * rx_num * 2;

	sec_cmd_set_default_result(sec);

	// allocate the internal buffer
	p_noise_data = kzalloc(buffer_size, GFP_KERNEL);
	if (!p_noise_data) {
		input_err(true, &rmi4_data->i2c_client->dev,
				"%s: Failed to alloc mem for p_noise_data\n",
				__func__);
		retval = -ENOMEM;
		goto out;
	}

	p_noise_max = (unsigned short *)kzalloc(buffer_size, GFP_KERNEL);
	if (!p_noise_max) {
		input_err(true, &rmi4_data->i2c_client->dev,
				"%s: Failed to alloc mem for p_noise_max\n",
				__func__);
		retval = -ENOMEM;
		goto out;
	}

	p_noise_min = (unsigned short *) kzalloc(buffer_size, GFP_KERNEL);
	if (!p_noise_min) {
		input_err(true, &rmi4_data->i2c_client->dev,
				"%s: Failed to alloc mem for p_noise_min\n",
				__func__);
		retval = -ENOMEM;
		goto out;
	}
	...
	for (repeat = 0 ; repeat < 20; repeat++) {
		...
		if (!synaptics_rmi4_f54_get_report_type(rmi4_data, F54_16BIT_IMAGE)) {
			snprintf(buf, sizeof(buf), "%s", "run_hic_noise_test: Failed to get report type 2");
			cmd_state = SEC_CMD_STATUS_FAIL;
			goto sw_reset;
		}

		memset(p_noise_data, 0x00, buffer_size);

		secure_memcpy(p_noise_data, buffer_size,
		...
		for (i = 0, offset = 0; i < tx_num; i++) {
			for (j = 0; j < rx_num; j++) {
				...
				report_data_16 =
					(signed short)p_noise_data[offset] +
					((signed short)p_noise_data[offset+1] << 8);
				offset += 2;

				p_noise_max[i*rx_num + j] =
					max_t(signed short, p_noise_max[i*rx_num + j], report_data_16);
				p_noise_min[i*rx_num + j] =
					min_t(signed short, p_noise_min[i*rx_num + j], report_data_16);
			}
		}
	}

	noise_delta = p_noise_max[0] - p_noise_min[0];
	max_delta_value = min_delta_value = noise_delta;

	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			noise_delta =
				p_noise_max[i*rx_num + j] - p_noise_min[i*rx_num + j];
/*
			if (noise_delta > HIC_NOISE_TEST_LIMIT )  {
				input_err(true, &rmi4_data->i2c_client->dev,
						"%s: fail at (tx%-2d, rx%-2d) = %-4d (limit = %d)\n",
						__func__, i, j, noise_delta, HIC_NOISE_TEST_LIMIT);
			}
*/
			max_delta_value = max(max_delta_value, noise_delta);
			min_delta_value = min(min_delta_value, noise_delta);

			input_info(true, &rmi4_data->i2c_client->dev, "%s: [%d] %d\n", __func__, i * rx_num + j, noise_delta);

		}
	}

	input_info(true, &rmi4_data->i2c_client->dev, "%s: min: %d, max: %d", __func__, min_delta_value, max_delta_value);
	snprintf(buf, sizeof(buf), "%d,%d", min_delta_value, max_delta_value);

	cmd_state = SEC_CMD_STATUS_OK;
	memcpy(rmi4_data->f54->factory_data->delta_data, p_noise_data, buffer_size);
...
static void run_hic_rx_short_test(void *dev_data)
{
	...
	short max_value;
	short min_value;
	short max_value_diagonal;
	short min_value_diagonal;

	rx_num = f54->rx_assigned;

	sec_cmd_set_default_result(sec);

	p_image = kzalloc(rx_num * rx_num * sizeof(signed short), GFP_KERNEL);
	if (!p_image) {
		input_err(true, &rmi4_data->i2c_client->dev,
				"%s: Failed to alloc mem for p_image\n",
				__func__);
	...
	if (!synaptics_rmi4_f54_get_report_type(rmi4_data, F54_HIC_RX_SHORT)) {
		snprintf(buf, sizeof(buf), "%s", "run_hic_rx_short_test: Failed to get report type 125");
		cmd_state = SEC_CMD_STATUS_FAIL;
		goto sw_reset;
	}

	for (i = 0, k = 0; i < rx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			p_image[i * rx_num + j] =
				(signed short)(f54->report_data[k] & 0xff ) | (signed short)(f54->report_data[k + 1] << 8);
	...
	max_value_diagonal = min_value_diagonal = p_image[0];
	max_value = min_value = p_image[1];

	for (i = 0, k = 0; i < rx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			if (i == j) {
				if (p_image[i * rx_num + j] < HIC_RX_SHORT_TEST_LIMIT_DIAGONAL) {
					input_err(true, &rmi4_data->i2c_client->dev,
							"%s: fail at (tx%-2d, rx%-2d) = %-4d (limit = %d)\n",
							__func__, i, j, p_image[i * rx_num + j], HIC_RX_SHORT_TEST_LIMIT_DIAGONAL);
				}


				max_value_diagonal = max(max_value_diagonal, p_image[i * rx_num + j]);
				min_value_diagonal = min(min_value_diagonal, p_image[i * rx_num + j]);
			}
			else {
				if (p_image[i * rx_num + j] > HIC_RX_SHORT_TEST_LIMIT_NONDIAGONAL) {
					input_err(true, &rmi4_data->i2c_client->dev,
							"%s: fail at (tx%-2d, rx%-2d) = %-4d (limit = %d)\n",
							__func__, i, j, p_image[i * rx_num + j], HIC_RX_SHORT_TEST_LIMIT_NONDIAGONAL);
				}
	...
	input_info(true, &rmi4_data->i2c_client->dev, "min: %d, max: %d, min: %d, max: %d", min_value, max_value, min_value_diagonal, max_value_diagonal);
	snprintf(buf, sizeof(buf), "%d,%d,%d,%d", min_value, max_value, min_value_diagonal, max_value_diagonal);
...
static void run_hic_tx_short_test(void *dev_data)
{
	...
	p_image = kzalloc(tx_num * tx_num * sizeof(signed short), GFP_KERNEL);
	if (!p_image) {
		input_err(true, &rmi4_data->i2c_client->dev,
				"%s: Failed to alloc mem for p_image\n",
				__func__);
	...
	if (!synaptics_rmi4_f54_get_report_type(rmi4_data, F54_HIC_TX_SHORT)) {
		snprintf(buf, sizeof(buf), "%s", "run_hic_tx_short_test: Failed to get report type 126");
		cmd_state = SEC_CMD_STATUS_FAIL;
		goto sw_reset;
	}
	...
	for (i = 0, k = 0; i < tx_num; i++) {
		for (j = 0; j < tx_num; j++) {
			if (i == j) {
				if (p_image[i * tx_num + j] < HIC_TX_SHORT_TEST_LIMIT_DIAGONAL) {
					input_err(true, &rmi4_data->i2c_client->dev,
							"%s: fail at (tx%-2d, rx%-2d) = %-4d (limit = %d)\n",
							__func__, i, j, p_image[i * tx_num + j], HIC_TX_SHORT_TEST_LIMIT_DIAGONAL);
				}


				max_value_diagonal = max(max_value_diagonal, p_image[i * tx_num + j]);
				min_value_diagonal = min(min_value_diagonal, p_image[i * tx_num + j]);
			}
			else {
				if (p_image[i * tx_num + j] > HIC_TX_SHORT_TEST_LIMIT_NONDIAGONAL) {
					input_err(true, &rmi4_data->i2c_client->dev,
							"%s: fail at (tx%-2d, rx%-2d) = %-4d (limit = %d)\n",
							__func__, i, j, p_image[i * tx_num + j], HIC_TX_SHORT_TEST_LIMIT_NONDIAGONAL);
				}

				max_value = max(max_value, p_image[i * tx_num + j]);
				min_value = min(min_value, p_image[i * tx_num + j]);
			}
		}
	}
...
static void run_hic_abs_rawcap_read_all(void *dev_data)
{
	...
	if (rmi4_data->touch_stopped)
		goto err_abs_mem_alloc;

	buf = kzalloc((f54->rx_assigned + f54->tx_assigned) * 6, GFP_KERNEL);
	if (!buf)
		goto err_abs_mem_alloc;

	disable_irq(rmi4_data->i2c_client->irq);

	if (!synaptics_rmi4_f54_get_report_type(rmi4_data, F54_ABS_HYBRID_RAW_CAP)) {
		snprintf(buf, SEC_CMD_STR_LEN, "FAIL");
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		goto sw_reset;
	}

	p_data = (int *)f54->report_data;

	for (ii = 0; ii < f54->rx_assigned; ii++) {
		input_info(true, &rmi4_data->i2c_client->dev, "%s: RX[%d] %d\n", __func__, ii, *p_data);
		snprintf(buffer, 7, "%d,", *p_data);
		strncat(buf, buffer, 7);

		p_data++ ;
	}

	for (ii = 0; ii < f54->tx_assigned; ii++) {
		input_info(true, &rmi4_data->i2c_client->dev, "%s: TX[%d] %d\n", __func__, ii, *p_data);
		snprintf(buffer, 7, "%d,", *p_data);
		strncat(buf, buffer, 7);
...
sw_reset:
	...
	sec_cmd_set_cmd_result(sec, buf, strlen(buf) - 1);
...
err_abs_mem_alloc:
	sec->cmd_state = SEC_CMD_STATUS_FAIL;
	snprintf(buffer, SEC_CMD_STR_LEN, "NG");
	sec_cmd_set_cmd_result(sec, buffer, SEC_CMD_STR_LEN);
}
...
static void run_hic_abs_rawcap_read(void *dev_data)
{
	...
	if (!synaptics_rmi4_f54_get_report_type(rmi4_data, F54_ABS_HYBRID_RAW_CAP)) {
		snprintf(buf, sizeof(buf), "%s", "run_hic_hybrid_raw_test: Failed to get report type 63");
		cmd_state = SEC_CMD_STATUS_FAIL;
		goto sw_reset;
	}

	p_data = (int *)f54->report_data;
	max_value_rx = min_value_rx = *p_data;
	for (i=0; i < rx_num; i++) {
/*
		if ((*p_data >= HIC_HYBRID_SENSING_RW_TEST_LIMIT_RX_UPPER) ||
			(*p_data <= HIC_HYBRID_SENSING_RW_TEST_LIMIT_RX_LOWER)) {
				input_err(true, &rmi4_data->i2c_client->dev,
						"%s: fail at rx%-2d = %-4d (limit = %d, %d)\n",
						__func__, i, *p_data,
						HIC_HYBRID_SENSING_RW_TEST_LIMIT_RX_LOWER, HIC_HYBRID_SENSING_RW_TEST_LIMIT_RX_UPPER);
		}
*/
		max_value_rx = max(max_value_rx, *p_data);
		min_value_rx = min(min_value_rx, *p_data);

		input_info(true, &rmi4_data->i2c_client->dev, "%s: RX[%d] %d\n", __func__, i, *p_data);

		p_data++ ;
	}
	max_value_tx = min_value_tx = *p_data;
	for (i=0; i < tx_num; i++) {
/*
		if ((*p_data >= HIC_HYBRID_SENSING_RW_TEST_LIMIT_TX_UPPER) ||
			(*p_data <= HIC_HYBRID_SENSING_RW_TEST_LIMIT_TX_LOWER)) {
				input_err(true, &rmi4_data->i2c_client->dev,
						"%s: fail at tx%-2d = %-4d (limit = %d, %d)\n",
						__func__, i, *p_data,
						HIC_HYBRID_SENSING_RW_TEST_LIMIT_TX_LOWER, HIC_HYBRID_SENSING_RW_TEST_LIMIT_TX_UPPER);
		}
*/
		max_value_tx = max(max_value_tx, *p_data);
		min_value_tx = min(min_value_tx, *p_data);
	...
	input_info(true, &rmi4_data->i2c_client->dev, "min_rx: %d, max_rx: %d, min_tx: %d, max_tx: %d", min_value_rx, max_value_rx, min_value_tx, max_value_tx);
	snprintf(buf, sizeof(buf), "%d,%d,%d,%d", min_value_rx, max_value_rx, min_value_tx, max_value_tx);
...
static void run_hic_interference_metric_read(void *dev_data)
{
	...
	char buf[SEC_CMD_STR_LEN] = { 0 };
	int retry;
	unsigned short im_min = 0xFFFF;
	unsigned short im_max = 0x0;
	unsigned short im = 0x00;
	int retval = 0;
	unsigned char command;
	...
	if (retval < 0) {
		input_err(true, &rmi4_data->i2c_client->dev,
			"%s: Failed to restore no sleep setting\n",
			__func__);
		snprintf(buf, SEC_CMD_STR_LEN, "%s", "Failed to restore no sleep setting");
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		goto exit;
	}
	...
	for (retry = 0 ; retry < 5; retry++) {
		retval = rmi4_data->i2c_read(rmi4_data, 
				f54->data_base_addr + DATA_IM_READ_OFFSET,
				data, 2);
		if (retval < 0)
			input_err(true, &rmi4_data->i2c_client->dev,
					"%s: Failed to read im value, %d\n",
					__func__, retval);

		im = data[0] | (data[1] << 8);

		input_info(true, &rmi4_data->i2c_client->dev, "(%d): %02X|%02X : %d",
					retry, data[1], data[0], im);
		im_max = max(im_max, im);
		im_min = min(im_min, im);
		msleep(20);
	}

	snprintf(buf, sizeof(buf), "%d,%d", im_min, im_max);
	sec->cmd_state = SEC_CMD_STATUS_OK;
...
static void run_rawcap_read(void *dev_data)
{
	...
	if (rmi4_data->touch_stopped || rmi4_data->sensor_sleep) {
		...
		snprintf(buf, sizeof(buf), "TSP is %s",
...
static void get_delta(void *dev_data)
{
	...
	node = check_rx_tx_num(rmi4_data);
	if (node < 0) {
		snprintf(buf, SEC_CMD_RESULT_STR_LEN, "%s", tostring(NA));
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		goto out;
	}
	...
	snprintf(buf, SEC_CMD_RESULT_STR_LEN, "%d", report_data);
	sec->cmd_state = SEC_CMD_STATUS_OK;
...
static void run_delta_read(void *dev_data)
{
	...
	if (rmi4_data->touch_stopped || rmi4_data->sensor_sleep) {
		input_err(true, &rmi4_data->i2c_client->dev, "%s: [ERROR] Touch is %s\n",
			__func__, rmi4_data->touch_stopped ? "stopped" : "Sleep state");
		snprintf(buf, SEC_CMD_STR_LEN, "TSP is %s",
			rmi4_data->touch_stopped ? "off" : "sleep");
	...
	if (!synaptics_rmi4_f54_get_report_type(rmi4_data, F54_16BIT_IMAGE)) {
		snprintf(buf, SEC_CMD_STR_LEN, "%s", "Error get report type");
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		goto out;
	}

	report_data = data->delta_data;
	memcpy(report_data, rmi4_data->f54->report_data, rmi4_data->f54->report_size);

	num_of_tx = rmi4_data->f54->tx_assigned;
	num_of_rx = rmi4_data->f54->rx_assigned;
...
static void run_abscap_read(void *dev_data)
{
	...
	if (!synaptics_rmi4_f54_get_report_type(rmi4_data, F54_ABS_RAW_CAP)) {
		snprintf(buf, SEC_CMD_STR_LEN, "%s", "Error get report type");
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		goto sw_reset;
	}
	...
	memcpy(report_data, rmi4_data->f54->report_data, rmi4_data->f54->report_size);
	memset(temp, 0, SEC_CMD_STR_LEN);
	memset(temp2, 0, SEC_CMD_STR_LEN);
	...
	for (ii = 0; ii < num_of_rx + num_of_tx ; ii++) {
		...
		if (ii == num_of_rx + num_of_tx -1)
			snprintf(temp, SEC_CMD_STR_LEN, "%d", *report_data);
		else
			snprintf(temp, SEC_CMD_STR_LEN, "%d,", *report_data);
		strncat(temp2, temp, SEC_CMD_STR_LEN);
		report_data++;
	}
	...
	snprintf(buf, SEC_CMD_STR_LEN, "%s", temp2);
	sec->cmd_state = SEC_CMD_STATUS_OK;
...
static void get_abscap_read_test(void *dev_data)
{
	...
	snprintf(buf, SEC_CMD_STR_LEN, "%d,%d,%d,%d",
...
static void run_absdelta_read(void *dev_data)
{
	...
	if (!synaptics_rmi4_f54_get_report_type(rmi4_data, F54_ABS_DELTA_CAP)) {
		snprintf(buf, SEC_CMD_STR_LEN, "%s", "Error get report type");
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		goto sw_reset;
	}
...
static void run_trx_short_test(void *dev_data)
{
	...
	if (!synaptics_rmi4_f54_get_report_type(rmi4_data, F54_TRX_SHORT_TDDI)) {
		snprintf(buf, sizeof(buf), "%s", "Error get report type");
		cmd_state = SEC_CMD_STATUS_FAIL;
		goto sw_reset;
	}
	...
	snprintf(buf, sizeof(buf), "%d,%d,%d,%d", min_value_2, max_value_2, min_value, max_value);
	cmd_state = SEC_CMD_STATUS_OK;
...
static void run_trx_open_test(void *dev_data)
{
	...
	/* input config */
	signed short in_iter_duration_1 = AMP_OPEN_TEST_INT_DUR_ONE;     // test input setting 1 : integration duration
	signed short in_iter_duration_2 = AMP_OPEN_TEST_INT_DUR_TWO;     // test input setting 2 : integration duration
...
/*	for (i = 0; i < tx_num; i++) {
	...
	snprintf(buf, sizeof(buf), "%d,%d,%d,%d", min_value, max_value, min_value_2, max_value_2);
...
static void clear_cover_mode(void *dev_data)
{
	...
	rmi4_data->f12.feature_enable = sec->cmd_param[0];

	if (sec->cmd_param[0] && rmi4_data->fast_glove_state)
		rmi4_data->f12.feature_enable |= FAST_GLOVE_DECTION_EN;
...
static void set_tsp_test_result(void *dev_data)
{
	...
	if (device_status != 0) {
		snprintf(buf, sizeof(buf), "%s", tostring(NR));
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
	...
	retval = synaptics_rmi4_set_tsp_test_result_in_config(rmi4_data, sec->cmd_param[0]);
	msleep(200);
...
static void get_tsp_test_result(void *dev_data)
{
	...
	if (result < TSP_FACTEST_RESULT_NONE || result > TSP_FACTEST_RESULT_PASS) {
		snprintf(buf, sizeof(buf), "%s", tostring(NG));
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		input_err(true, &rmi4_data->i2c_client->dev, "%s: failed [%d]\n",
				__func__, result);
		goto out;
...
int change_report_rate(struct synaptics_rmi4_data *rmi4_data, int mode)
{
	...
	input_info(true, &rmi4_data->i2c_client->dev,
		...
		sec->cmd_param[0] == SYNAPTICS_RPT_RATE_90HZ ? "90" :
		sec->cmd_param[0] == SYNAPTICS_RPT_RATE_60HZ ? "60" : "30",
...
static void report_rate(void *dev_data)
{
	...
	if (sec->cmd_param[0] < SYNAPTICS_RPT_RATE_START
		 || sec->cmd_param[0] >= SYNAPTICS_RPT_RATE_END) {
		snprintf(buf, sizeof(buf), "NG");
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		goto out;
	}

	if (rmi4_data->tsp_change_report_rate == sec->cmd_param[0]){
		input_info(true, &rmi4_data->i2c_client->dev, "%s: already change report rate[%d]\n",
			__func__, rmi4_data->tsp_change_report_rate);
		goto success;
	} else
		rmi4_data->tsp_change_report_rate = sec->cmd_param[0];
...
static void stylus_enable(void *dev_data)
{
	...
	if (rmi4_data->use_stylus != (sec->cmd_param[0] ? true : false)) {
		rmi4_data->use_stylus = sec->cmd_param[0] ? true : false;
		synpatics_rmi4_release_all_event(rmi4_data, RELEASE_TYPE_FINGER);
	}

	value = sec->cmd_param[0] ? 0x01 : 0x00;
		...
		unsigned char page)
{
	...
	// syna 20161208
	// the f55 allocation, this section was placed in the synaptics_rmi4_f54_init
	if (!rmi4_data->f55) {
...
static void synaptics_rmi4_remove_factory_mode(struct synaptics_rmi4_data *rmi4_data)
{
	...
	sysfs_remove_link(&rmi4_data->sec.fac_dev->kobj, "input");
	sysfs_remove_group(&rmi4_data->sec.fac_dev->kobj, &cmd_attr_group);
	sysfs_remove_group(&rmi4_data->sec.fac_dev->kobj, &param_attr_group);
	sec_cmd_exit(&rmi4_data->sec, SEC_CLASS_DEVT_TSP);
...
static int synaptics_rmi4_init_factory_mode(struct synaptics_rmi4_data *rmi4_data)
{
	...
	rmi4_data->f54->factory_data = factory_data;
	...
	retval = sec_cmd_init(&rmi4_data->sec, ft_cmds,
			ARRAY_SIZE(ft_cmds), SEC_CLASS_DEVT_TSP);
	if (retval < 0) {
		input_err(true, &rmi4_data->i2c_client->dev,
	...
	retval = sysfs_create_link(&rmi4_data->sec.fac_dev->kobj,
		&rmi4_data->input_dev->dev.kobj, "input");
	if (retval < 0) {
		input_err(true, &rmi4_data->i2c_client->dev, "%s [ERROR] sysfs_create_link\n", __func__);
		goto exit_sysfs_create_link;
	}
	
	retval = sysfs_create_group(&rmi4_data->sec.fac_dev->kobj, &cmd_attr_group);
	if (unlikely(retval)) {
		input_err(true, &rmi4_data->i2c_client->dev, "Failed to create touchkey sysfs group\n");
		goto err_create_sysfs;
	}

	retval = sysfs_create_group(&rmi4_data->sec.fac_dev->kobj, &param_attr_group);
	if (unlikely(retval)) {
		input_err(true, &rmi4_data->i2c_client->dev, "Failed to create parameter sysfs group\n");
...
err_create_sysfs_cmd:
	sysfs_remove_group(&rmi4_data->sec.fac_dev->kobj, &cmd_attr_group);	
err_create_sysfs:
	sysfs_delete_link(&rmi4_data->f54->factory_data->sec.fac_dev->kobj,
		&rmi4_data->input_dev->dev.kobj, "input");
//exit_cmd_attr_group:
//	sysfs_remove_group(&factory_data->fac_dev_ts->kobj, &cmd_attr_group);
exit_sysfs_create_link:
	sec_cmd_exit(&rmi4_data->f54->factory_data->sec, SEC_CLASS_DEVT_TSP);
...
static int synaptics_rmi4_f54_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	// syna 20161208
	// move the f55 handle allocation into the synaptics_rmi4_f54_set_regs
	// once detecting the function_55 during the pdt parsing
	/*
	if (!rmi4_data->f55) {
		f55 = kzalloc(sizeof(struct synaptics_rmi4_f55_handle), GFP_KERNEL);
		if (!f55) {
...
static int synaptics_rmi4_f54_reset(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!f55) {
		input_err(true, &rmi4_data->i2c_client->dev,
			"%s: Failed to alloc mem for f55\n",
					__func__);
		goto exit_free_f54mem;
	}
	rmi4_data->f55 = f55;

	retval = synaptics_rmi4_f54_scan_pdt(rmi4_data);
	if (retval < 0)
		goto exit_free_f55mem;
