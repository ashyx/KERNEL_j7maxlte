...
static int mms_dev_fs_open(struct inode *node, struct file *fp)
{
	struct mms_ts_info *info =
	    container_of(node->i_cdev, struct mms_ts_info, cdev);
...
static int mms_proc_table_data(struct mms_ts_info *info, u8 size,
			       ...
			       u8 buf_addr_h, u8 buf_addr_l, u8 row_num,
		...
		for (i_col = 0; i_col < lim_col; i_col++) {
			if (data_sign == 0) {
				if (data_size == 1) {
					...
					uValue =
					    (u16) (rbuf[data_size * i_col] |
						   (rbuf[data_size * i_col + 1]
						    << 8));
				} else if (data_size == 4) {
					uValue =
					    (u32) (rbuf[data_size * i_col] |
						   (rbuf[data_size * i_col + 1]
						    << 8) | (rbuf[data_size *
								  i_col +
								  2] << 16) |
						   (rbuf[data_size * i_col + 3]
						    << 24));
				} else {
					input_err(true, &info->client->dev,
						  "%s [ERROR] data_size [%d]\n",
				...
				if (data_size == 1) {
					...
					sValue =
					    (s16) (rbuf[data_size * i_col] |
						   (rbuf[data_size * i_col + 1]
						    << 8));
				} else if (data_size == 4) {
					sValue =
					    (s32) (rbuf[data_size * i_col] |
						   (rbuf[data_size * i_col + 1]
						    << 8) | (rbuf[data_size *
								  i_col +
								  2] << 16) |
...
int mms_run_test(struct mms_ts_info *info, u8 test_type)
{
	...
	if (mms_proc_table_data(info, size, data_type_size, data_type_sign,
				buf_addr_h, buf_addr_l, row_num, col_num,
				buffer_col_num, rotate, key_num)) {
		input_err(true, &info->client->dev,
			  "%s [ERROR] mms_proc_table_data\n", __func__);
		goto EXIT;
	}
	wbuf[0] = MIP_R0_CTRL;
	wbuf[1] = MIP_R1_CTRL_MODE;
 ...
 * Print channel info
 */
static ssize_t mms_sys_info(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	u8 data[255];
...
static DEVICE_ATTR(mode_glove, S_IRUGO | S_IWUSR | S_IWGRP,
		   mms_sys_glove_mode_show, mms_sys_glove_mode_store);
...
static DEVICE_ATTR(mode_palm_rejection, S_IRUGO | S_IWUSR | S_IWGRP,
		   mms_sys_palm_rejection_mode_show,
...
static DEVICE_ATTR(test_short, S_IRUGO, mms_sys_test_short, NULL);
