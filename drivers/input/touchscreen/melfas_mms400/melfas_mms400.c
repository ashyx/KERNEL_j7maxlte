	...
	if (atomic_read(&info->secure_enabled) == SECURE_TOUCH_ENABLED) {
		if (atomic_cmpxchg(&info->secure_pending_irqs, 0, 1) == 0) {
			input_info(true, &info->client->dev,
				   "%s: pending irq:%d\n", __func__, (int)
				   atomic_read(&info->secure_pending_irqs));
					...
					char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%d",
			atomic_read(&info->secure_enabled));
}
...
static int secure_touch_init(struct mms_ts_info *info)
{
	...
	if (IS_ERR(info->core_clk) || IS_ERR(info->iface_clk)) {
		input_err(true, &info->client->dev,
			  "%s: failed to get core clk: %ld\n", __func__,
			  PTR_ERR(info->core_clk));
		goto err_core_clk;
	}
...
static int mms_input_open(struct input_dev *dev)
{
	...
	if (!info->init) {
		input_info(true, &info->client->dev, "%s %s\n",
			   __func__,
			   info->lowpower_mode ? "exit LPM mode" : "");
#ifdef CONFIG_SECURE_TOUCH
...
int mms_get_fw_version(struct mms_ts_info *info, u8 *ver_buf)
{
	...
	input_info(true, &info->client->dev,
		   ...
		   __func__, ver_buf[0], ver_buf[1], ver_buf[2], ver_buf[3],
		   ver_buf[4]
...
static irqreturn_t mms_interrupt(int irq, void *dev_id)
{
	...
	if (info->ic_status == LPM_SUSPEND) {
		...
		ret =
		    wait_for_completion_interruptible_timeout
		    (&info->resume_done, msecs_to_jiffies(5 * MSEC_PER_SEC));
		if (ret == 0) {
			input_err(true, &info->client->dev,
	...
	if (category == 0) {
		if (size > event_size) {
			if (mms_i2c_read_next
			    (info, rbuf, (1 + event_size),
			     (size - event_size))) {
				input_err(true, &client->dev,
					  "%s [ERROR] Read next packet\n",
					  __func__);
				goto ERROR;
			}
		}
...
int mms_fw_update_from_storage(struct mms_ts_info *info, bool force)
{
	...
	if (0 < fw_size) {
		...
		input_info(true, &info->client->dev,
			   "%s - path [%s] size [%d]\n", __func__,
			   EXTERNAL_FW_PATH, (int)fw_size);

		if (nread != fw_size) {
			input_err(true, &info->client->dev,
				  "%s [ERROR] vfs_read - size[%d] read[%d]\n",
...
static int mms_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		info->dtdata =
		    devm_kzalloc(&client->dev,
				 sizeof(struct mms_devicetree_data),
				 GFP_KERNEL);
		if (!info->dtdata) {
			input_err(true, &client->dev,
	...
	if (ret) {
		input_err(true, &client->dev,
			  "%s [ERROR] mms_fw_update_from_kernel\n", __func__);
	}
#endif
