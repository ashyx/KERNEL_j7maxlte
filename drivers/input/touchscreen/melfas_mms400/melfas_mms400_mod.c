...
int mms_power_control(struct mms_ts_info *info, int enable)
{
	...
	input_info(true, &client->dev, "%s: %s", __func__,
		   enable ? "on" : "off");
...
void mms_input_event_handler(struct mms_ts_info *info, u8 sz, u8 *buf)
{
	...
	for (i = 1; i < sz; i += info->event_size) {
		...
		if (id < 0) {
			input_err(true, &client->dev,
				  "%s strange id value: [%d, %d, %d, %d, %d, %d, %d, %d]\n",
...
#endif
		{
			...
			if (info->finger[id].finger_state == 0) {
				...
				input_info(true, &client->dev,
					   ...
					   id, location_detect(info, y, 1),
					   location_detect(info, x, 0),
					   pressure, palm, touch_major,
					   touch_minor, info->touch_count);
#else
				input_err(true, &client->dev,
...
int mms_pinctrl_configure(struct mms_ts_info *info, int active)
{
	...
	if (active) {
		set_state_i2c =
		    pinctrl_lookup_state(info->pinctrl, "tsp_gpio_active");
		...
		set_state_i2c =
		    pinctrl_lookup_state(info->pinctrl, "tsp_gpio_suspend");
		if (IS_ERR(set_state_i2c)) {
			input_err(true, &info->client->dev,
...
int mms_parse_devicetree(struct device *dev, struct mms_ts_info *info)
{
	...
	ret =
	    of_property_read_string(np, "melfas,fw_name",
				    &info->dtdata->fw_name);
	if (ret < 0)
		info->dtdata->fw_name = NULL;

	ret =
	    of_property_read_u32(np, "melfas,bringup", &info->dtdata->bringup);
	if (ret < 0)
		info->dtdata->bringup = 0;

	info->dtdata->support_lpm =
	    of_property_read_bool(np, "melfas,support_lpm");

	input_info(true, dev,
		   "%s: max_x:%d max_y:%d int:%d irq:%d sda:%d scl:%d"
		   ...
		   info->dtdata->fw_name, info->dtdata->bringup,
		   info->dtdata->support_lpm);
...
void mms_config_input(struct mms_ts_info *info)
{
	...
	input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0, info->max_x - 1,
			     0, 0);
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0, info->max_y - 1,
			     0, 0);
#ifdef CONFIG_SEC_FACTORY
	input_set_abs_params(input_dev, ABS_MT_PRESSURE, 0, INPUT_PRESSURE_MAX,
			     0, 0);
#endif
...
static int __init mms_init_charger_notify(void)
{
	...
	if (!tsp_data) {
		input_info(true, &tsp_data->client->dev,
			   "%s tsp driver is null\n", __func__);
		return 0;
	}
	input_info(true, &tsp_data->client->dev,
		   "%s register extcon notifier for usb and ta\n", __func__);
...
void mms_set_cover_type(struct mms_ts_info *info)
{
	...
	if (ret) {
		input_err(true, &info->client->dev,
			  "%s [ERROR] mms_i2c_write %x %x %x, ret %d\n",
...
int mms_lowpower_mode(struct mms_ts_info *info, int on)
{
	...
	if (rbuf[0] != on) {
		input_err(true, &info->client->dev,
			  "%s [ERROR] not changed to %s mode, rbuf %x\n",
	...
	input_info(true, &info->client->dev, "%s: %s mode", __func__,
		   on ? "LPM" : "normal");
	return 0;
}
