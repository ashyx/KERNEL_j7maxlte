...
struct device *key_reset;
int check_vdkey_press;
EXPORT_SYMBOL(check_vdkey_press);
int check_pkey_press;
EXPORT_SYMBOL(check_pkey_press);
...
static ssize_t  sysfs_key_power_onoff_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int state = 0;
	state = check_pkey_press | check_vdkey_press;
...
static ssize_t reset_enable_store(struct device *dev, struct device_attribute *attr, const char *buf,size_t count)
{
	int err = 0;
...
static DEVICE_ATTR(sec_power_key_pressed, 0444 , sysfs_key_power_onoff_show, NULL);
...
/* This func use as mrdump now, if powerky use kpd eint it need to open another API */
	...
	sec_power_key = sec_device_create(pdev, "sec_power_key");
	if (IS_ERR(sec_power_key))
		dev_err(&pdev->dev, "%s: Failed to create device(sec_key)!\n", __func__);

	err = device_create_file(sec_power_key, &dev_attr_sec_power_key_pressed);
	if (err) {
		dev_err(&pdev->dev, "%s: Failed to create device file in sysfs entries(%s)!\n",
			__func__, dev_attr_sec_power_key_pressed.attr.name);
		}
...
{
	kpd_suspend = true;
#ifndef CONFIG_KPD_PMIC_RST_KEY_AS_WAKEUP_SRC
