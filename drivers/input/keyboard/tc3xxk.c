...
#define TC3XXK_GRIPCODE			0x0f
#define TC305K_GRIP1		0x30
#define TC305K_GRIP2		0x40
#define TC305K_GRIP3		0x50
#define TC305K_GRIP4		0x60

#define TC305K_GRIP_THD_PRESS		0x0
#define TC305K_GRIP_THD_RELEASE		0x2
#define TC305K_GRIP_THD_NOISE		0x4
#define TC305K_GRIP_CH_PERCENT		0x6
#define TC305K_GRIP_DIFF_DATA		0x8
#define TC305K_GRIP_RAW_DATA		0xA
#define TC305K_GRIP_BASELINE		0xC
#define TC305K_GRIP_TOTAL_CAP		0xE
...
static int tc3xxk_touchkey_power(struct tc3xxk_data *data, bool on)
{
	...
	if (data->pwr_reg) {
		...
		if (gpio_direction_output(dtdata->power_gpio, on) < 0) {
...
int tc3xxk_get_fw_version(struct tc3xxk_data *data, bool probe)
{
	...
	input_err(true, &client->dev, "[TK]%s read buf(%d)\n",
				__func__, buf);
	if (buf < 0) {
		while (retry--) {
...
#ifdef FEATURE_GRIP_FOR_SAR
	ret = i2c_smbus_read_byte_data(client, TC3XXK_GRIPCODE);
	if (ret < 0) {
		retry = 3;
		while (retry--) {
			input_err(true, &client->dev, "[TK] %s read fail ret=%d(retry:%d)\n",
...
*/
	...
	for (i = 0 ; i < data->key_num * 2 ; i++){
		...
		if (grip_handle_flag) {
			data->grip_event = data->grip_ev_val[i].grip_status;
			input_report_key(data->input_dev,
					data->grip_ev_val[i].grip_code, data->grip_ev_val[i].grip_status);
			input_info(true, &client->dev, "%s [%s] (0x%02X) ver0x%02x\n",
					data->grip_ev_val[i].grip_name,
					data->grip_ev_val[i].grip_status? "P" : "R", grip_val,
					data->fw_ver);
#ifdef CONFIG_SEC_FACTORY
			data->diff = read_tc3xxk_register_data(data, TC305K_GRIP_DIFF_DATA + TC305K_GRIP1, 0);
 ...
 *	...
 */
static int touchkey_vbus_notification(struct notifier_block *nb,
				 unsigned long cmd, void *data)
{
	muic_attached_dev_t vbus_type = *(muic_attached_dev_t *)data;

	input_info(true, &g_data->client->dev, "%s cmd=%lu, vbus_type=%d\n", __func__, cmd, vbus_type);

	switch (cmd) {
	case MUIC_NOTIFY_CMD_DETACH:
	case MUIC_NOTIFY_CMD_LOGICALLY_DETACH:
		if (vbus_type == ATTACHED_DEV_OTG_MUIC)
			tc3xxk_mode_enable(g_data->client, TC3XXK_CMD_TA_OFF);
		break;
	case MUIC_NOTIFY_CMD_ATTACH:
	case MUIC_NOTIFY_CMD_LOGICALLY_ATTACH:
		if (vbus_type == ATTACHED_DEV_OTG_MUIC)
			tc3xxk_mode_enable(g_data->client, TC3XXK_CMD_TA_ON);
		break;
	default:
		break;
	}
...
static ssize_t grip_threshold_func(struct tc3xxk_data *data, u8 cmd, char *buf)
{
	int ret;

	mutex_lock(&data->lock_fac);
	ret = read_tc3xxk_register_data(data, TC305K_GRIP_THD_PRESS + cmd, 0);
	mutex_unlock(&data->lock_fac);
	if (ret < 0) {
	...
	mutex_lock(&data->lock_fac);
	ret = read_tc3xxk_register_data(data, TC305K_GRIP_THD_RELEASE + cmd, 0);
	mutex_unlock(&data->lock_fac);
	if (ret < 0) {
	...
	mutex_lock(&data->lock_fac);
	ret = read_tc3xxk_register_data(data, TC305K_GRIP_THD_NOISE + cmd, 0);
	mutex_unlock(&data->lock_fac);
	if (ret < 0) {
...
{
	struct tc3xxk_data *data = dev_get_drvdata(dev);
	return grip_threshold_func(data, TC305K_GRIP1, buf);
}
...
{
	struct tc3xxk_data *data = dev_get_drvdata(dev);
	return grip_threshold_func(data, TC305K_GRIP2, buf);
}

static ssize_t grip_total_cap_func(struct tc3xxk_data *data, u8 cmd, char *buf)
{
	int ret;
	ret = i2c_smbus_read_byte_data(data->client, TC305K_GRIP_TOTAL_CAP + cmd);
	if (ret < 0) {
		input_err(true, &data->client->dev, "%s fail(%d)\n", __func__, ret);
		return sprintf(buf, "%d\n", 0);
	}
...
{
	struct tc3xxk_data *data = dev_get_drvdata(dev);
	return grip_total_cap_func(data, TC305K_GRIP1, buf);
}
...
{
	struct tc3xxk_data *data = dev_get_drvdata(dev);
	return grip_total_cap_func(data, TC305K_GRIP2, buf);
}
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = i2c_smbus_read_byte_data(data->client, TC305K_GRIP_REF_CAP);

	input_info(true, &data->client->dev, "%s read(%d)\n", __func__, ret);

	return sprintf(buf, "%d\n", ret);
}

static ssize_t grip_func(struct tc3xxk_data *data, u8 cmd, char *buf)
{
	int ret;

	mutex_lock(&data->lock_fac);
	ret = read_tc3xxk_register_data(data, TC305K_GRIP_DIFF_DATA + cmd, 0);
	mutex_unlock(&data->lock_fac);
	if (ret < 0) {
		input_err(true, &data->client->dev, "%s fail(%d)\n", __func__, ret);
		data->grip_s1 = 0;
...
{
	struct tc3xxk_data *data = dev_get_drvdata(dev);
	return grip_func(data, TC305K_GRIP1, buf);
}
...
{
	struct tc3xxk_data *data = dev_get_drvdata(dev);
	return grip_func(data, TC305K_GRIP2, buf);
}

static ssize_t grip_baseline_func(struct tc3xxk_data *data, u8 cmd, char *buf)
{
	int ret;

	mutex_lock(&data->lock_fac);
	ret = read_tc3xxk_register_data(data, TC305K_GRIP_BASELINE + cmd, 0);
	mutex_unlock(&data->lock_fac);
	if (ret < 0) {
		input_err(true, &data->client->dev, "%s fail(%d)\n", __func__, ret);
...
{
	struct tc3xxk_data *data = dev_get_drvdata(dev);
	return grip_baseline_func(data, TC305K_GRIP1, buf);
}
...
{
	struct tc3xxk_data *data = dev_get_drvdata(dev);
	return grip_baseline_func(data, TC305K_GRIP2, buf);
}

static ssize_t grip_raw_func(struct tc3xxk_data *data, u8 cmd, char *buf)
{
	int ret;

	mutex_lock(&data->lock_fac);
	ret = read_tc3xxk_register_data(data, TC305K_GRIP_RAW_DATA + cmd, 0);
	mutex_unlock(&data->lock_fac);
	if (ret < 0) {
		input_err(true, &data->client->dev, "%s fail(%d)\n", __func__, ret);
		data->grip_raw1 = 0;
...
{
	struct tc3xxk_data *data = dev_get_drvdata(dev);
	return grip_raw_func(data, TC305K_GRIP1, buf);
}
...
{
	struct tc3xxk_data *data = dev_get_drvdata(dev);
	return grip_raw_func(data, TC305K_GRIP2, buf);
}
...
static int tc3xxk_set_power(struct tc3xxk_data *data)
{
	struct tc3xxk_devicetree_data *dtdata = data->dtdata;
	struct device *dev = &data->client->dev;
	...
	reg = regulator_get(dev, dtdata->regulator);
	if (IS_ERR(reg)) {
	...
	if (gpio_is_valid(dtdata->power_gpio)) {
		input_info(true, dev, "succesed to get vtouch_2p2 gpio\n");
		gpio_direction_output(dtdata->power_gpio, 1);
		ret = 0;
		return ret;
	} else {
		input_info(true, dev, "failed to get vtouch_2p2 gpio\n");
	}
...
static int tc3xxk_dev_pinctrl_configure(struct tc3xxk_data *data, bool active)
{
	...
	set_state = pinctrl_lookup_state(data->pinctrl, active ? "touchkey_active" : "touchkey_suspend");
	if (IS_ERR(set_state)) {
	...
	if (active) {
		gpio_direction_input(data->dtdata->sda_gpio);
		gpio_direction_input(data->dtdata->scl_gpio);
		gpio_direction_input(data->dtdata->irq_gpio);
	}
			...
			struct tc3xxk_devicetree_data *dtdata)
{
	...
	if (!gpio_is_valid(dtdata->scl_gpio)) {
		input_err(true, dev, "%s: Failed to get sda-gpio\n", __func__);
		return -EINVAL;
	}
	...
	ret = of_property_read_string(np, "coreriver,regulator-name", &dtdata->regulator);
	if (ret < 0)
		dtdata->regulator = NULL;
	...
	input_info(true, dev, "%s: scl:%d, sda:%d, irq:%d, vdd_gpio:%d, reg booton:%d, use led:%d(led_en:%d),"
			" sub_det:%d, sensing ch:%d, bringup:%d, tc300k:%d, fw name:%s\n",
		...
		const struct i2c_device_id *id)
{
	...
	ret = tc3xxk_dev_pinctrl_configure(data, true);
	if (ret) 
		input_err(true, dev, "Failed pinctrl_configure check it\n");
	
	input_dev = tc3xxk_set_input_dev_data(data);
	if (!input_dev) {
		input_err(true, dev, "Failed to set input device data\n");
		return -ENOMEM;
	}
	...
	if (ret) {
		input_err(true, dev, "Failed to request gpio\n");
		return ret;
	}

	ret = tc3xxk_set_power(data);
	if (ret) {
		input_err(true, dev, "Failed to set power\n");
		return ret;
	}
	...
	client->irq = gpio_to_irq(dtdata->irq_gpio);
	...
	if (ret)
		input_err(true, dev, "Failed to create sysfs group\n");
	...
	if (ret < 0)
		input_err(true, dev, "Failed to create input symbolic link\n");
...
#ifdef CONFIG_MUIC_NOTIFIER
	g_data = data;
	muic_notifier_register(&data->muic_nb, touchkey_vbus_notification,
